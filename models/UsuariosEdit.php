...
class UsuariosEdit extends Usuarios
{
    ...
    // Page object name
    public $PageObjName = "UsuariosEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "UsuariosEdit";
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$Security->canAdmin()) { // System admin
                ...
                $curVal = trim(strval($this->nivel->CurrentValue));
                if ($curVal != "") {
                    $this->nivel->ViewValue = $this->nivel->lookupCacheOption($curVal);
                } else {
                    $this->nivel->ViewValue = $this->nivel->Lookup !== null && is_array($this->nivel->lookupOptions()) && count($this->nivel->lookupOptions()) > 0 ? $curVal : null;
                }
                if ($this->nivel->ViewValue !== null) { // Load from cache
                    $this->nivel->EditValue = array_values($this->nivel->lookupOptions());
                } else { // Lookup from database
                    if ($curVal == "") {
                        $filterWrk = "0=1";
                    } else {
                        $filterWrk = SearchFilter($this->nivel->Lookup->getTable()->Fields["userlevelid"]->searchExpression(), "=", $this->nivel->CurrentValue, $this->nivel->Lookup->getTable()->Fields["userlevelid"]->searchDataType(), "");
                    }
                    $sqlWrk = $this->nivel->Lookup->getSql(true, $filterWrk, '', $this, false, true);
                    $conn = Conn();
                    $config = $conn->getConfiguration();
                    $config->setResultCache($this->Cache);
                    $rswrk = $conn->executeCacheQuery($sqlWrk, [], [], $this->CacheProfile)->fetchAll();
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->nivel->Visible && $this->nivel->Required) {
                if ($Security->canAdmin() && !$this->nivel->IsDetailKey && EmptyValue($this->nivel->FormValue)) {
    ...
    protected function getEditRow($rsold)
    {
        ...
        if (!IsMaskedPassword($this->clave->CurrentValue)) {
            $this->clave->setDbValueDef($rsnew, $this->clave->CurrentValue, $this->clave->ReadOnly || Config("ENCRYPTED_PASSWORD") && $rsold['clave'] == $this->clave->CurrentValue);
        }
