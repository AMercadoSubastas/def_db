 ...
 * Page class
 */
class DetremiList extends Detremi
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DetremiList";
    ...
    public $FormName = "fdetremilist";
    public $FormActionName = "";
    public $FormBlankRowName = "";
    public $FormKeyCountName = "";

    // CSS class/style
    public $CurrentPageName = "DetremiList";
    ...
    public function setVisibility()
    {
        ...
        $this->desclote->Visible = false;
        $this->descorlote->setVisibility();
    ...
    public function __construct()
    {
        ...
        // Table object (detremi)
        if (!isset($GLOBALS["detremi"]) || $GLOBALS["detremi"]::class == PROJECT_NAMESPACE . "detremi") {
            $GLOBALS["detremi"] = &$this;
        }
        ...
        // Initialize URLs
        $this->AddUrl = "DetremiAdd";
        $this->InlineAddUrl = $pageUrl . "action=add";
        $this->GridAddUrl = $pageUrl . "action=gridadd";
        $this->GridEditUrl = $pageUrl . "action=gridedit";
        $this->MultiEditUrl = $pageUrl . "action=multiedit";
        $this->MultiDeleteUrl = "DetremiDelete";
        $this->MultiUpdateUrl = "DetremiUpdate";

        // Table name (for backward compatibility only)
        if (!defined(PROJECT_NAMESPACE . "TABLE_NAME")) {
            define(PROJECT_NAMESPACE . "TABLE_NAME", 'detremi');
        }
        ...
        if ($url != "") {
            ...
            if ($this->IsModal) { // Show as modal
                ...
                if (!SameString($pageName, GetPageName($this->getListUrl()))) { // Not List page
                    ...
                    $result["view"] = SameString($pageName, "DetremiView"); // If View page, no primary button
                } else { // List page
    ...
    public function getFilterList()
    {
        ...
        if (Config("SEARCH_FILTER_OPTION") == "Server") {
            $savedFilterList = Profile()->getSearchFilters("fdetremisrch");
        }
        ...
        $filterList = Concat($filterList, $this->desclote->AdvancedSearch->toJson(), ","); // Field desclote
        $filterList = Concat($filterList, $this->descorlote->AdvancedSearch->toJson(), ","); // Field descorlote
    ...
    protected function processFilterList()
    {
        if (Post("ajax") == "savefilters") { // Save filter request (Ajax)
            ...
            Profile()->setSearchFilters("fdetremisrch", $filters);
    ...
    protected function restoreFilterList()
    {
        ...
        // Field desclote
        $this->desclote->AdvancedSearch->SearchValue = @$filter["x_desclote"];
        $this->desclote->AdvancedSearch->SearchOperator = @$filter["z_desclote"];
        $this->desclote->AdvancedSearch->SearchCondition = @$filter["v_desclote"];
        $this->desclote->AdvancedSearch->SearchValue2 = @$filter["y_desclote"];
        $this->desclote->AdvancedSearch->SearchOperator2 = @$filter["w_desclote"];
        $this->desclote->AdvancedSearch->save();

        // Field descorlote
        $this->descorlote->AdvancedSearch->SearchValue = @$filter["x_descorlote"];
        $this->descorlote->AdvancedSearch->SearchOperator = @$filter["z_descorlote"];
        $this->descorlote->AdvancedSearch->SearchCondition = @$filter["v_descorlote"];
        $this->descorlote->AdvancedSearch->SearchValue2 = @$filter["y_descorlote"];
        $this->descorlote->AdvancedSearch->SearchOperator2 = @$filter["w_descorlote"];
        $this->descorlote->AdvancedSearch->save();
    ...
    public function basicSearchWhere($default = false)
    {
        ...
        // Fields to search
        $searchFlds = [];
        $searchFlds[] = &$this->desclote;
        $searchFlds[] = &$this->descorlote;
    ...
    protected function setupSortOrder()
    {
        ...
        if (Get("order") !== null) {
            ...
            $this->updateSort($this->descorlote); // descorlote
    ...
    protected function resetCmd()
    {
        ...
        if (StartsString("reset", $this->Command)) {
            ...
            if ($this->Command == "resetsort") {
                ...
                $this->desclote->setSort("");
                $this->descorlote->setSort("");
                $this->estado->setSort("");
                ...
                if ($listAction->Select == ACTION_SINGLE && $allowed) {
                    ...
                    if ($action != "") {
                        ...
                        $link = $disabled
                            ...
                            : "<li><button type=\"button\" class=\"dropdown-item ew-action ew-list-action\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdetremilist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button></li>";
                        $links[] = $link;
                        if ($body == "") { // Setup first button
                            $body = $disabled
                            ? "<div class=\"alert alert-light\">" . $icon . " " . $caption . "</div>"
                            : "<button type=\"button\" class=\"btn btn-default ew-action ew-list-action\" title=\"" . $title . "\" data-caption=\"" . $title . "\" data-ew-action=\"submit\" form=\"fdetremilist\" data-key=\"" . $this->keyToJson(true) . "\"" . $listAction->toDataAttributes() . ">" . $icon . " " . $caption . "</button>";
                        }
                    }
                }
            }
    ...
    protected function setupOtherOptions()
    {
        ...
        if ($this->UseColumnVisibility) {
            ...
            $this->createColumnOption($option, "descorlote");
        ...
        $item->Body = "<a class=\"ew-save-filter\" data-form=\"fdetremisrch\" data-ew-action=\"none\">" . $Language->phrase("SaveCurrentFilter") . "</a>";
        $item->Visible = true;
        $item = &$this->FilterOptions->add("deletefilter");
        $item->Body = "<a class=\"ew-delete-filter\" data-form=\"fdetremisrch\" data-ew-action=\"none\">" . $Language->phrase("DeleteFilter") . "</a>";
        $item->Visible = true;
    ...
    public function renderOtherOptions()
    {
        ...
        foreach ($this->ListActions as $listAction) {
            if ($listAction->Select == ACTION_MULTIPLE) {
                ...
                $item->Body = '<button type="button" class="btn btn-default ew-action ew-list-action" title="' . HtmlEncode($caption) . '" data-caption="' . HtmlEncode($caption) . '" data-ew-action="submit" form="fdetremilist"' . $listAction->toDataAttributes() . '>' . $icon . '</button>';
    ...
    public function setupRow()
    {
        ...
        if ($this->isGridAdd() || $this->isGridEdit()) {
            if ($this->RowIndex === '$rowindex$') { // Render template row first
                ...
                // Set row properties
                $this->resetAttributes();
                $this->RowAttrs->merge(["data-rowindex" => $this->RowIndex, "id" => "r0_detremi", "data-rowtype" => RowType::ADD]);
    ...
    protected function newRow()
    {
        ...
        $row['desclote'] = $this->desclote->DefaultValue;
        $row['descorlote'] = $this->descorlote->DefaultValue;
    ...
    protected function setupSearchOptions()
    {
        ...
        $item->Body = "<a class=\"btn btn-default ew-search-toggle" . $searchToggleClass . "\" role=\"button\" title=\"" . $Language->phrase("SearchPanel") . "\" data-caption=\"" . $Language->phrase("SearchPanel") . "\" data-ew-action=\"search-toggle\" data-form=\"fdetremisrch\" aria-pressed=\"" . ($searchToggleClass == " active" ? "true" : "false") . "\">" . $Language->phrase("SearchLink") . "</a>";
        $item->Visible = true;
