...
$query_cabfac = sprintf("SELECT * FROM cabfac WHERE (tcomp BETWEEN 51 AND 64 OR tcomp IN (86,89,92,93,94,103,104,105,111,112)) AND fecreg BETWEEN %s AND %s ORDER BY fecreg, nrodoc, tcomp", $fecha_desde, $fecha_hasta);
...
{	
	...
	if ($i <= 22) {
		...
		$provincias = mysqli_query($amercado, $query_provincias) or die ("ERROR LEYENDO PROVINCIAS 212 REM: ".$codrem);
        ...
        switch ($tcomp) {
            ...
            case 104:
            case 112:
                if ($tcomp == 103) { //FACTURA DE EXPORTACION
                    if ($row_cabecerafac["totcomis"] != 0.00 || $row_cabecerafac["totimp"]  != 0.00) {
                        $tot_servicios = $row_cabecerafac["totimp"];
                        $tot_comision = $row_cabecerafac["totcomis"];
                        $cliente      = $row_cabecerafac["cliente"];
                        $lotes21      = $row_cabecerafac["totneto21"];
		        ...
		        $query_detfac2 = "SELECT * FROM detfac WHERE tcomp = $tcomp AND ncomp = $ncomp AND (descrip like '%PUBLIC%' OR descrip like '%RECUPE%' OR descrip like '%FINANC%')";
				...
				$query_detfac3 = "SELECT * FROM detfac WHERE tcomp = $tcomp AND ncomp = $ncomp AND ( descrip not like '%SERVICIO%' and descrip not like '%PUBLIC%' and descrip not like '%RECUPE%' and descrip not like '%FINANC%')";
            ...
            case 105:
				...
				if (mysqli_num_rows($detfac1) > 0) {
					...
					if (strpos($descri_det, "DEB")== FALSE && strpos($descri_det, "D�B")== FALSE && strpos($descri_det, "Nd")== FALSE) {
                        ...
                        $cabefac = mysqli_query($amercado, $query_cabf) or die("ACA STA LA CAGADA 1 ".$nrodoc_rel." - ".$tcomp." - ".$ncomp);
                    }
					else {
				...
				if (mysqli_num_rows($detfac1) > 0) {
					...
					if ($row_cabefac = mysqli_fetch_assoc($cabefac)) {
						if ($row_cabefac["tcomp"] == 54) {
							...
							$detfacfc2 = mysqli_query($amercado, $query_detfacfc2) or die("ERROR LEYENDO DETFAC 813");
							if (mysqli_num_rows($detfacfc2) > 0) {
								$row_detfacfc2 = mysqli_fetch_assoc($detfacfc2);
								$cliente      = $row_cabecerafac["cliente"];
							...
							$detfacfc3 = mysqli_query($amercado, $query_detfacfc3) or die("ERROR LEYENDO DETFAC 833");
							if (mysqli_num_rows($detfacfc3) > 0) {
								$row_detfacfc3 = mysqli_fetch_assoc($detfacfc3);
								$cliente      = $row_cabecerafac["cliente"];
								$tot_comision = $row_detfacfc3["neto"];
...
$pdf->Cell(26,16,'    Servicios ',1,0,'L');
$pdf->Cell(26,16,'    Comisiones ',1,0,'L');
$pdf->Cell(26,16,' Public. y Comis. ',1,0,'L');
...
for ($j = 1; $j < 2; $j++) {
	...
	$subtot_ac_prov = $tot_ac_comis + $tot_ac_public;
...
$pdf->SetY($valor_y);
$pdf->SetX(5);
$pdf->Cell(50,6,"6 % (comisi�n + publicidad) ===>  ",0,0,'L');
...
$pdf->SetY($valor_y);
$pdf->SetX(5);
$pdf->Cell(50,6,"6 % (comisi�n + publicidad + tasa) ===>  ",0,0,'L');
