 ...
 * Page class
 */
class CabfacAdd extends Cabfac
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "CabfacAdd";
        ...
        switch ($this->CurrentAction) {
            ...
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "CabfacView") {
    ...
    protected function loadDefaultValues()
    {
        ...
        $this->totneto->DefaultValue = $this->totneto->getDefault(); // PHP
        $this->totneto->OldValue = $this->totneto->DefaultValue;
        $this->totbruto->DefaultValue = $this->totbruto->getDefault(); // PHP
        $this->totbruto->OldValue = $this->totbruto->DefaultValue;
        ...
        $this->totimp->DefaultValue = $this->totimp->getDefault(); // PHP
        $this->totimp->OldValue = $this->totimp->DefaultValue;
        $this->totcomis->DefaultValue = $this->totcomis->getDefault(); // PHP
        $this->totcomis->OldValue = $this->totcomis->DefaultValue;
        $this->totneto105->DefaultValue = $this->totneto105->getDefault(); // PHP
        $this->totneto105->OldValue = $this->totneto105->DefaultValue;
        $this->totneto21->DefaultValue = $this->totneto21->getDefault(); // PHP
        $this->totneto21->OldValue = $this->totneto21->DefaultValue;
        $this->tipoiva->DefaultValue = $this->tipoiva->getDefault(); // PHP
        $this->tipoiva->OldValue = $this->tipoiva->DefaultValue;
        $this->porciva->DefaultValue = $this->porciva->getDefault(); // PHP
        $this->porciva->OldValue = $this->porciva->DefaultValue;
        $this->usuario->DefaultValue = $this->usuario->getDefault(); // PHP
        $this->usuario->OldValue = $this->usuario->DefaultValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            if (!$this->direc->Raw) {
                $this->direc->CurrentValue = HtmlDecode($this->direc->CurrentValue);
            }
            $this->direc->EditValue = HtmlEncode($this->direc->CurrentValue);
            $this->direc->PlaceHolder = RemoveHtml($this->direc->caption());
            ...
            if (!$this->telef->Raw) {
                $this->telef->CurrentValue = HtmlDecode($this->telef->CurrentValue);
            }
            $this->telef->EditValue = HtmlEncode($this->telef->CurrentValue);
            $this->telef->PlaceHolder = RemoveHtml($this->telef->caption());
            ...
            $this->porciva->CurrentValue = FormatNumber($this->porciva->getDefault(), $this->porciva->formatPattern());
            ...
            if (!$this->leyendafc->Raw) {
                $this->leyendafc->CurrentValue = HtmlDecode($this->leyendafc->CurrentValue);
            }
            $this->leyendafc->EditValue = HtmlEncode($this->leyendafc->CurrentValue);
            $this->leyendafc->PlaceHolder = RemoveHtml($this->leyendafc->caption());
    ...
    protected function validateForm()
    {
        ...
        if (in_array("detfac", $detailTblVar) && $detailPage->DetailAdd) {
            $detailPage->run();
    ...
    protected function addRow($rsold = null)
    {
        ...
        if ($addRow) {
            $detailTblVar = explode(",", $this->getCurrentDetailTable());
            $detailPage = Container("DetfacGrid");
            if (in_array("detfac", $detailTblVar) && $detailPage->DetailAdd && $addRow) {
                $detailPage->tcomp->setSessionValue($this->tcomp->CurrentValue); // Set master key
                $detailPage->serie->setSessionValue($this->serie->CurrentValue); // Set master key
                $detailPage->ncomp->setSessionValue($this->ncomp->CurrentValue); // Set master key
                $Security->loadCurrentUserLevel($this->ProjectID . "detfac"); // Load user level of detail table
                $addRow = $detailPage->gridInsert();
                $Security->loadCurrentUserLevel($this->ProjectID . $this->TableName); // Restore user level of master table
                if (!$addRow) {
                $detailPage->tcomp->setSessionValue(""); // Clear master key if insert failed
                $detailPage->serie->setSessionValue(""); // Clear master key if insert failed
                $detailPage->ncomp->setSessionValue(""); // Clear master key if insert failed
                }
            }
        }
    ...
    protected function getAddRow()
    {
        ...
        // fecval
        $this->fecval->setDbValueDef($rsnew, UnFormatDateTime($this->fecval->CurrentValue, $this->fecval->formatPattern()), false);

        // fecdoc
        $this->fecdoc->setDbValueDef($rsnew, UnFormatDateTime($this->fecdoc->CurrentValue, $this->fecdoc->formatPattern()), false);

        // fecreg
        $this->fecreg->setDbValueDef($rsnew, UnFormatDateTime($this->fecreg->CurrentValue, $this->fecreg->formatPattern()), false);

        // cliente
        $this->cliente->setDbValueDef($rsnew, $this->cliente->CurrentValue, false);

        // direc
        $this->direc->setDbValueDef($rsnew, $this->direc->CurrentValue, false);
        ...
        // telef
        $this->telef->setDbValueDef($rsnew, $this->telef->CurrentValue, false);
        ...
        $this->emitido->setDbValueDef($rsnew, $tmpBool, false);

        // totneto
        $this->totneto->setDbValueDef($rsnew, $this->totneto->CurrentValue, strval($this->totneto->CurrentValue) == "");

        // totbruto
        $this->totbruto->setDbValueDef($rsnew, $this->totbruto->CurrentValue, strval($this->totbruto->CurrentValue) == "");
        ...
        // totimp
        $this->totimp->setDbValueDef($rsnew, $this->totimp->CurrentValue, strval($this->totimp->CurrentValue) == "");

        // totcomis
        $this->totcomis->setDbValueDef($rsnew, $this->totcomis->CurrentValue, strval($this->totcomis->CurrentValue) == "");

        // totneto105
        $this->totneto105->setDbValueDef($rsnew, $this->totneto105->CurrentValue, strval($this->totneto105->CurrentValue) == "");

        // totneto21
        $this->totneto21->setDbValueDef($rsnew, $this->totneto21->CurrentValue, strval($this->totneto21->CurrentValue) == "");

        // porciva
        $this->porciva->setDbValueDef($rsnew, $this->porciva->CurrentValue, strval($this->porciva->CurrentValue) == "");

        // nrengs
        $this->nrengs->setDbValueDef($rsnew, $this->nrengs->CurrentValue, false);
        ...
        // leyendafc
        $this->leyendafc->setDbValueDef($rsnew, $this->leyendafc->CurrentValue, false);
        ...
        // en_liquid
        $this->en_liquid->setDbValueDef($rsnew, $this->en_liquid->CurrentValue, false);

        // CAE
        $this->CAE->setDbValueDef($rsnew, $this->CAE->CurrentValue, false);

        // CAEFchVto
        $this->CAEFchVto->setDbValueDef($rsnew, UnFormatDateTime($this->CAEFchVto->CurrentValue, $this->CAEFchVto->formatPattern()), false);
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['telef'])) { // telef
            $this->telef->setFormValue($row['telef']);
        }
        ...
        if (isset($row['emitido'])) { // emitido
            $this->emitido->setFormValue($row['emitido']);
        }
        if (isset($row['totneto'])) { // totneto
            $this->totneto->setFormValue($row['totneto']);
        }
        if (isset($row['totbruto'])) { // totbruto
            $this->totbruto->setFormValue($row['totbruto']);
        }
        ...
        if (isset($row['totimp'])) { // totimp
            $this->totimp->setFormValue($row['totimp']);
        }
        if (isset($row['totcomis'])) { // totcomis
            $this->totcomis->setFormValue($row['totcomis']);
        }
        if (isset($row['totneto105'])) { // totneto105
            $this->totneto105->setFormValue($row['totneto105']);
        }
        if (isset($row['totneto21'])) { // totneto21
            $this->totneto21->setFormValue($row['totneto21']);
        }
        if (isset($row['porciva'])) { // porciva
            $this->porciva->setFormValue($row['porciva']);
        }
        if (isset($row['nrengs'])) { // nrengs
            $this->nrengs->setFormValue($row['nrengs']);
        }
        ...
        if (isset($row['leyendafc'])) { // leyendafc
            $this->leyendafc->setFormValue($row['leyendafc']);
        }
        ...
        if (isset($row['en_liquid'])) { // en_liquid
            $this->en_liquid->setFormValue($row['en_liquid']);
        }
        if (isset($row['CAE'])) { // CAE
            $this->CAE->setFormValue($row['CAE']);
        }
        if (isset($row['CAEFchVto'])) { // CAEFchVto
            $this->CAEFchVto->setFormValue($row['CAEFchVto']);
        }
        if (isset($row['Resultado'])) { // Resultado
            $this->Resultado->setFormValue($row['Resultado']);
        }
    }
