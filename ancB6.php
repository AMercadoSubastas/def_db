...
$query_comprobante = sprintf("SELECT * FROM series  WHERE series.codnum = %s", "61");
	...
	if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text")!="NULL") {
		...
		VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
			...
			if (!empty($_POST['imprime'])) { 
				...
				$insertGoTo = "facturaCBI_ok.php?factura=$facnum";
				if (isset($_SERVER['QUERY_STRING'])) {
					$insertGoTo .= (strpos($insertGoTo, '?')) ? "&" : "?";
					$insertGoTo .= $_SERVER['QUERY_STRING'];
				}
				header(sprintf("Location: %s", "facturaCBI_ok.php?factura=$facnum")); 
			}
		}
	}
}
//setcookie("concepto","");
mysqli_select_db($amercado, $database_amercado);
$query_facturas_a = "SELECT * FROM series  WHERE series.codnum=32"; // antes decia 1 que es por lotes
...
$tcomp = 62;
mysqli_select_db($amercado, $database_amercado);
$query_facturas_b = "SELECT * FROM series  WHERE series.codnum=32"; // antes decia 23 que es por lotes
...
mysqli_select_db($amercado, $database_amercado);
$query_tipo_comprobante = "SELECT * FROM tipcomp WHERE codnum='62'";
...
var selec = form.tipos.options;
    ...
    if (selec[2].selected == true)
    {
		...
		factura.tcbterel_texto.value = "N DEB B0006";
...
function agregarOpciones(form)
{
	var selec = form.tipos.options;
		...
		factura.serie_texto.value = "SERIE DE N CRED B0006";
		factura.tcomp.value = 62;
...
<form id="factura" name="factura" method="POST" action="<?php echo $editFormAction; ?>">
  <table width="900" height="750" border="2" align="left" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
    <tr>
      ...
      <td colspan="2" valign="top" bgcolor="#FFFFFF"><table width="100%" border="1" cellspacing="1" cellpadding="1">
		 ...
		 <td width="14%" height="20" class="ewTableHeader">&nbsp;Tipo de Cbte </td>
          <td width="36%"><select name="tipos" onChange="agregarOpciones(this.form)">
                                      <option value="61">NOTA DE CRED INMOB B0006</option>
          ...
          <td width="35%"><input name="serie_texto" type="text" value="SERIE DE N CRED INMOB B0006" size="34" />
...
?>
        ...
        <td colspan="3" valign="top" bgcolor="#FFFFFF"><table width="100%" border="1" cellspacing="1" cellpadding="1">
          ...
          <td width="20%"><select name="tcbterel2" onChange="agregarOpciones2(this.form)">
                              ...
                              <option value="7">N DEB B0006</option>
    ...
    <tr>
	  ...
	  <table width="100%" border="1" cellpadding="1" cellspacing="1" bgcolor="#0FFFFFF">
        <tr>
		...
		<tr>
          ...
          <td width="65" bgcolor="#FFFFFF"><input name="importe" type="text" id="importe"  onchange="validarFormulario(this.form)" size="20"   /></td>
          </tr>
		 <tr>
