...
/**
 * Table class for tipoiva
 */
class Tipoiva extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->descor->Nullable = false; // NOT NULL field
        $this->descor->Required = true; // Required field
        $this->descor->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        // discrimina
        $this->discrimina = new DbField(
            $this, // Table
            'x_discrimina', // Variable name
            'discrimina', // Name
            '`discrimina`', // Expression
            '`discrimina`', // Basic search expression
            16, // Type
            1, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`discrimina`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->discrimina->addMethod("getDefault", fn() => 1);
        $this->discrimina->InputTextType = "text";
        $this->discrimina->Raw = true;
        $this->discrimina->Nullable = false; // NOT NULL field
        $this->discrimina->setDataType(DataType::BOOLEAN);
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
                $this->discrimina->Lookup = new Lookup($this->discrimina, 'tipoiva', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->discrimina->Lookup = new Lookup($this->discrimina, 'tipoiva', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
        }
        $this->discrimina->OptionCount = 2;
        $this->discrimina->DefaultErrorMessage = $Language->phrase("IncorrectField");
        $this->discrimina->SearchOperators = ["=", "<>"];
        $this->Fields['discrimina'] = &$this->discrimina;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->activo->Lookup = new Lookup($this->activo, 'tipoiva', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->activo->Lookup = new Lookup($this->activo, 'tipoiva', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "tipoiva";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->discrimina->DbValue = $row['discrimina'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TipoivaList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "TipoivaView" => $Language->phrase("View"),
            "TipoivaEdit" => $Language->phrase("Edit"),
            "TipoivaAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TipoivaView",
            Config("API_ADD_ACTION") => "TipoivaAdd",
            Config("API_EDIT_ACTION") => "TipoivaEdit",
            Config("API_DELETE_ACTION") => "TipoivaDelete",
            Config("API_LIST_ACTION") => "TipoivaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TipoivaView", $parm);
        } else {
            $url = $this->keyUrl("TipoivaView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TipoivaEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TipoivaList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TipoivaAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TipoivaList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("TipoivaDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        // discrimina
        $this->discrimina->EditValue = $this->discrimina->options(false);
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->discrimina);
            ...
            if ($recCnt >= $startRec) {
                ...
                if (!$doc->ExportCustom) {
                    ...
                    if ($exportPageType == "view") {
                        ...
                        $doc->exportField($this->discrimina);
