...
<span id="el_bancos_activo">
    <select
        ...
        data-select2-id="fbancosedit_x_activo"
        <?php } ?>
        data-table="bancos"
        data-field="x_activo"
...
<script>
loadjs.ready("fbancosedit", function() {
    var options = { name: "x_activo", selectId: "fbancosedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fbancosedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fbancosedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fbancosedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
