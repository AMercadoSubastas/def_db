...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
	...
	// Aca me tengo que fijar si falta algun dato antes de mandar a a Bejerman y luego a Afip
	...
	$localidad = $row_cliente2['tipoiva'];
	...
	$loc_cliente = $row_localidad['descripcion'];
...
if ($sigo_y_grabo == 1) {
	if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text") != "NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
			$insertSQL = sprintf(
				"INSERT INTO detfac (tcomp, serie, ncomp, nreng, codrem , descrip, neto, comcob, concafac, usuario) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
				FC_A2,
				SERIE_A2,
	...
	if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text") != "NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
			...
			$fechaDateTime = new DateTime($fecha_factura1);
			$fechaDateTime->modify('+10 days');
			$fechaModificada = $fechaDateTime->format('Y-m-d');
			...
			for ($i = 0; $i <= $renglones; $i++) {
				...
				// armo conceptos
				array_push($concepto, $_POST[$iConcepto]);
			}
...
<script language="javascript">
	function liquidacion(form) {
		if (factura.GrupoOpciones2[0].checked == false) {} else {}
	}
</script>
<script language="javascript">
	// Remate
...
<body>
	<form id="factura" name="factura" class="container" method="POST" action="VAfcautxconc">
		<input type="hidden" name="<?= $TokenNameKey ?>" value="<?= $TokenName ?>"><!-- CSRF token name -->
		<input type="hidden" name="<?= $TokenValueKey ?>" value="<?= $TokenValue ?>"><!-- CSRF token value -->
		<table width="900" height="800" border="2" align="left" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
			<tr>
				<td colspan="3">
					<div align="center"><img src="images/carga_aut_conceptos.gif" width="450" height="30" /></div>
				</td>
		...
		<tr>
			<td width="100%" rowspan="3" valign="top">
				<table width="100%" height="20" border="1" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
					<tr>
						<td width="15%" height="25" bgcolor="#FFFFFF" class="ewTableHeader" border="0">&nbsp;Imprimir </td>
						<td width="85%" bgcolor="#FFFFFF"><input type="checkbox" name="imprime" value="1" />
						</td>
					</tr>
				</table>
			</td>
		...
		<tr>
			<td colspan="4" bgcolor="#FFFFFF">
				<table width="900" border="0" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
					<tr>
