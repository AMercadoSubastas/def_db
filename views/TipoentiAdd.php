...
<span id="el_tipoenti_activo">
    <select
        ...
        data-select2-id="ftipoentiadd_x_activo"
        <?php } ?>
        data-table="tipoenti"
        data-field="x_activo"
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("ftipoentiadd", function() {
    var options = { name: "x_activo", selectId: "ftipoentiadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftipoentiadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "ftipoentiadd" };
    } else {
        options.ajax = { id: "x_activo", form: "ftipoentiadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
