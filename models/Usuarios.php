...
class Usuarios extends DbTable
{
    ...
    public function __construct()
    {
        ...
        $this->nivel->Required = true; // Required field
        $this->nivel->setSelectMultiple(false); // Select one
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->nivel->Lookup = new Lookup($this->nivel, 'userlevels', false, 'userlevelid', ["userlevelname","","",""], '', '', [], [], [], [], [], [], false, '', '', "`userlevelname`");
                break;
            default:
                $this->nivel->Lookup = new Lookup($this->nivel, 'userlevels', false, 'userlevelid', ["userlevelname","","",""], '', '', [], [], [], [], [], [], false, '', '', "`userlevelname`");
                break;
        ...
        $this->nivel->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->activo->Lookup = new Lookup($this->activo, 'usuarios', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->activo->Lookup = new Lookup($this->activo, 'usuarios', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
        ...
        // perfil
        $this->perfil = new DbField(
            $this, // Table
            'x_perfil', // Variable name
            'perfil', // Name
            '`perfil`', // Expression
            '`perfil`', // Basic search expression
            200, // Type
            65535, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`perfil`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->perfil->InputTextType = "text";
        $this->perfil->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY", "IS NULL", "IS NOT NULL"];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("UsuariosList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "UsuariosView" => $Language->phrase("View"),
            "UsuariosEdit" => $Language->phrase("Edit"),
            "UsuariosAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function renderEditRow()
    {
        ...
        $this->clave->EditValue = $Language->phrase("PasswordMask"); // Show as masked password
        ...
        // perfil
        $this->perfil->setupEditAttributes();
        if (!$this->perfil->Raw) {
            $this->perfil->CurrentValue = HtmlDecode($this->perfil->CurrentValue);
        }
    ...
    public function sendRegisterEmail($row)
    {
        ...
        // Get user language
        $langId = UserProfile::create()
            ->setUserName($userName)
            ->load(HtmlDecode($row[Config("USER_PROFILE_FIELD_NAME")] ?? ""))
            ->getLanguageId();
        $email = $this->prepareRegisterEmail($user, $langId);
        $args = ["rs" => $row];
        $emailSent = false;
    ...
    public function prepareRegisterEmail($user, $langId = "")
    {
        ...
        $fields = [
            'usuario' => (object)[ "caption" => $this->usuario->caption(), "value" => $user->get('usuario') ],
            'nombre' => (object)[ "caption" => $this->nombre->caption(), "value" => $user->get('nombre') ],
            'activo' => (object)[ "caption" => $this->activo->caption(), "value" => $user->get('activo') ],
            'email' => (object)[ "caption" => $this->_email->caption(), "value" => $user->get('email') ],
            'perfil' => (object)[ "caption" => $this->perfil->caption(), "value" => $user->get('perfil') ],
        ];
        $email = new Email();
        $data = [
