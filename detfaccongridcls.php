<?php include_once "detfacconinfo.php" ?>
...
$detfaccon_grid = NULL; // Initialize page object first

class cdetfaccon_grid extends cdetfaccon {
	...
	// Table name
	var $TableName = 'detfaccon';

	// Page object name
	var $PageObjName = 'detfaccon_grid';

	// Grid form hidden field names
	var $FormName = 'fdetfaccongrid';
	...
	function __construct() {
		...
		// Table object (detfaccon)
		if (!isset($GLOBALS["detfaccon"]) || get_class($GLOBALS["detfaccon"]) == "cdetfaccon") {
			$GLOBALS["detfaccon"] = &$this;

//			$GLOBALS["MasterTable"] = &$GLOBALS["Table"];
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detfaccon"];
		...
		if (!defined("EW_TABLE_NAME"))
			define("EW_TABLE_NAME", 'detfaccon');
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $detfaccon;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($detfaccon);
				$doc->Text = $sContent;
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function Page_Main() {
		...
		// Load master record
		if ($this->CurrentMode <> "add" && $this->GetMasterFilter() <> "" && $this->getCurrentMasterTable() == "cabfaccon") {
			global $cabfaccon;
			$rsmaster = $cabfaccon->LoadRs($this->DbMasterFilter);
			$this->MasterRecordExists = ($rsmaster && !$rsmaster->EOF);
			if (!$this->MasterRecordExists) {
				$this->setFailureMessage($Language->Phrase("NoRecord")); // Set no record found
				$this->Page_Terminate("cabfacconlist.php"); // Return to master page
			} else {
				$cabfaccon->LoadListRowValues($rsmaster);
				$cabfaccon->RowType = EW_ROWTYPE_MASTER; // Master row
				$cabfaccon->RenderListRow();
				$rsmaster->Close();
			}
		}
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			// iva
			if (strval($this->iva->CurrentValue) <> "") {
				switch ($this->iva->CurrentValue) {
					case $this->iva->FldTagValue(1):
						$this->iva->ViewValue = $this->iva->FldTagCaption(1) <> "" ? $this->iva->FldTagCaption(1) : $this->iva->CurrentValue;
						break;
					case $this->iva->FldTagValue(2):
						$this->iva->ViewValue = $this->iva->FldTagCaption(2) <> "" ? $this->iva->FldTagCaption(2) : $this->iva->CurrentValue;
						break;
					default:
						$this->iva->ViewValue = $this->iva->CurrentValue;
				}
			} else {
				$this->iva->ViewValue = NULL;
			}
			...
			// porciva
			if (strval($this->porciva->CurrentValue) <> "") {
				switch ($this->porciva->CurrentValue) {
					case $this->porciva->FldTagValue(1):
						$this->porciva->ViewValue = $this->porciva->FldTagCaption(1) <> "" ? $this->porciva->FldTagCaption(1) : $this->porciva->CurrentValue;
						break;
					case $this->porciva->FldTagValue(2):
						$this->porciva->ViewValue = $this->porciva->FldTagCaption(2) <> "" ? $this->porciva->FldTagCaption(2) : $this->porciva->CurrentValue;
						break;
					default:
						$this->porciva->ViewValue = $this->porciva->CurrentValue;
				}
			} else {
				$this->porciva->ViewValue = NULL;
			}
			...
			$arwrk = array();
			$arwrk[] = array($this->iva->FldTagValue(1), $this->iva->FldTagCaption(1) <> "" ? $this->iva->FldTagCaption(1) : $this->iva->FldTagValue(1));
			$arwrk[] = array($this->iva->FldTagValue(2), $this->iva->FldTagCaption(2) <> "" ? $this->iva->FldTagCaption(2) : $this->iva->FldTagValue(2));
			array_unshift($arwrk, array("", $Language->Phrase("PleaseSelect")));
			$this->iva->EditValue = $arwrk;
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			// Check referential integrity for master table 'cabfaccon'
			$bValidMasterRecord = TRUE;
			$sMasterFilter = $this->SqlMasterFilter_cabfaccon();
			...
			if ($bValidMasterRecord) {
				$rsmaster = $GLOBALS["cabfaccon"]->LoadRs($sMasterFilter);
				$bValidMasterRecord = ($rsmaster && !$rsmaster->EOF);
				$rsmaster->Close();
			}
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "cabfaccon", $Language->Phrase("RelatedRecordRequired"));
	...
	function AddRow($rsold = NULL) {
		...
		// Set up foreign key field value from Session
			if ($this->getCurrentMasterTable() == "cabfaccon") {
		...
		if ($this->ncomp->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@ncomp@", ew_AdjustSql($this->ncomp->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($this->tcomp->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@tcomp@", ew_AdjustSql($this->tcomp->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($this->serie->getSessionValue() <> "") {
			$sMasterFilter = str_replace("@serie@", ew_AdjustSql($this->serie->getSessionValue()), $sMasterFilter);
		} else {
			$bValidMasterRecord = FALSE;
		}
		if ($bValidMasterRecord) {
		...
		// tcomp
		if ($this->tcomp->getSessionValue() <> "") {
			$rsnew['tcomp'] = $this->tcomp->getSessionValue();
		}

		// serie
		if ($this->serie->getSessionValue() <> "") {
			$rsnew['serie'] = $this->serie->getSessionValue();
		}

		// ncomp
		if ($this->ncomp->getSessionValue() <> "") {
			$rsnew['ncomp'] = $this->ncomp->getSessionValue();
		}
	...
	function SetUpMasterParms() {
		...
		if ($sMasterTblVar == "cabfaccon") {
			$this->ncomp->Visible = FALSE;
			if ($GLOBALS["cabfaccon"]->EventCancelled) $this->EventCancelled = TRUE;
