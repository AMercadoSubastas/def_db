...
//$sigo_y_grabo = 1; //SACAR ESTO PARA PRODUCCION
if ($sigo_y_grabo == 1 && $todo_ok ==1) {
	...
	if (isset($_POST['lote']) && GetSQLValueString($_POST['lote'], "int")!="NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
			...
			$descrip1 = GetSQLValueString($_POST['descripcion'], "text");
			$pos = strpos($descrip1, "A000");
			$nrodoc_fc = substr($descrip1, $pos, 14);
			echo "NRODOC = ".$nrodoc_fc."  ";
			// Regrabo la factura con estado "S"
			$updateSQLcabfac = sprintf("UPDATE cabfac SET estado = %s WHERE nrodoc = %s","S", $nrodoc_fc);
...
$iva_15_desc = $row[2];	// Obtener el valor de la tercera columna (ÃƒÂ­ndice 2)
$iva_15_porcen = $row[1];
?>
  ...
  <table width="640" border="1" align="left" cellpadding="1" cellspacing="1" bgcolor="#82BADD">
    ...
    <tr>
      ...
      <td width="14%" height="20" bgcolor="#82BADD">&nbsp;<span class="ewTableHeader">Tipo de Cbte </span></td>
		  ...
		  <input name="tcomp" id="tcomp" value="119" type="hidden" />
		         </td>
          <td width="1%">&nbsp;</td>
