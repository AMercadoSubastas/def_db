...
$query_comprobante = sprintf("SELECT * FROM series  WHERE series.codnum = %s", "43");
...
//setcookie("concepto","");
mysqli_select_db($amercado, $database_amercado);
$query_facturas_a = "SELECT * FROM series  WHERE series.codnum=43"; // antes decia 1 que es por lotes
...
$tcomp = 99;
mysqli_select_db($amercado, $database_amercado);
$query_facturas_b = "SELECT * FROM series  WHERE series.tipcomp=43"; // antes decia 23 que es por lotes
...
mysqli_select_db($amercado, $database_amercado);
$query_tipo_comprobante = "SELECT * FROM tipcomp WHERE codnum='99'";
...
mysqli_select_db($amercado, $database_amercado);
$query_conceptos_a_facturar = "SELECT * FROM concafactven WHERE impuesto = 5 ORDER BY nroconc  ASC";
...
function agregarOpciones(form)
{
	var selec = form.tipos.options;
		...
		factura.serie.value = 43;
		factura.serie_texto.value = "SERIE CBTE INT X0002";
		factura.tcomp.value = 99;
  	...
  	for (i=0; i<(args.length-2); i+=3) { 
		...
		val=MM_findObj(args[i]);
			...
			if ((val=val.value)!="") {
					...
					num = parseFloat(val);
        				if (isNaN(val)) 
						errors+='El importe debe contener un número.\n';
...
<body>
<form id="factura" name="factura" method="POST" class="container" action="VWcbteintdev">
      ...
      <td colspan="2" valign="top" bgcolor="#82BADD"><table width="900" border="1" cellspacing="1" cellpadding="1">
		 ...
		 <td width="14%" height="20" class="ewTableHeader">Tipo de Comprobante </td>
          <td width="36%"><select name="tipos" onChange="agregarOpciones(this.form)">
                        <option value="99">CBTE INTERNO X0002</option>
                                     
                           </select>
		  <input name="tcomp" type="hidden" value="99" size="25" >
		         </td>
          
          <td width="15%" class="ewTableHeader">Serie</td>
          <td width="35%"><input name="serie_texto" value="SERIE CBTE INT X0002" type="text"  size="28" />
		  <input name="serie" type="hidden" value="43" size="25"/>
            </td>
        </tr>
        <tr>
