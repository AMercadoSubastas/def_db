...
$Page->showMessage();
?>
<form name="fentidadesadd" id="fentidadesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<span id="el_entidades_codpais">
    <select
        ...
        data-select2-id="fentidadesadd_x_codpais"
        <?php } ?>
...
<?php if (!$Page->codpais->IsNativeSelect) { ?>
<script>
loadjs.ready("fentidadesadd", function() {
    var options = { name: "x_codpais", selectId: "fentidadesadd_x_codpais" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fentidadesadd.lists.codpais?.lookupOptions.length) {
        options.data = { id: "x_codpais", form: "fentidadesadd" };
    } else {
        options.ajax = { id: "x_codpais", form: "fentidadesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_entidades_codprov">
    <select
        ...
        data-select2-id="fentidadesadd_x_codprov"
        <?php } ?>
        data-table="entidades"
...
<script>
loadjs.ready("fentidadesadd", function() {
    var options = { name: "x_codprov", selectId: "fentidadesadd_x_codprov" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fentidadesadd.lists.codprov?.lookupOptions.length) {
        options.data = { id: "x_codprov", form: "fentidadesadd" };
    } else {
        options.ajax = { id: "x_codprov", form: "fentidadesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_entidades_codloc">
    <select
        ...
        data-select2-id="fentidadesadd_x_codloc"
        <?php } ?>
        data-table="entidades"
        data-field="x_codloc"
...
<script>
loadjs.ready("fentidadesadd", function() {
    var options = { name: "x_codloc", selectId: "fentidadesadd_x_codloc" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fentidadesadd.lists.codloc?.lookupOptions.length) {
        options.data = { id: "x_codloc", form: "fentidadesadd" };
    } else {
        options.ajax = { id: "x_codloc", form: "fentidadesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_entidades_tipoent">
    <select
        ...
        data-select2-id="fentidadesadd_x_tipoent"
        <?php } ?>
        data-table="entidades"
...
<script>
loadjs.ready("fentidadesadd", function() {
    var options = { name: "x_tipoent", selectId: "fentidadesadd_x_tipoent" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fentidadesadd.lists.tipoent?.lookupOptions.length) {
        options.data = { id: "x_tipoent", form: "fentidadesadd" };
    } else {
        options.ajax = { id: "x_tipoent", form: "fentidadesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_entidades_tipoiva">
    <select
        ...
        data-select2-id="fentidadesadd_x_tipoiva"
        <?php } ?>
        data-table="entidades"
...
<script>
loadjs.ready("fentidadesadd", function() {
    var options = { name: "x_tipoiva", selectId: "fentidadesadd_x_tipoiva" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fentidadesadd.lists.tipoiva?.lookupOptions.length) {
        options.data = { id: "x_tipoiva", form: "fentidadesadd" };
    } else {
        options.ajax = { id: "x_tipoiva", form: "fentidadesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready("fentidadesadd", function() {
    fentidadesadd.createAutoSuggest(Object.assign({"id":"x_cuit","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->cuit->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.entidades.fields.cuit.autoSuggestOptions));
});
</script>
</span>
</div></div>
...
<span id="el_entidades_calif">
    <select
        ...
        data-select2-id="fentidadesadd_x_calif"
        <?php } ?>
        data-table="entidades"
        data-field="x_calif"
...
<script>
loadjs.ready("fentidadesadd", function() {
    var options = { name: "x_calif", selectId: "fentidadesadd_x_calif" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fentidadesadd.lists.calif?.lookupOptions.length) {
        options.data = { id: "x_calif", form: "fentidadesadd" };
    } else {
        options.ajax = { id: "x_calif", form: "fentidadesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_entidades_tipoind">
    <select
        ...
        data-select2-id="fentidadesadd_x_tipoind"
        <?php } ?>
        data-table="entidades"
...
<script>
loadjs.ready("fentidadesadd", function() {
    var options = { name: "x_tipoind", selectId: "fentidadesadd_x_tipoind" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fentidadesadd.lists.tipoind?.lookupOptions.length) {
        options.data = { id: "x_tipoind", form: "fentidadesadd" };
    } else {
        options.ajax = { id: "x_tipoind", form: "fentidadesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fentidadesadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fentidadesadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
