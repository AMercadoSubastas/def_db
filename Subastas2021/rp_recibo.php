...
// Leo los renglones
$query_det_recibo = "SELECT detrecibo.nrodoc , detrecibo.netocbterel FROM detrecibo WHERE ncomp ='$pncomp' AND tcomp ='$tcomp'";
...
$fact_nrodoc = "";
*/
$query_det_recibo2 = "SELECT * FROM detrecibo WHERE ncomp ='$pncomp' AND tcomp = '$tcomp'";
...
for ($k=0;$k<2;$k++) {
    ...
    // DEPOSITOS
    $query_depositos = "SELECT codban,codsuc,codchq,importe,fechapago FROM cartvalores WHERE ncomprel ='$pncomp' AND tcomprel ='$tcomp' AND serierel='$serie' AND tcomp='9' AND serie='7'";
	...
	$tcompefe5 = 0;
    // CANCELA CONTRA CC
    $query_efectivo_5 = "SELECT importe, codchq,tcomp  FROM cartvalores WHERE ncomprel ='$pncomp' AND tcomprel ='$tcomp' AND serierel='$serie' AND tcomp='141'";
    $selec_efectivo_5 = mysqli_query($amercado,  $query_efectivo_5) or die("ERROR LEYENDO 187");
    $efe_tot_5 = 0 ;
    while($row2_5 = mysqli_fetch_array($selec_efectivo_5, MYSQLI_BOTH)){
        $efe_tot0_5	= $row2_5['0'];
        $efe_tot_5 = $efe_tot_5 + $efe_tot0_5 ;
        $ndeb5 = $row2_5['1'];
        $tcompefe5 = $row2_5['2'];
	...
	if ($tcomp == 2)
		...
		$pdf->Cell(160,10,'Recibo Int. Nro.: ');
    $pdf->SetFont('Arial','BI',10);
    $pdf->SetY(24);
    $pdf->SetX(120);
	...
	*/
	...
	if (isset($codp_cliente))
		$pdf->Cell(100,10,utf8_decode($calle_cliente).' '.utf8_decode($nro_cliente).' ('.$codp_cliente.') '.utf8_decode($nomloc),0,0,'L');
	else
		$pdf->Cell(100,10,utf8_decode($calle_cliente).' '.$nro_cliente.' '.utf8_decode($nomloc),0,0,'L');
	$pdf->SetY(54);
	$pdf->Cell(18);
	$pdf->SetX(120);
    ...
    if ($tcompefe == 13) {
        $pdf->Cell(20,10,utf8_decode('Depósito en dólares:-T. cbio ($347,50)-- U$S 7845,91.'),0,0,'L');
        $pdf->SetX(85);
        $efe_totf  = number_format($efe_tot, 2, ',','.');
        $pdf->Cell(20,10,$efe_totf,0,0,'R');	
        $valorY += 5;
	...
	if ($tcompefe5 == 141) {
        $pdf->Cell(20,10,utf8_decode('Compensación CC :    ').$ndeb5.'   $ ',0,0,'L');
        $pdf->SetX(160);
        $efe_tot_5f  = number_format($efe_tot_5, 2, ',','.');
        $pdf->Cell(20,10, $efe_tot_5f,0,0,'R');
        $valorY += 5;
        $pdf->SetY($valorY);
    }
	...
	if (mysqli_num_rows($selec_cheques)!=0) {	
	    ...
	    //echo $str_cheques;
		if ($fact_tcomp[0] == 115 || $fact_tcomp[0] == 116 || $fact_tcomp[0] == 117 || $fact_tcomp[0] == 142) 
    ...
    if (mysqli_num_rows($selec_efectivo_4)!=0) {           
        if ($tcompefe4 == 97) {
            ...
            $y = $y+4 ;
            $pdf->SetY($y);
            $pdf->Cell(20,10,'Total Comp. Ac.Com :  $  '.$efe_tot_4f,0,0,'L');	
        }
    }
	if (mysqli_num_rows($selec_efectivo_5)!=0) {           
        if ($tcompefe5 == 141) {
            $pdf->Cell(20,10,': ',0,0,'L');	
            $y = $y+4 ;
            $pdf->SetY($y);
            $pdf->Cell(20,10,utf8_decode('Total Compensación CC :  $ ').$efe_tot_5f,0,0,'L');	
        }
    }
        ...
        if ((mysqli_num_rows($selec_ing_brutos)!=0)) {
	  		...
	  		while ($ing_brutos = mysqli_fetch_array($selec_ing_brutos, MYSQLI_BOTH)){	
				...
				$totalRows_tipcomp_ret = mysqli_num_rows($tipcomp_ret);
	  			$descr_ret = utf8_decode($row_tipcomp_ret['descripcion']);
...
for ($j=0;$j < $tope_fact;$j++) {
    ...
    if ($fact_tcomp[$j] == 115 || $fact_tcomp[$j] == 116 || $fact_tcomp[$j] == 117 || $fact_tcomp[$j] == 142 || $fact_tcomp[$j] == 103 ) {
        for ($k=0;$k<2;$k++) {
        ...
        $query_detfac = sprintf("SELECT * FROM detfac WHERE tcomp = %s AND serie = %s AND ncomp = %s AND codlote is not NULL ORDER BY codlote" , $fact_tcomp[$j], $fact_serie[$j], $fact_ncomp[$j]);
        ...
        $pdf->Cell(18);
        if (isset($codp_cliente))
            $pdf->Cell(100,10,utf8_decode($calle_cliente).' '.$nro_cliente.' ('.$codp_cliente.') '.utf8_decode($nomloc),0,0,'L');
        else
            $pdf->Cell(100,10,utf8_decode($calle_cliente).' '.$nro_cliente.' '.utf8_decode($nomloc),0,0,'L');
        $pdf->SetY(54);
        $pdf->Cell(18);
        ...
        if ($remate!="" ) {
            ...
            while($row_detallefac = mysqli_fetch_array($detallefac)) {
                ...
                if ($remate == 535 && ($lote_num == 60 || $lote_num == 61))
                    continue;
                if ($lote_num=="" ) {
