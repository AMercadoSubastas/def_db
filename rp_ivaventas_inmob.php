...
$query_cabfac = sprintf("SELECT * FROM cabfac WHERE fecreg BETWEEN %s AND %s  AND usuario in (25,26) ORDER BY fecreg, nrodoc", $fecha_desde, $fecha_hasta);
...
// Inicio el pdf con los datos de cabecera
  ...
  $pdf->SetY(15);
  $pdf->Cell(130);
  $pdf->Cell(20,10,' Libro IVA Ventas Inmobiliaria',0,0,'L');
  ...
  $pdf->Cell(24,16,' IVA Debito',1,0,'L');
  $pdf->Cell(24,16,'   Servicios',1,0,'L');
...
{	
   //if ($row_cabecerafac["usuario"] != 25 && $row_cabecerafac["usuario"] != 26)
   //     continue;
	...
	if ($i <= 22) {
		...
		if (($tcomp == 57 || $tcomp == 58 || $tcomp == 59 || $tcomp = 60) && ($row_cabecerafac["totiva21"] == 0.00 &&  $row_cabecerafac["totiva105"] == 0.00)) {
			$tot_neto21   = 0.00;
			$tot_neto105  = 0.00;
			$tot_exento   = $row_cabecerafac["totneto21"] * $signo;
		}
		else {
