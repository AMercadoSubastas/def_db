...
if(isset($_POST['remate']) || isset($_GET['remate'])){  
	...
	if ($tipo_de_iva== 1) {
		$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) , SUM( `totneto21` *'$procen_iva' ) , SUM( `totneto21` *'$procen_iva' + `totneto21`)  as total_21 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND `tcomp`!='52'  AND `tcomp`!='57'  AND `tcomp`!='58' AND `tcomp`!='59'  AND `en_liquid` = 1 )" ;
		$query_remate = mysqli_query($amercado, $res1) or die(mysqli_error($amercado));
		$totalRows_remate = mysqli_num_rows($query_remate); 
	}
	else {
		$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) , SUM( `totneto21` *'$procen_iva' ) , SUM( `totneto21` *'$procen_iva' + `totneto21`)  as total_21 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND `tcomp`!='54' AND `tcomp`!='57'  AND `tcomp`!='58' AND `tcomp`!='59'  AND `en_liquid` = 1 )" ;
		$query_remate = mysqli_query($amercado, $res1) or die(mysqli_error($amercado));
		$totalRows_remate = mysqli_num_rows($query_remate); 
	...
	if ($tipo_de_iva== 1) {
		$res2 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105_2 , SUM( `totneto21` ) , SUM( `totneto21` *'$procen_iva' ) , SUM( `totneto21` *'$procen_iva' + `totneto21`)  as total_21_2 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND (`tcomp`='57' OR `tcomp`='58' OR `tcomp`='93') AND `en_liquid` = 1 )" ;
...
if ($tipo_de_iva== 1) {
	$query_factura_total     = "SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 52 || cabfac.tcomp = 54 || cabfac.tcomp = 59) AND cabfac.estado != 'A' AND cabfac.codrem ='$rematenum' AND cabfac.en_liquid = 1";
	...
	$query_factura_total     = "SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 54 || cabfac.tcomp = 54 || cabfac.tcomp = 59)  AND cabfac.estado != 'A' AND cabfac.codrem ='$rematenum'  AND cabfac.en_liquid = 1" ;
	$factura_total           = mysqli_query($amercado, $query_factura_total) or die(mysqli_error($amercado));
	$row_factura_total       = mysqli_fetch_assoc($factura_total);
	$totalRows_factura_total = mysqli_num_rows($factura_total);
	$totfactura1             = number_format ($row_factura_total['SUM( cabfac.totbruto)'] , 2 , "," , ".");
