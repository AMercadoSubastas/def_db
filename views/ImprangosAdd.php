...
<span id="el_imprangos_activo">
    <select
        ...
        data-select2-id="fimprangosadd_x_activo"
        <?php } ?>
        data-table="imprangos"
        data-field="x_activo"
...
<script>
loadjs.ready("fimprangosadd", function() {
    var options = { name: "x_activo", selectId: "fimprangosadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fimprangosadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fimprangosadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fimprangosadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
