...
$Page->showMessage();
?>
<form name="fseriesadd" id="fseriesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<span id="el_series_tipcomp">
    <select
        ...
        data-select2-id="fseriesadd_x_tipcomp"
        <?php } ?>
        data-table="series"
...
<?php if (!$Page->tipcomp->IsNativeSelect) { ?>
<script>
loadjs.ready("fseriesadd", function() {
    var options = { name: "x_tipcomp", selectId: "fseriesadd_x_tipcomp" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fseriesadd.lists.tipcomp?.lookupOptions.length) {
        options.data = { id: "x_tipcomp", form: "fseriesadd" };
    } else {
        options.ajax = { id: "x_tipcomp", form: "fseriesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
loadjs.ready(["fseriesadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fseriesadd", "x_fechatope", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fseriesadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fseriesadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
