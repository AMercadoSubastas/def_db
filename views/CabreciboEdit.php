...
?>
<main class="edit">
<form name="fcabreciboedit" id="fcabreciboedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
...
<?php if ($Page->codnum->Visible) { // codnum ?>
    <div id="r_codnum"<?= $Page->codnum->rowAttributes() ?>>
        <label id="elh_cabrecibo_codnum" class="<?= $Page->LeftColumnClass ?>"><?= $Page->codnum->caption() ?><?= $Page->codnum->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<input type="hidden" data-table="cabrecibo" data-field="x_codnum" data-hidden="1" name="x_codnum" id="x_codnum" value="<?= HtmlEncode($Page->codnum->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<span id="el_cabrecibo_tcomp">
    <select
        ...
        data-select2-id="fcabreciboedit_x_tcomp"
        <?php } ?>
...
<?php if (!$Page->tcomp->IsNativeSelect) { ?>
<script>
loadjs.ready("fcabreciboedit", function() {
    var options = { name: "x_tcomp", selectId: "fcabreciboedit_x_tcomp" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcabreciboedit.lists.tcomp?.lookupOptions.length) {
        options.data = { id: "x_tcomp", form: "fcabreciboedit" };
    } else {
        options.ajax = { id: "x_tcomp", form: "fcabreciboedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_cabrecibo_serie">
    <select
        ...
        data-select2-id="fcabreciboedit_x_serie"
        <?php } ?>
        data-table="cabrecibo"
        data-field="x_serie"
...
<script>
loadjs.ready("fcabreciboedit", function() {
    var options = { name: "x_serie", selectId: "fcabreciboedit_x_serie" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcabreciboedit.lists.serie?.lookupOptions.length) {
        options.data = { id: "x_serie", form: "fcabreciboedit" };
    } else {
        options.ajax = { id: "x_serie", form: "fcabreciboedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<script>
loadjs.ready(["fcabreciboedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fcabreciboedit", "x_fecha", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_cabrecibo_cliente">
    <select
        ...
        data-select2-id="fcabreciboedit_x_cliente"
        <?php } ?>
        data-table="cabrecibo"
...
<script>
loadjs.ready("fcabreciboedit", function() {
    var options = { name: "x_cliente", selectId: "fcabreciboedit_x_cliente" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fcabreciboedit.lists.cliente?.lookupOptions.length) {
        options.data = { id: "x_cliente", form: "fcabreciboedit" };
    } else {
        options.ajax = { id: "x_cliente", form: "fcabreciboedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<?php
    if (in_array("detrecibo", explode(",", $Page->getCurrentDetailTable())) && $detrecibo->DetailEdit) {
?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fcabreciboedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fcabreciboedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
