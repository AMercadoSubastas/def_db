...
/**
 * Table class for tipoindustria
 */
class Tipoindustria extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        // Update Table
        $this->UpdateTable = "tipoindustria";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->activo->Lookup = new Lookup($this->activo, 'tipoindustria', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->activo->Lookup = new Lookup($this->activo, 'tipoindustria', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
        ...
        // nomre
        $this->nomre = new DbField(
            $this, // Table
            'x_nomre', // Variable name
            'nomre', // Name
            '`nomre`', // Expression
            '`nomre`', // Basic search expression
            200, // Type
            50, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`nomre`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->nomre->InputTextType = "text";
        $this->nomre->Nullable = false; // NOT NULL field
        $this->nomre->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        $this->Fields['nomre'] = &$this->nomre;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "tipoindustria";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("TipoindustriaList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "TipoindustriaView" => $Language->phrase("View"),
            "TipoindustriaEdit" => $Language->phrase("Edit"),
            "TipoindustriaAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "TipoindustriaView",
            Config("API_ADD_ACTION") => "TipoindustriaAdd",
            Config("API_EDIT_ACTION") => "TipoindustriaEdit",
            Config("API_DELETE_ACTION") => "TipoindustriaDelete",
            Config("API_LIST_ACTION") => "TipoindustriaList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("TipoindustriaView", $parm);
        } else {
            $url = $this->keyUrl("TipoindustriaView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        if ($parm != "") {
            $url = "TipoindustriaAdd?" . $parm;
        } else {
            $url = "TipoindustriaAdd";
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TipoindustriaEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TipoindustriaList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TipoindustriaAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("TipoindustriaList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("TipoindustriaDelete", $parm);
        }
    }
    ...
    public function renderEditRow()
    {
        ...
        // nomre
        $this->nomre->setupEditAttributes();
        if (!$this->nomre->Raw) {
            $this->nomre->CurrentValue = HtmlDecode($this->nomre->CurrentValue);
        }
        $this->nomre->EditValue = $this->nomre->CurrentValue;
        $this->nomre->PlaceHolder = RemoveHtml($this->nomre->caption());
