...
/**
 * Table class for sucbancos
 */
class Sucbancos extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = "sucbancos";
        $this->TableName = 'sucbancos';
        $this->TableType = "TABLE";
        $this->ImportUseTransaction = $this->supportsTransaction() && Config("IMPORT_USE_TRANSACTION");
        $this->UseTransaction = $this->supportsTransaction() && Config("USE_TRANSACTION");

        // Update Table
        $this->UpdateTable = "sucbancos";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->codpais->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['codpais'] = &$this->codpais;

        // codbanco
        $this->codbanco = new DbField(
            $this, // Table
            'x_codbanco', // Variable name
            'codbanco', // Name
            '`codbanco`', // Expression
            '`codbanco`', // Basic search expression
            3, // Type
            4, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`codbanco`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
        ...
        $this->codbanco->InputTextType = "text";
        $this->codbanco->Raw = true;
        $this->codbanco->Nullable = false; // NOT NULL field
        $this->codbanco->Required = true; // Required field
        $this->codbanco->DefaultErrorMessage = $Language->phrase("IncorrectInteger");
        $this->codbanco->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        $this->Fields['codbanco'] = &$this->codbanco;
        ...
        $this->codsuc->Nullable = false; // NOT NULL field
        $this->codsuc->Required = true; // Required field
        $this->codsuc->SearchOperators = ["=", "<>", "IN", "NOT IN", "STARTS WITH", "NOT STARTS WITH", "LIKE", "NOT LIKE", "ENDS WITH", "NOT ENDS WITH", "IS EMPTY", "IS NOT EMPTY"];
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->activo->Lookup = new Lookup($this->activo, 'sucbancos', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->activo->Lookup = new Lookup($this->activo, 'sucbancos', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "sucbancos";
    }
    ...
    protected function loadDbValues($row)
    {
        ...
        $this->codbanco->DbValue = $row['codbanco'];
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("SucbancosList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "SucbancosView" => $Language->phrase("View"),
            "SucbancosEdit" => $Language->phrase("Edit"),
            "SucbancosAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "SucbancosView",
            Config("API_ADD_ACTION") => "SucbancosAdd",
            Config("API_EDIT_ACTION") => "SucbancosEdit",
            Config("API_DELETE_ACTION") => "SucbancosDelete",
            Config("API_LIST_ACTION") => "SucbancosList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("SucbancosView", $parm);
        } else {
            $url = $this->keyUrl("SucbancosView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SucbancosEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SucbancosList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SucbancosAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("SucbancosList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("SucbancosDelete", $parm);
        }
    }
    ...
    public function renderListRow()
    {
        ...
        $this->codbanco->ViewValue = FormatNumber($this->codbanco->ViewValue, $this->codbanco->formatPattern());
    ...
    public function renderEditRow()
    {
        ...
        // codbanco
        $this->codbanco->setupEditAttributes();
        $this->codbanco->EditValue = $this->codbanco->CurrentValue;
        $this->codbanco->PlaceHolder = RemoveHtml($this->codbanco->caption());
        if (strval($this->codbanco->EditValue) != "" && is_numeric($this->codbanco->EditValue)) {
            $this->codbanco->EditValue = FormatNumber($this->codbanco->EditValue, null);
        }

        // codsuc
        $this->codsuc->setupEditAttributes();
        if (!$this->codsuc->Raw) {
            $this->codsuc->CurrentValue = HtmlDecode($this->codsuc->CurrentValue);
        }
        ...
        if (!$doc->ExportCustom) {
            ...
            if ($doc->Horizontal) { // Horizontal format, write header
                ...
                if ($exportPageType == "view") {
                    ...
                    $doc->exportCaption($this->codbanco);
