...
<span id="el_paises_activo">
    <select
        ...
        data-select2-id="fpaisesedit_x_activo"
        <?php } ?>
        data-table="paises"
        data-field="x_activo"
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fpaisesedit", function() {
    var options = { name: "x_activo", selectId: "fpaisesedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpaisesedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fpaisesedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fpaisesedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
