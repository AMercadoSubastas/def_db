...
<span id="el_impuestos_activo">
    <select
        ...
        data-select2-id="fimpuestosadd_x_activo"
        <?php } ?>
        data-table="impuestos"
        data-field="x_activo"
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fimpuestosadd", function() {
    var options = { name: "x_activo", selectId: "fimpuestosadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fimpuestosadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fimpuestosadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fimpuestosadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
