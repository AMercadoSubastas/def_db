...
$query_comprobante = "SELECT * FROM series  WHERE series.codnum =9";
...
//	VALUES ('$tcomp','$serie','$num_comp','$banco','$sucursal','$numcheque','$codpais','$importe','$vencimiento', '$fecha_ingreso' //, '$tcomprel','$serierel','$num_comp' ,'P' ,'$moneda','$codrem')";
	$strSQL = "INSERT INTO cartvalores
	(tcomp, serie, ncomp, codban, codsuc, codchq , codpais , importe , fechapago , fechaingr ,  serierel , tcomprel , estado , moneda , ncomprel )
	VALUES ('$tcomp','$serie','$num_comp','$banco','$sucursal','$numcheque' ,'$codpais','$importe','$vencimiento', '$fecha_ingreso' ,'$serierel','$tcomprel' ,'P' ,'$moneda',  '$numfac' )";
	
	// 4. Ejecuto la consulta.	
$result = mysqli_query($amercado, $strSQL);				         
			 
$actualiza = "UPDATE `series` SET `nroact` = '$num_comp' WHERE `series`.`codnum` ='9'" ;				 
	$resultado=mysqli_query($amercado,	$actualiza);				              
//$resultado =mysqli_query($amercado, $insetar_banco);
// INSERTAR COTIZACION
$inserta_cotiz = "INSERT INTO cotiz (codmon , fecha , valor ) VALUES ('2' ,'$fecha_ingreso' , $cotizacion)"; 
$resul=mysqli_query($amercado, $inserta_cotiz);	
#comprobamos el resultado de la insercion


echo "<script language='javascript' >";
echo "window.location.href='medios_pago_conA_i.php'";
echo "</script>";
