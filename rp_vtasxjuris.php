...
$query_cabfac = sprintf("SELECT * FROM cabfac WHERE tcomp IN (103, 115,116,117,119,120,121,122,123,124,125,126,127,133,134,135,136,137,138,142,143) AND fecreg BETWEEN %s AND %s ORDER BY fecreg, nrodoc, tcomp", $fecha_desde, $fecha_hasta);
...
$pdf->Cell(22,16,'Rec. Gs. Pub. ',1,0,'L');
...
{	
	...
	elseif ($tcomp == 122 ||  $tcomp == 123 ||  $tcomp ==  124 ||  $tcomp ==  138){
		$tc = "ND-";
		$signo = 1;
	}
	else {
	...
	if ($i <= 22) {
		if ($codrem != "" && $codrem > 0) {
			...
			if (mysqli_num_rows($dir_exhib) > 0) {
				...
				if ($tcomp == 115 || $tcomp == 116 || $tcomp == 117 || $tcomp == 142) {
		...
		$fecha        = substr($row_cabecerafac["fecdoc"],8,2)."-".substr($row_cabecerafac["fecdoc"],5,2)."-".substr($row_cabecerafac["fecdoc"],0,4);
 		switch ($tcomp) {
            ...
            case 142:
                ...
                if ($tcomp == 103)
                    $lotes21      = $row_cabecerafac["totneto"] ;
				$tot_public   = 0.00;
				$tot_gastos   = 0.00;
            ...
            case 137:
                ...
                while ($row_detfac = mysqli_fetch_array($detfac, MYSQLI_BOTH)) {
					...
					if ($row_detfac["concafac"] == 41 || $row_detfac["concafac"] == 43 || $row_detfac["concafac"] == 71)
                        $tot_gastos   += $row_detfac["neto"];
                    if ($row_detfac["concafac"] == 14 || $row_detfac["concafac"] == 17)
				        $lotes21      += $row_detfac["neto"];
                    if ($row_detfac["concafac"] == 13 || $row_detfac["concafac"] == 16)
				        $lotes105     += $row_detfac["neto"];
                    if ($row_detfac["concafac"] == 19 || $row_detfac["concafac"] == 36 || $row_detfac["concafac"] == 44)                        $tot_comision = $row_detfac["neto"];
                ...
                $tot_servicios= 0.00;
				 ...
				 while ($row_detfac = mysqli_fetch_array($detfac, MYSQLI_BOTH)) {
                     ...
                     if ($row_detfac["concafac"] == 10)
                         $lotes105 += $row_detfac["neto"];
                     if ($row_detfac["concafac"] == 11 || $row_detfac["concafac"] == 14)
                         $lotes21 += $row_detfac["neto"];
                     if ($row_detfac["concafac"] == 27  || $row_detfac["concafac"] == 19)
                         $tot_servicios += $row_detfac["neto"];
                     if (($row_detfac["concafac"] == 26 || $row_detfac["concafac"] == 44) && $tot_comision == 0.00)
                         $tot_comision += $row_detfac["neto"];
                }
                $cliente      = $row_cabecerafac["cliente"];
                $tot_iva21    = $row_cabecerafac["totiva21"];
        ...
        $pdf->Cell(24,6,utf8_decode($tot_resol),0,0,'L');
 		...
 		switch ($tcomp) {
            ...
            case 137:
                ...
                while ($row_detfac = mysqli_fetch_array($detfac, MYSQLI_BOTH)) {
                    ...
                    if ($row_detfac["concafac"] == 13 || $row_detfac["concafac"] == 16)
				        $lotes105     += $row_detfac["neto"];
                    if ($row_detfac["concafac"] == 19 || $row_detfac["concafac"] == 36 || $row_detfac["concafac"] == 44)
                        $tot_comision = $row_detfac["neto"];
                    if ($row_detfac["concafac"] == 18 || $row_detfac["concafac"] == 15 || $row_detfac["concafac"] == 21 || $row_detfac["concafac"] == 25 || $row_detfac["concafac"] == 32 || $row_detfac["concafac"] == 42)
				        $tot_servicios += $row_detfac["neto"];
                }
...
$pdf->Cell(26,16,' Rec. Gs. Pub. ',1,0,'L');
...
for ($j = 1; $j < 30; $j++) {
	...
	$pdf->SetY($valor_y);
	$pdf->Cell(10);
	$pdf->Cell(30,6,utf8_decode($descrip_prov[$j]),0,0,'L');
	$pdf->SetX(70);
