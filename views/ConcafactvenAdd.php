...
$Page->showMessage();
?>
<form name="fconcafactvenadd" id="fconcafactvenadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<script>
loadjs.ready(["fconcafactvenadd", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fconcafactvenadd", "x_fechahora", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_concafactven_activo">
    <select
        ...
        data-select2-id="fconcafactvenadd_x_activo"
        <?php } ?>
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fconcafactvenadd", function() {
    var options = { name: "x_activo", selectId: "fconcafactvenadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fconcafactvenadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fconcafactvenadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fconcafactvenadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<script>
loadjs.ready("fconcafactvenadd", function() {
    fconcafactvenadd.createAutoSuggest(Object.assign({"id":"x_tipoiva","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->tipoiva->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.concafactven.fields.tipoiva.autoSuggestOptions));
});
</script>
...
<script>
loadjs.ready("fconcafactvenadd", function() {
    fconcafactvenadd.createAutoSuggest(Object.assign({"id":"x_impuesto","forceSelect":false}, { lookupAllDisplayFields: <?= $Page->impuesto->Lookup->LookupAllDisplayFields ? "true" : "false" ?> }, ew.vars.tables.concafactven.fields.impuesto.autoSuggestOptions));
});
</script>
...
<span id="el_concafactven_tieneresol">
    <select
        ...
        data-select2-id="fconcafactvenadd_x_tieneresol"
        <?php } ?>
        data-table="concafactven"
...
<script>
loadjs.ready("fconcafactvenadd", function() {
    var options = { name: "x_tieneresol", selectId: "fconcafactvenadd_x_tieneresol" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fconcafactvenadd.lists.tieneresol?.lookupOptions.length) {
        options.data = { id: "x_tieneresol", form: "fconcafactvenadd" };
    } else {
        options.ajax = { id: "x_tieneresol", form: "fconcafactvenadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fconcafactvenadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fconcafactvenadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
