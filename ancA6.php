...
$query_comprobante = sprintf("SELECT * FROM series  WHERE series.codnum = %s", "31");
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
	...
	$DocNro = $cuit_enti2;//30710183437; //30661087753; // 30710183437; //30661087753;
    /**
   	...
   	if (isset($_POST['totiva105']) && $_POST['totiva105'] != 0.00 && isset($_POST['totiva21']) && $_POST['totiva21'] != 0.00) {
		...
		if (isset($_POST['totiva21']) && $_POST['totiva21'] != 0.00) {
			...
			if (isset($_POST['totiva105']) && $_POST['totiva105'] != 0.00) {
    			...
    			$IvaAlicuotaImporte = $_POST['totiva105'] ;//10.5.00;   
			}
			else {
	...
	if ($ta = $wsaa->loginCms())
	{
    	...
    	//La estructura de este array esta dise�ada de 
		//acuerdo al registro XML del WebService y
		//utiliza las variables antes declaradas.
        $FeCAEReq = array (
            'FeCAEReq' => array (
                'FeCabReq' => array (
                    'CantReg' => 1,
	...
	if ($sigo_y_grabo == 1) {
		...
		// HASTA ACA ==========================================================================
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
            $query_mascara2 = "SELECT * FROM series  WHERE series.codnum='31'";
            ...
            if (GetSQLValueString($_POST['tcbterel'], "int") == 2)
                $mascara2 =substr($_POST['descripcion'],1,60).' Apl.Nd. '.$mascara2;
            else
                $mascara2 =substr($_POST['descripcion'],1,60).' Apl.Fc. '.$mascara2;
            $mascara2 = "'".$mascara2."'";
            //echo $mascara2;
  			$insertSQL = sprintf("INSERT INTO detfac (tcomp, serie, ncomp,  concafac, descrip, neto, porciva, comcob, usuario) VALUES (%s, %s, %s, %s,  %s, %s, %s, %s, %s)",
                       61, //GetSQLValueString($_POST['tcomp'], "int"),
                       31, //GetSQLValueString($_POST['serie'], "int"),
                       $num_fac,
			...
			if (GetSQLValueString($_POST['concepto'], "int") == 18) {
                $importe_tasa += GetSQLValueString($_POST['importe'], "double");
            }
		}
	}
	if (isset($_POST['concepto1']) && GetSQLValueString($_POST['concepto1'], "int")!="NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
  			$insertSQL = sprintf("INSERT INTO detfac (tcomp, serie, ncomp,  concafac, descrip, neto, porciva, comcob, usuario) VALUES (%s, %s, %s, %s,  %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString(substr(utf8_encode($_POST['descripcion1']),3,80), "text"),
	       ...
	       if (GetSQLValueString($_POST['concepto1'], "int") == 18) {
                $importe_tasa += GetSQLValueString($_POST['importe1'], "double");
            }
		}
	}
	if (isset($_POST['concepto2']) && GetSQLValueString($_POST['concepto2'], "int")!="NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
  			$insertSQL = sprintf("INSERT INTO detfac (tcomp, serie, ncomp,  concafac, descrip, neto, porciva, comcob, usuario) VALUES (%s, %s, %s, %s,  %s, %s, %s, %s, %s)",
                       ...
                       GetSQLValueString(substr(utf8_encode($_POST['descripcion2']),3,80), "text"),
			...
			if (GetSQLValueString($_POST['concepto2'], "int") == 18) {
                $importe_tasa += GetSQLValueString($_POST['importe2'], "double");
            }
		}
	}
	if (isset($_POST['concepto3']) && GetSQLValueString($_POST['concepto3'], "int")!="NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
            ...
            if (GetSQLValueString($_POST['concepto3'], "int") == 18) {
                $importe_tasa += GetSQLValueString($_POST['importe3'], "double");
            }
		}
	}
	if (isset($_POST['concepto4']) && GetSQLValueString($_POST['concepto4'], "int")!="NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
	       ...
	       if (GetSQLValueString($_POST['concepto4'], "int") == 18) {
                $importe_tasa += GetSQLValueString($_POST['importe4'], "double");
            }
		}
	}
	...
	if (isset($_POST['concepto9']) && GetSQLValueString($_POST['concepto9'], "int")!="NULL") {
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
  			$insertSQL = sprintf("INSERT INTO detfac (tcomp, serie, ncomp, codlote, descrip, neto, porciva, comcob, usuario) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)",
                       61, //GetSQLValueString($_POST['tcomp'], "int"),
                       31, //GetSQLValueString($_POST['serie'], "int"),
                       $num_fac,
	...
	if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text")!="NULL") {
		...
		if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
			...
			$insertSQL = sprintf("INSERT INTO cabfac (tcomp, serie, ncomp, fecval, fecdoc, fecreg, cliente, fecvenc, estado, emitido, totbruto, totiva21, totneto21, nrengs, nrodoc, en_liquid, CAE, CAEFchVto, Resultado, usuario, usuarioultmod, fecultmod) VALUES (%s, %s, %s, '$fecha_factura1','$fecha_factura1', '$fecha_factura1', %s, '$fecha_factura1', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
						   61, //GetSQLValueString($_POST['tcomp'], "int"),
						   31, //GetSQLValueString($_POST['serie'], "int"),
						   $num_fac,
		...
		//==========================================================================
			if (!empty($_POST['imprime'])) { 
				...
				$facnum = $CbteDesde;
				$insertGoTo = "facturaCAI_ok.php?factura=$facnum";
				if (isset($_SERVER['QUERY_STRING'])) {
					$insertGoTo .= (strpos($insertGoTo, '?')) ? "&" : "?";
					$insertGoTo .= $_SERVER['QUERY_STRING'];
				}
				header(sprintf("Location: %s", "facturaCAI_ok.php?factura=$facnum")); 
			}
		}
	}
}
...
mysqli_select_db($amercado, $database_amercado);
$query_facturas_a = "SELECT * FROM series  WHERE series.codnum=31";
...
$tcomp = 61;
mysqli_select_db($amercado, $database_amercado);
$query_facturas_b = "SELECT * FROM series  WHERE series.codnum=31";
...
mysqli_select_db($amercado, $database_amercado);
$query_tipo_comprobante = "SELECT * FROM tipcomp WHERE codnum='61'";
...
var selec = form.tipos.options;
    ...
    if (selec[2].selected == true)
    {
		...
		form.tcbterel.value = 2;
		form.tcbterel_texto.value = "N DEB A0006";
...
{
    //alert("entr� a validarFormulario");
    ...
    if (monto.length!=0) {
        //alert("entro al primer concepto");
		if (eval(porciva)===0) {
    ...
    //strAlerta = "form.totneto_1.value " + form.totneto_1.value + "\n" + "form.totneto21_1.value " + form.totneto21_1.value + "\n" + "form.totiva21_1.value " + form.totiva21_1.value + "\n" + "form.tot_general_1.value " + form.tot_general_1.value ;
	//alert(strAlerta);
    // Oculto   
	...
	form.tot_general.value = tot_general.toFixed(2) ;
    // Visible
    //alert("le paso los totales al pie ");
    ...
    strAlerta =  "form.totneto21_1.value " + form.totneto21_1.value + "\n" + "form.totiva21_1.value " + form.totiva21_1.value + "\n" + "form.tot_general_1.value " + form.tot_general_1.value ;
	//alert(strAlerta);
...
function agregarOpciones(form)
{
    ...
    if (selec[1].selected == true) 	{
		form.serie.value = 31;
		form.serie_texto.value = "SERIE DE N CRED A0006";
		form.tcomp.value = 61;
	}
}
</script>
...
<form id="factura" name="factura" method="POST" action="<?php echo $editFormAction; ?>">
  	<table width="900" height="740" border="2" align="left" cellpadding="1" cellspacing="1" bgcolor="#FFFFFF">
    <tr>
      <td colspan="3" ><div align="center"><img src="images/not_cred_auto.gif" width="450" height="30" /></div></td>
    </tr>
    <tr>
      ...
      <tr>
         ...
         <td width="36%"><select name="tipos" onChange="agregarOpciones(this.form)">
                                <option value="61">NOTA DE CRED INMOB A0006</option>
                         </select>
		  	<input name="tcomp" type="hidden" value="61" size="25" >
		 </td>
         <td width="15%" class="ewTableHeader">Serie</td>
         <td width="35%"><input name="serie_texto" type="text" value="SERIE DE N CRED INMOB A00006" size="24" />
    ...
    <tr>
        <td colspan="3" valign="top" bgcolor="#FFFFFF"><table width="100%" border="1" cellspacing="1" cellpadding="1">
          ...
          <td width="20%"><select name="tcbterel2" onChange="agregarOpciones2(this.form)">
                              ...
                              <option value="2">N DEB A0006</option>
        ...
        <tr>
 		...
 		<tr>
			...
			<td bgcolor="#FFFFFF"><input name="tipoiva" size="10" onchange="getprov(this.form)">
          	...
          	<td bgcolor="#FFFFFF"><input name="importe" type="text" id="importe"  onchange="validarFormulario(this.form)" size="20"   /></td>
        ...
        <tr>
        	<td align="center"><input name="totneto21" type="hidden" id="totneto21" size="12" /></td>
          	<td align="center"><input name="totiva21"  type="hidden"  id="totiva21" size="12" /></td>
           	<td align="center"><input name="tot_general" type="hidden"  id="tot_general" size="15" />
			</td>
        </tr>
        <tr>
            ...
            <td align="center"><input name="tot_general_1" type="text" id="tot_general_1" size="25" />
			</td>
       	</tr>
      	</table>
		</td>
