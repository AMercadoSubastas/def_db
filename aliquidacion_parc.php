...
//echo "COD NUM  ".$codnum."<br>";
$codigo  = explode (",",$codnum);
//echo "SIZE  ".sizeof($codigo)."<br>";
//echo "REMATE  ".$rematenum."<br>";
?>
<?php
	...
	$insertSQL = sprintf("INSERT INTO liquidacion (tcomp, serie, cliente, codrem, fecharem, totremate, totneto1, totiva21, subtot1, totneto2, totiva105, subtot2, totacuenta, totgastos, totvarios, saldoafav , ncomp ,  fechaliq , codpais , codprov , codloc , rubro ,nrodoc ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s , '$fecha_liquidacion' , %s, %s, %s,%s ,'$nrodoc')",
  	...
  	$actualiza_serie = mysqli_query($amercado, $actualiza) or die(mysqli_error($amercado));
   	...
   	for ($a=0 ; $a < sizeof($codigo); $a++) {
   		...
   		$actualizafac = sprintf("UPDATE cabfac SET tcompsal= %s , seriesal= %s  , ncompsal = %s  WHERE codnum='$codigo[$a]'",
                       GetSQLValueString($_POST['tcomp'], "int"),
                       GetSQLValueString($_POST['serie'], "int"),
					   GetSQLValueString($_POST['liquidacion'], "int"));
		$actualiza_fac = mysqli_query($amercado, $actualizafac) or die(mysqli_error($amercado));
 		$query_Recordset1 = "SELECT cabfac.tcomp , cabfac.serie, cabfac.ncomp FROM cabfac WHERE codnum='$codigo[$a]'";
    	$Recordset1 = mysqli_query($amercado, $query_Recordset1) or die(mysqli_error($amercado));
   		$row_Recordset1 = mysqli_fetch_assoc($Recordset1);
   		$tip_comp= $row_Recordset1['tcomp'];
   		$serie_comp= $row_Recordset1['serie'];
   		$nro_comp= $row_Recordset1['ncomp'];
   		$actualizadetfac = sprintf("UPDATE detfac SET tcomsal= %s , seriesal= %s  , ncompsal = %s  WHERE tcomp ='$tip_comp' AND
		serie = '$serie_comp' AND ncomp = '$nro_comp' ",
                       GetSQLValueString($_POST['tcomp'], "int"),
                       GetSQLValueString($_POST['serie'], "int"),
					   GetSQLValueString($_POST['liquidacion'], "int"));
	
		$actualizadet_fac = mysqli_query($amercado, $actualizadetfac) or die(mysqli_error($amercado));
...
$rematenum = $_POST['remate'];
$liquinum = $_POST['remate'];
if(isset($_POST['remate'])){  
	$res = mysqli_query($amercado, "select * from remates where ncomp='$rematenum'") or die(mysqli_error($amercado));
	...
	$total_21 = 0 ;
	for ($a=0 ; $a<sizeof($codigo);$a++) {
		$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) , SUM( `totneto21` *'$procen_iva' ) , SUM( `totneto21` *'$procen_iva' + `totneto21`)  as total_21 FROM `cabfac` WHERE `codnum` = '$codigo[$a]'" ;
		$query_remate = mysqli_query($amercado, $res1) or die(mysqli_error($amercado));
		$totalRows_remate = mysqli_num_rows($query_remate); 

		$totneto105 = $totneto105 + mysqli_result($query_remate,0,0);
		$iva105 = $iva105 + mysqli_result($query_remate,0,1);
		$total_105 = $total_105 +mysqli_result($query_remate,0,2);
		$totneto21 = $totneto21 + mysqli_result($query_remate,0,3);
		$iva21 = $iva21 + mysqli_result($query_remate,0,4);
		$total_21 = $total_21+ mysqli_result($query_remate,0,5);
	}
...
mysqli_select_db($amercado, $database_amercado);
$query_cheques_total = "SELECT SUM( cartvalores.importe) FROM cartvalores WHERE  cartvalores.estado = 'S' AND cartvalores.codrem ='$rematenum' AND cartvalores.ncompsal = '$liquidacion'" ;
...
$query_factura_total = "SELECT SUM( cabfac.totbruto) FROM cabfac WHERE cabfac.tcomp =18 AND cabfac.serie = 1 AND cabfac.estado = 'S' AND cabfac.codrem ='$rematenum'AND cabfac.ncompsal = NULL" ;
...
<form name="cheque_tercero" action="cheques_terceros_2.php" method="post">
...
function manda_gastos_autorizados(form)
{
	...
	//gastos_autorizados.fecha_remate_b.value = liquidacion.fecha_remate.value ;
...
?>
      ...
      <input name="num_cliente" type="hidden"  value="<?php echo $clientes ?>" readonly=""/>
        ...
        </tr><input name="cod_num" value="<?php echo $codnum ?>" type="hidden"  />
      </table></td>
    </tr>
    <tr>
