...
$csv_file = "VTAS_X_JURIS".$anio.$mes.".txt";  
	...
	*/
	...
	if ($i <= 22) {
		...
		switch ($tcomp) {
					...
					if ($row_cabefac = mysqli_fetch_assoc($cabefac)) {
						if ($row_cabefac["tcomp"] == 54) {
							...
							if ($row_cabecerafac["totiva105"] == 0.00 && $row_cabecerafac["totiva21"] == 0.00) {
                             ...
                             $cliente      = $row_cabecerafac["cliente"];
                              $tot_servicios =  $row_cabecerafac["totcomis"];
			...
			case 94:
			case 60:
				if (($row_cabecerafac["totiva21"] != 0 && $row_cabecerafac["totiva105"] != 0) && ($row_cabecerafac["ncomp"] != 177 && $row_cabecerafac["ncomp"] != 378 && $row_cabecerafac["ncomp"] != 379 && $row_cabecerafac["ncomp"] != 380)) {
                    $cliente      = $row_cabecerafac["cliente"];
                    $lotes21      = $row_cabecerafac["totneto21"] ;
                    $lotes105     = $row_cabecerafac["totneto105"];
                    $tot_comision = $row_cabecerafac["totcomis"];
  	...
  	/*
			...
			$valor_y = $valor_y + 6;
			*/
			$csv.=$fecha.$csv_sep.$nroorig.$csv_sep.$nom_cliente.$csv_sep.$tot_public.$csv_sep.$tot_servicios.$csv_sep.$tot_comision.$csv_sep.$lotes21.$csv_sep.$lotes105.$csv_sep.$tot_iva.$csv_sep.$tot_resol.$csv_sep.$total.$csv_sep.$codrem.$csv_end;
            //echo $csv;
		}
	}
	else {
...
echo "<BR>"."======================== SE GENERO CORRECTAMENTE EL ARCHIVO ".$csv_file."========================"."<BR>";
