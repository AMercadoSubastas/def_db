...
$Page->showMessage();
?>
<form name="fformapagoadd" id="fformapagoadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<span id="el_formapago_activo">
    <select
        ...
        data-select2-id="fformapagoadd_x_activo"
        <?php } ?>
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fformapagoadd", function() {
    var options = { name: "x_activo", selectId: "fformapagoadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fformapagoadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fformapagoadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fformapagoadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fformapagoadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fformapagoadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
