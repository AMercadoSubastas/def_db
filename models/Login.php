...
class Login extends Usuarios
{
    ...
    public function run()
    {
        ...
        $this->Username = new DbField("usuarios", "username", "username", "username", "", 202, 255, -1, false, "", false, false, false);
        $this->Username->EditAttrs->appendClass("form-control ew-form-control");
        $this->Password = new DbField("usuarios", "password", "password", "password", "", 202, 255, -1, false, "", false, false, false);
        $this->Password->EditAttrs->appendClass("form-control ew-form-control");
        if (Config("ENCRYPTED_PASSWORD")) {
            $this->Password->Raw = true;
        }
        $this->LoginType = new DbField("usuarios", "type", "logintype", "logintype", "", 202, 255, -1, false, "", false, false, false);
        ...
        if (IsLoggingIn()) { // After changing password or authorized by 2FA
            ...
            if ($validate) {
                ...
                if ($validate) {
                    ...
                    if ($validPwd && $profile->emptyPasswordChangedDate()) {
                        $profile->setLastPasswordChangedDate(StdCurrentDate())->saveToStorage();
                    }
