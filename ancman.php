...
if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text")!="NULL") {
	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
		...
		$actualiza1 = sprintf("UPDATE `series` SET `nroact` = %s WHERE `series`.`codnum` = %s", 
				GetSQLValueString($_POST['num_factura'], "int"), GetSQLValueString($_POST['serie'], "int")) ;				 
...
<script language="javascript">
 function forma_pago(form)
 {
  	...
  	if (tipo_comprobante=="" || total=="" || serie_num=="" || fecha_factura1=="" ) {
		...
		if (fecha_factura1=="") {
          	error = error+"      Fecha de la Nota de Credito\n"; 
        }	
...
var selec = form.tipos.options;
    ...
    if (selec[1].selected == true)
    {
		...
		factura.tcomp.value = 7;
    }

    if (selec[2].selected == true)
    {
		factura.serie.value = 12;
		factura.serie_texto.value = "NOTA DE CREDITO B0002";
		factura.tcomp.value = 26;
    }
}
</script>
</script>
...
<script type="text/javascript">
	...
	function getFactura()
	{
		...
		if( facturaId!=currentFacturaID){
			...
			ajax01.requestFile = 'getnumfactura.php?getfacturaId='+facturaId+'&&getserieId='+GetSerie+'&&gettcompId='+GetTcomp;	// Specifying which file to get
...
<form id="factura" name="factura" method="POST" action="<?php echo $editFormAction; ?>">
  <table width="640" border="0" align="left" cellpadding="1" cellspacing="1" bgcolor="#0094FF">
    <tr>
      <td colspan="3" background="images/fondo_titulos.jpg"><div align="center"><img src="images/not_cred_manu.gif" width="330" height="30" /></div></td>
    </tr>
    <tr>
      <td colspan="3" valign="top" bgcolor="#003366"><table width="100%" border="0" cellspacing="1" cellpadding="1">
		 ...
		 <td height="20" class="ewTableHeader">&nbsp;Tipo de Comprobante </td>
          <td width="24%"><select name="tipos" onChange="agregarOpciones(this.form)">
                                      ...
                                      <option value="7">NOTA DE CREDITO MANUAL A0002</option>
                                      <option value="26">NOTA DE CREDITO MANUAL B0002</option>
                           </select>
