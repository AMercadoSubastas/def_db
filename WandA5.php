...
if ($sigo_y_grabo == 1 && $todo_ok ==1) {
	...
	if (isset($_POST['lote']) && GetSQLValueString($_POST['lote'], "int")!="NULL") {
  		...
  		$query_mascara = "SELECT * FROM series  WHERE series.codnum='37'";//'$tcomp'  AND series.codnum='$serie'";
    ...
    $beLoc = $row_loc['descripcion'];
			...
			if ($i == 0) {
				...
				// importe de IVA
				$importeIva = (intval($_POST[$iNameImpuesto]) / 100) * intval($_POST[$iName]);
			...
			for ($i=0; $i < $renglones; $i++) {
				...
				$item = [
					...
					"Item_ImporteIVANoGravado" => $itemIvaNoInscript[$i],
					"Item_TipoIVA" => "1",
		...
		$comprobanteRelacionado = [
			"Comprobante_Cancelatorio_Tipo" => "ND",
	...
	mysqli_select_db($amercado, $database_amercado);
	$query_facturas_a = "SELECT * FROM series  WHERE  series.codnum=37";
	...
	mysqli_select_db($amercado, $database_amercado);
	$query_facturas_b = "SELECT * FROM series  WHERE  series.codnum=37";
...
var selec = form.tipos.options;
    ...
    if (selec[1].selected == true)
    {
		...
		factura.serie_texto.value = "SERIE NOTA DEB A00005";
		factura.tcomp.value = 94;
...
function  valor_prueba(form) {
	...
	if (factura.impuesto[2].checked==true) {
    	var impuesto = 0; //(factura.impuesto[2].value)/100;
    }
...
function validarFormulario(form)
{
	...
	if (factura.impuesto[2].checked==true) {
		if (monto.length!=0) {
			...
			tot_mon21 = eval(monto); 
			tot_mon21 = tot_mon21.toFixed(2);
	        imp_tot21 = eval(monto+('+')+((comi*monto)/100))*imp0;
		}  
	}
...
chainedSelects.init(); 
	...
	function validar_fatura(form)
	{
		...
		// DESDE ACA VALIDACION DE IMPUESTOS//
  		...
  		var impuesto =   factura.impuesto[0].checked;//
  		var impuesto_0 = factura.impuesto[1].checked;//
		var impuesto_1 = factura.impuesto[2].checked;//
  		var impuesto1 =   factura.impuesto1[0].checked;//
  		var impuesto1_0 = factura.impuesto1[1].checked;//
  		var impuesto2 =   factura.impuesto2[0].checked;//
  		var impuesto2_0 = factura.impuesto2[1].checked;//
    	...
    	var impuesto5 =   factura.impuesto5[0].checked;//
  		...
  		var impuesto14_0 = factura.impuesto14[1].checked;//
  		var imp = 1;
  		if (impuesto==false && impuesto_0==false && impuesto_1==false && lote !="" ) {
	  		var imp = 0; 
  		}
   		if (impuesto1==false && impuesto1_0==false && lote1 !="" ) {
			var imp = 0; 
    ...
    <tr>
      ...
      <td width="14%" height="20" bgcolor="#82BADD">&nbsp;<span class="ewTableHeader">Tipo de Cbte </span></td>
          <td width="40%"><select name="tipos" onChange="agregarOpciones(this.form)">
                                      ...
                                      <option value="94">NOTA DE DEBITO A00005</option>
    ...
    <tr>
	  ...
	  <table width="100%" border="1" cellpadding="1" cellspacing="1" bgcolor="#82BADD">
        ...
        <tr>
          ...
          <td class="ewTableHeader"><div align="center"><?php echo 0     ?></div></td>
        </tr>
		...
		<tr>
         ...
         <?php if($nivel=='9') { ?> <td bgcolor="#82BADD"><input name="comision" type="text" id="comision" size="3" /></td> <?php } else { ?>
          ...
          <td bgcolor="#82BADD" width="26" align="center"><input type="radio" name="impuesto" value="<?php  echo 0     ?>" onClick="validarFormulario(this.form)"/></td>
