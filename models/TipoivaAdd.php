 ...
 * Page class
 */
class TipoivaAdd extends Tipoiva
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "TipoivaAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("TipoivaList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    } elseif (GetPageName($returnUrl) == "TipoivaView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "TipoivaList") {
                            ...
                            $returnUrl = "TipoivaList"; // Return list page content
                        }
                    }
    ...
    {
        $this->discrimina->DefaultValue = $this->discrimina->getDefault(); // PHP
        $this->discrimina->OldValue = $this->discrimina->DefaultValue;
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'discrimina' first before field var 'x_discrimina'
        $val = $CurrentForm->hasValue("discrimina") ? $CurrentForm->getValue("discrimina") : $CurrentForm->getValue("x_discrimina");
        if (!$this->discrimina->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->discrimina->Visible = false; // Disable update for API request
            } else {
                $this->discrimina->setFormValue($val);
            }
        }
    ...
    public function restoreFormValues()
    {
        ...
        $this->discrimina->CurrentValue = $this->discrimina->FormValue;
    ...
    public function renderRow()
    {
        ...
        // discrimina
        $this->discrimina->RowCssClass = "row";
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            ...
            if ($this->discrimina->Visible && $this->discrimina->Required) {
                if ($this->discrimina->FormValue == "") {
                    $this->discrimina->addErrorMessage(str_replace("%s", $this->discrimina->caption(), $this->discrimina->RequiredErrorMessage));
                }
            }
    ...
    protected function restoreAddFormFromRow($row)
    {
        ...
        if (isset($row['discrimina'])) { // discrimina
            $this->discrimina->setFormValue($row['discrimina']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("TipoivaList"), "", $this->TableVar, true);
