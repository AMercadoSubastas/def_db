...
if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text")!="NULL") {
	...
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
		...
		//echo "   FECHA_FACTURA1 -    ".$fecha_factura1."    -    ";
		...
		if ($totalRows_medios_pago==0 && strcmp(GetSQLValueString($_POST['GrupoOpciones1'], "text"),"'S'")==0) {
   			...
   			$totalRows_comprobante = mysqli_num_rows($comprobante);
    		$num_comp = ($row_comprobante['nroact'])+1 ; 
  			$strSQL = sprintf("INSERT INTO cartvalores (tcomp, serie, ncomp,  codpais , importe , fechapago , fechaingr ,  serierel , tcomprel , estado , moneda ,codrem , ncomprel )	VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
...
setcookie("concepto","");
mysqli_select_db($amercado, $database_amercado);
$query_facturas_a = "SELECT * FROM series  WHERE series.tipcomp=1";
...
mysqli_select_db($amercado, $database_amercado);
$query_facturas_b = "SELECT * FROM series  WHERE series.tipcomp=23";
...
mysqli_select_db($amercado, $database_amercado);
$query_cliente = "SELECT * FROM entidades WHERE tipoent='2' OR tipoent='1' ORDER BY razsoc ASC";
...
function agregarOpciones(form)
{
    ...
    if (selec[1].selected == true)
    {
		...
		factura.serie_texto.value = "NOTA DE CREDITO A0002";
		factura.tcomp.value = 22;
    }
    ...
    if (selec[2].selected == true)
    {
		...
		factura.serie_texto.value = "NOTA DE DEBITO B0002";
		factura.tcomp.value = 30;
	}
}
</script>
...
<script type="text/javascript">
	...
	var ajax01 = new sack();
	var currentFacturaID=false;
	// Primer Lote
	function getFactura()
	{
		var facturaId = document.getElementById('num_factura').value.replace(/[^0-9]/g,'');
		var GetSerie  = document.getElementById('serie').value ;
		var GetTcomp = document.getElementById('tcomp').value  ;
	if( facturaId!=currentFacturaID){
		currentFacturaId = facturaId
		ajax01.requestFile = 'getnumfactura.php?getfacturaId='+facturaId+'&&getserieId='+GetSerie+'&&gettcompId='+GetTcomp;	
		ajax01.onCompletion = showFactura;	// Specify function that will be executed after file has been found
		ajax01.runAJAX();		// Execute AJAX function			
	}
}
...
function showFactura() // Muestra Factura
{
	var formObj01 = document.forms['factura'];	
	eval(ajax01.response);
}	
...
function initFormEvents()
{
	...
	document.getElementById('num_factura').onblur = getFactura;
...
<form id="factura" name="factura" method="POST" action="<?php echo $editFormAction; ?>">
  <table width="640" border="0" align="left" cellpadding="1" cellspacing="1" bgcolor="#0094FF">
    <tr>
      <td colspan="3" background="images/fondo_titulos.jpg"><div align="center"><img src="images/not_deb_manu.gif" width="330" height="30" /></div></td>
    </tr>
    <tr>
      <td colspan="3" valign="top" bgcolor="#003366"><table width="100%" border="0" cellspacing="1" cellpadding="1">
         <tr>
		 <td height="20" class="ewTableHeader">&nbsp;Tipo de Comprobanta </td>
          <td width="24%"><select name="tipos" onChange="agregarOpciones(this.form)">
                                      <option value="">[seleccione una opci&oacute;n]</option>
                                      <option value="18">NOTA DE DEBITO MANUAL A0002</option>
                                      <option value="27">NOTA DE DEBITO MANUAL B0002</option>
                           </select>
