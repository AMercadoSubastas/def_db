 ...
 * Page class
 */
class DetfacPreview extends Detfac
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "DetfacPreview";
    ...
    // CSS class/style
    public $CurrentPageName = "DetfacPreview";
    ...
    protected function masterKeyUrl()
    {
        ...
        if ($masterTblVar == "cabfac") {
            $url = "" . Config("TABLE_SHOW_MASTER") . "=cabfac&" . GetForeignKeyUrl("fk_tcomp", $this->tcomp->QueryStringValue) . "&" . GetForeignKeyUrl("fk_serie", $this->serie->QueryStringValue) . "&" . GetForeignKeyUrl("fk_ncomp", $this->ncomp->QueryStringValue) . "";
        }
        return $url;
    }
    ...
    protected function setupForeignKeys($keys)
    {
        $masterTblVar = Get("t", "");
        if ($masterTblVar == "cabfac") {
            $this->tcomp->setQueryStringValue($keys[0] ?? "");
            $this->serie->setQueryStringValue($keys[1] ?? "");
            $this->ncomp->setQueryStringValue($keys[2] ?? "");
        }
    }
