...
/**
 * Table class for concafact
 */
class Concafact extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->fechahora = new DbField(
            ...
            CastDateFieldForLike("`fechahora`", 1, "DB"), // Basic search expression
            135, // Type
            19, // Size
            1, // Date/Time format
            false, // Is upload field
        ...
        $this->fechahora->DefaultErrorMessage = str_replace("%s", DateFormat(1), $Language->phrase("IncorrectDate"));
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->activo->Lookup = new Lookup($this->activo, 'concafact', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->activo->Lookup = new Lookup($this->activo, 'concafact', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
        ...
        $this->tipoiva->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        $this->impuesto->setSelectMultiple(false); // Select one
        $this->impuesto->UsePleaseSelect = true; // Use PleaseSelect by default
        $this->impuesto->PleaseSelectText = $Language->phrase("PleaseSelect"); // "PleaseSelect" text
        global $CurrentLanguage;
        switch ($CurrentLanguage) {
            case "en-US":
        ...
        $this->impuesto->SearchOperators = ["=", "<>", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN", "IS NULL", "IS NOT NULL"];
        ...
        switch ($CurrentLanguage) {
            case "en-US":
                $this->tieneresol->Lookup = new Lookup($this->tieneresol, 'concafact', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
            default:
                $this->tieneresol->Lookup = new Lookup($this->tieneresol, 'concafact', false, '', ["","","",""], '', '', [], [], [], [], [], [], false, '', '', "");
                break;
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "concafact";
    }
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("ConcafactList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "ConcafactView" => $Language->phrase("View"),
            "ConcafactEdit" => $Language->phrase("Edit"),
            "ConcafactAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "ConcafactView",
            Config("API_ADD_ACTION") => "ConcafactAdd",
            Config("API_EDIT_ACTION") => "ConcafactEdit",
            Config("API_DELETE_ACTION") => "ConcafactDelete",
            Config("API_LIST_ACTION") => "ConcafactList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("ConcafactView", $parm);
        } else {
            $url = $this->keyUrl("ConcafactView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ConcafactEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ConcafactList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ConcafactAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("ConcafactList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("ConcafactDelete", $parm);
        }
    }
