...
$Page->showMessage();
?>
<form name="fdir_rematesadd" id="fdir_rematesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<span id="el_dir_remates_codrem">
    <select
        ...
        data-select2-id="fdir_rematesadd_x_codrem"
        <?php } ?>
...
<?php if (!$Page->codrem->IsNativeSelect) { ?>
<script>
loadjs.ready("fdir_rematesadd", function() {
    var options = { name: "x_codrem", selectId: "fdir_rematesadd_x_codrem" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdir_rematesadd.lists.codrem?.lookupOptions.length) {
        options.data = { id: "x_codrem", form: "fdir_rematesadd" };
    } else {
        options.ajax = { id: "x_codrem", form: "fdir_rematesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_dir_remates_codpais">
    <select
        ...
        data-select2-id="fdir_rematesadd_x_codpais"
        <?php } ?>
        data-table="dir_remates"
...
<script>
loadjs.ready("fdir_rematesadd", function() {
    var options = { name: "x_codpais", selectId: "fdir_rematesadd_x_codpais" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdir_rematesadd.lists.codpais?.lookupOptions.length) {
        options.data = { id: "x_codpais", form: "fdir_rematesadd" };
    } else {
        options.ajax = { id: "x_codpais", form: "fdir_rematesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_dir_remates_codprov">
    <select
        ...
        data-select2-id="fdir_rematesadd_x_codprov"
        <?php } ?>
        data-table="dir_remates"
...
<script>
loadjs.ready("fdir_rematesadd", function() {
    var options = { name: "x_codprov", selectId: "fdir_rematesadd_x_codprov" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdir_rematesadd.lists.codprov?.lookupOptions.length) {
        options.data = { id: "x_codprov", form: "fdir_rematesadd" };
    } else {
        options.ajax = { id: "x_codprov", form: "fdir_rematesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_dir_remates_codloc">
    <select
        ...
        data-select2-id="fdir_rematesadd_x_codloc"
        <?php } ?>
        data-table="dir_remates"
        data-field="x_codloc"
...
<script>
loadjs.ready("fdir_rematesadd", function() {
    var options = { name: "x_codloc", selectId: "fdir_rematesadd_x_codloc" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fdir_rematesadd.lists.codloc?.lookupOptions.length) {
        options.data = { id: "x_codloc", form: "fdir_rematesadd" };
    } else {
        options.ajax = { id: "x_codloc", form: "fdir_rematesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fdir_rematesadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fdir_rematesadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
