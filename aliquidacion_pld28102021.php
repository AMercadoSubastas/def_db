...
//$fecha_hoy = $fecha_hoy - 30;
/*
$nro = 2;
$query_ser = "SELECT * FROM series WHERE  codnum ='$nro'";
$ser       = mysqli_query($amercado, $query_ser) or die("ERROR LEYENDO SERIES 1");

//$totalRows_ser = mysqli_num_rows($ser);
...
$nro = 13;
$query_ser = "SELECT * FROM series WHERE  codnum ='$nro'";
$ser       = mysqli_query($amercado, $query_ser) or die("ERROR LEYENDO SERIES 2");
...
if(isset($_POST['remate']) || isset($_GET['remate'])){  
	...
	if ($tipo_de_iva== 1) {
		$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) ,SUM( `totneto21` *'$porcen_iva' ) ,  SUM( (`totneto21`  *'$porcen_iva') + `totneto21`)  as total_21 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND `tcomp`!='52'  AND `tcomp`!='54' AND `tcomp`!='57'  AND `tcomp`!='58'  AND `tcomp`!='59'   AND `tcomp`!='92'   AND `tcomp`!='104' AND `en_liquid` = 1)" ; 
	...
	if ($tipo_de_iva== 1) {
		$res2 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105_2 , SUM( `totneto21` ) , SUM( `totneto21` *'$porcen_iva' ) , SUM( `totneto21` *'$porcen_iva' + `totneto21`)  as total_21_2 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND (`tcomp`='57' OR `tcomp`='58' OR `tcomp`='93') AND `en_liquid` = 1 AND `cliente` != '$clientes')" ;
	...
	if ($tipo_de_iva== 1) {
		$res3 ="SELECT SUM(`detfac`.`neto`) , SUM(`detfac`.`neto` * 0.21), SUM(`detfac`.`neto` * 1.21) FROM `detfac`, `cabfac` WHERE (`detfac`.`codrem` = '$rematenum' AND (`detfac`.`tcomp`='57' OR `detfac`.`tcomp`='58' OR `detfac`.`tcomp`='93') AND `detfac`.`tcomp` = `cabfac`.`tcomp` AND `detfac`.`ncomp` = `cabfac`.`ncomp` AND `cabfac`.`en_liquid` = 1  AND `detfac`.`descrip` LIKE '%TASA%')" ;
...
if ($tipo_de_iva== 1) {
	$query_factura_total     = sprintf("SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 52 || cabfac.tcomp = 54 || cabfac.tcomp = 59 || cabfac.tcomp = 92 || cabfac.tcomp = 104 || cabfac.tcomp = 93) AND cabfac.estado != 'A' AND cabfac.codrem ='$rematenum' AND cabfac.en_liquid = 1 AND cabfac.cliente = %s", $clientes);
    ...
    //if ($rematenum == 3339)
    //    $totfactura = $totfactura + 62920.00;
	$totfactura              = round($totfactura,2);
}
else {
	$query_factura_total     = sprintf("SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 52 || cabfac.tcomp = 54 || cabfac.tcomp = 59 || cabfac.tcomp = 92  || cabfac.tcomp = 104 || cabfac.tcomp = 93)  AND cabfac.estado != 'A' AND cabfac.codrem ='$rematenum' AND cabfac.en_liquid = 1 AND cabfac.cliente = %s", $clientes) ;
	$factura_total           = mysqli_query($amercado, $query_factura_total) or die("ERROR LEYENDO CABFAC 52 , 54, 59 y 60 2");
	$row_factura_total       = mysqli_fetch_assoc($factura_total);
	$totalRows_factura_total = mysqli_num_rows($factura_total);
	$totfactura1             = number_format ($row_factura_total['SUM( cabfac.totbruto)'] , 2 , "," , ".");
	$totfactura              = $row_factura_total['SUM( cabfac.totbruto)'];
...
$serie       = mysqli_query($amercado, $query_serie) or die("ERROR LEYENDO SERIES 36");
