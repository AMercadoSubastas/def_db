...
// Inicio el pdf con los datos de cabecera
	...
	$pdf->SetMargins(0.5, 0.5 , 0.5);
  	...
  	$pdf->SetY(15);
  	$pdf->Cell(25);
  	$pdf->Cell(20,10,'  Impuesto a los dep�sitos bancarios por liquidaci�n desde: '.$fecha_desde.' hasta: '.$fecha_hasta,0,0,'L');
  	$pdf->SetFont('Arial','B',9);
  	$pdf->SetY(25);
  	$pdf->Cell(10);
  	$pdf->Cell(20,16,' Liquidaci�n ',1,0,'L');
	$pdf->Cell(20,16,' Remate ',1,0,'L');
	$pdf->Cell(20,16,' Cliente ',1,0,'L');
	$pdf->Cell(70,16,' Medio de pago ',1,0,'L');
  	$pdf->Cell(30,16,' Importe ',1,0,'L');
	$pdf->Cell(30,16,' Impuesto ',1,0,'L');
  	$pdf->SetY(33);
  	$pdf->Cell(115);
	...
	while($row_entidades = mysqli_fetch_array($enti)) {
			...
			while($row_liquidacion = mysqli_fetch_array($liquidacion)) {
				...
				$pdf->SetY($valor_y);
				$pdf->Cell(10);
				$pdf->Cell(70,8,'LIQUIDACION  '.$nrodocliq.'  REMATE :   '.$remate.' IMPORTE :   '.$total_liq.'  ',0,0,'L');
			...
			if ($tot_chq_terceros != 0.0) {
					...
					$tot_chq_terceros_imp = $tot_chq_terceros * $impchq;
					$tot_chq_terceros_imp = number_format($tot_chq_terceros_imp, 2, ',','.');
					$tot_chq_terceros   = number_format($tot_chq_terceros, 2, ',','.');
					$pdf->Cell(40);
					$pdf->Cell(25,8,$tot_chq_terceros,0,0,'R');
					$pdf->Cell(25,8,$tot_chq_terceros_imp,0,0,'R');
					$valor_y = $valor_y + 6;
					$tot_chq_terceros = 0;
					$tot_chq_terceros_imp = 0;
			}
			...
			if ($tot_depositos != 0.0) { 
					...
					$total     += $tot_depositos;
					$tot_depositos_imp = $tot_depositos * $impchq;
					$tot_depositos_imp      = number_format($tot_depositos_imp, 2, ',','.');
					$tot_depositos      = number_format($tot_depositos, 2, ',','.');
					$pdf->Cell(40);
					$pdf->Cell(25,8,$tot_depositos,0,0,'R');
					$pdf->Cell(25,8,$tot_depositos_imp,0,0,'R');
					$valor_y = $valor_y + 6;
					$tot_depositos = 0;
					$tot_depositos_imp = 0;
			}	
			...
			if ($tot_efvo != 0.0) {
					...
					$total_cli += $tot_efvo;
					$total     += $tot_efvo;
					$tot_efvo_imp = $tot_efvo * $impchq;
					$tot_efvo_imp       = number_format($tot_efvo_imp, 2, ',','.');
					$tot_efvo           = number_format($tot_efvo, 2, ',','.');
					$pdf->Cell(40);
					$pdf->Cell(25,8,$tot_efvo,0,0,'R');
					$pdf->Cell(25,8,$tot_efvo_imp,0,0,'R');
					$valor_y = $valor_y + 6;
					$tot_efvo = 0;
					$tot_efvo_imp = 0;
			}
			...
			if ($tot_chq_propio != 0.0) {
					...
					$tot_chq_propio_imp = $tot_chq_propio * $impchq;
					$tot_chq_propio   = number_format($tot_chq_propio, 2, ',','.');
					$tot_chq_propio_imp   = number_format($tot_chq_propio_imp, 2, ',','.');
					$pdf->Cell(40);
					$pdf->Cell(25,8,$tot_chq_propio,0,0,'R');
					$pdf->Cell(25,8,$tot_chq_propio_imp,0,0,'R');
					$valor_y = $valor_y + 6;
					$tot_chq_propio = 0;
					$tot_chq_propio_imp = 0;
			}
	...
	if ($cant_reng > 28) {
			...
			$pdf->Cell(60,8,'TOTAL LIQUIDACIONES: ',0,0,'R');
	...
	if ($cant_reng > 10) {
			...
			$tot_gral_chq_terceros_imp = $tot_gral_chq_terceros * $impchq;
			$total_imp += $tot_gral_chq_terceros_imp;
			$tot_gral_chq_terceros   = number_format($tot_gral_chq_terceros, 2, ',','.');
			$tot_gral_chq_terceros_imp   = number_format($tot_gral_chq_terceros_imp, 2, ',','.');
			$pdf->Cell(60,8,'TOTAL CHEQUES 3ROS: ',0,0,'R');
			$pdf->Cell(35,8,$tot_gral_chq_terceros,0,0,'R');
			$pdf->Cell(35,8,$tot_gral_chq_terceros_imp,0,0,'R');
			$valor_y = $valor_y + 6;
			$pdf->SetY($valor_y);
			$pdf->Cell(60);

			$tot_gral_chq_propio_imp = $tot_gral_chq_propio * $impchq;
			$total_imp += $tot_gral_chq_propio_imp;
			$tot_gral_chq_propio   = number_format($tot_gral_chq_propio, 2, ',','.');
			$tot_gral_chq_propio_imp   = number_format($tot_gral_chq_propio_imp, 2, ',','.');
			$pdf->Cell(60,8,'TOTAL CHEQUES PROPIOS: ',0,0,'R');
			$pdf->Cell(35,8,$tot_gral_chq_propio,0,0,'R');
			$pdf->Cell(35,8,$tot_gral_chq_propio_imp,0,0,'R');
			

			$valor_y = $valor_y + 6;
			$pdf->SetY($valor_y);
			$pdf->Cell(60);
			$tot_gral_depositos_imp = $tot_gral_depositos * $impchq;
			$total_imp += $tot_gral_depositos_imp;
			$tot_gral_depositos   = number_format($tot_gral_depositos, 2, ',','.');
			$tot_gral_depositos_imp   = number_format($tot_gral_depositos_imp, 2, ',','.');
			$pdf->Cell(60,8,'TOTAL DEPOSITOS: ',0,0,'R');
			$pdf->Cell(35,8,$tot_gral_depositos,0,0,'R');
			$pdf->Cell(35,8,$tot_gral_depositos_imp,0,0,'R');
			
			$valor_y = $valor_y + 6;
			$pdf->SetY($valor_y);
			$pdf->Cell(60);
			$tot_gral_efvo_imp = $tot_gral_efvo * $impchq;
			$total_imp += $tot_gral_efvo_imp;
			$tot_gral_efvo   = number_format($tot_gral_efvo, 2, ',','.');
			$tot_gral_efvo_imp   = number_format($tot_gral_efvo_imp, 2, ',','.');
			$pdf->Cell(60,8,'TOTAL EFECTIVO: ',0,0,'R');
			$pdf->Cell(35,8,$tot_gral_efvo,0,0,'R');
			$pdf->Cell(35,8,$tot_gral_efvo_imp,0,0,'R');
			
			$valor_y = $valor_y + 6;
			$pdf->SetY($valor_y);
			$pdf->Cell(60);
			$tot_gral_dolares   = number_format($tot_gral_dolares, 2, ',','.');
			$pdf->Cell(60,8,'TOTAL DOLARES: ',0,0,'R');
			...
			$pdf->Cell(60,8,'TOTAL RETENCIONES: ',0,0,'R');
			...
			$pdf->Cell(60,8,'TOTAL FACTURAS DE GASTOS: ',0,0,'R');
			$pdf->Cell(35,8,$tot_gral_facgstos,0,0,'R');
		
			$valor_y = $valor_y + 6;
			$pdf->SetY($valor_y);
			$pdf->Cell(60);
			$total_imp   = number_format($total_imp, 2, ',','.');
			$pdf->Cell(60,8,'TOTAL IMPUESTO: ',0,0,'R');
			$pdf->Cell(70,8,$total_imp,0,0,'R');
