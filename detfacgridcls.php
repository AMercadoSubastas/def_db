<?php include_once "detfacinfo.php" ?>
...
$detfac_grid = NULL; // Initialize page object first

class cdetfac_grid extends cdetfac {
	...
	// Table name
	var $TableName = 'detfac';

	// Page object name
	var $PageObjName = 'detfac_grid';

	// Grid form hidden field names
	var $FormName = 'fdetfacgrid';
	...
	function __construct() {
		...
		// Table object (detfac)
		if (!isset($GLOBALS["detfac"]) || get_class($GLOBALS["detfac"]) == "cdetfac") {
			$GLOBALS["detfac"] = &$this;

//			$GLOBALS["MasterTable"] = &$GLOBALS["Table"];
//			if (!isset($GLOBALS["Table"])) $GLOBALS["Table"] = &$GLOBALS["detfac"];
	...
	function Page_Terminate($url = "") {
		...
		// Export
		global $EW_EXPORT, $detfac;
		if ($this->CustomExport <> "" && $this->CustomExport == $this->Export && array_key_exists($this->CustomExport, $EW_EXPORT)) {
				$sContent = ob_get_contents();
			if ($gsExportFile == "") $gsExportFile = $this->TableVar;
			$class = $EW_EXPORT[$this->CustomExport];
			if (class_exists($class)) {
				$doc = new $class($detfac);
				$doc->Text = $sContent;
...
//		$GLOBALS["Table"] = &$GLOBALS["MasterTable"];
	...
	function RenderRow() {
		...
		if ($this->RowType == EW_ROWTYPE_VIEW) { // View row
			...
			$this->concafac->EditValue = ew_HtmlEncode($this->concafac->CurrentValue);
			$this->concafac->PlaceHolder = ew_RemoveHtml($this->concafac->FldCaption());
	...
	function ValidateForm() {
		...
		if (!ew_CheckInteger($this->concafac->FormValue)) {
			ew_AddMessage($gsFormError, $this->concafac->FldErrMsg());
		}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$sMasterFilter = $this->SqlMasterFilter_cabfac();
			...
			if ($bValidMasterRecord) {
				$rsmaster = $GLOBALS["cabfac"]->LoadRs($sMasterFilter);
				$bValidMasterRecord = ($rsmaster && !$rsmaster->EOF);
				$rsmaster->Close();
			}
			if (!$bValidMasterRecord) {
				$sRelatedRecordMsg = str_replace("%t", "cabfac", $Language->Phrase("RelatedRecordRequired"));
