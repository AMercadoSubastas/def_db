...
if(isset($_POST['remate']) || isset($_GET['remate'])){  
	...
	if ($tipo_de_iva== 1) {
		$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) , SUM( `totneto21` *'$procen_iva' ) , SUM( `totneto21` *'$procen_iva' + `totneto21`)  as total_21 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND `tcomp`!='52'  AND `tcomp`!='57'  AND `tcomp`!='58' AND `tcomp`!='59'  AND `tcomp`!='60' AND `en_liquid` = 1 )" ;
		$query_remate = mysqli_query($amercado, $res1) or die(mysqli_error($amercado));
		$totalRows_remate = mysqli_num_rows($query_remate); 
	}
	else {
		$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) , SUM( `totneto21` *'$procen_iva' ) , SUM( `totneto21` *'$procen_iva' + `totneto21`)  as total_21 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND `tcomp`!='54' AND `tcomp`!='57'  AND `tcomp`!='58' AND `tcomp`!='59'  AND `tcomp`!='60' AND `en_liquid` = 1 )" ;
		$query_remate = mysqli_query($amercado, $res1) or die(mysqli_error($amercado));
		$totalRows_remate = mysqli_num_rows($query_remate); 
