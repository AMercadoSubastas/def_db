	...
	if ($remate!="" && $remate != 0) {
		while($row_detallefac = mysqli_fetch_array($detallefac)) {
			...
			if ($ptcomp == 116 || $ptcomp == 126) {
				...
				$importe  = number_format($neto, 2, ',','.');//number_format(($row_detallefac["neto"] * (1 + ($row_detallefac["porciva"] / 100.0))), 2, ',','.');
				$df_neto  = number_format($neto, 2, ',','.');//number_format(($row_detallefac["neto"] * (1 + ($row_detallefac["porciva"] / 100.0))), 2, ',','.');
				$df_importe    = $df_importe.$importe."\n";
			}
			else {
				$neto          	= $row_detallefac["neto"] * (1 + ($row_detallefac["porciva"] / 100.0));
				$importe  = number_format($neto, 2, ',','.');//($row_detallefac["neto"] * (1 + ($row_detallefac["porciva"] / 100.0)));
		...
		while($row_detallefac1 = mysqli_fetch_array($detallefac1)) {
			...
			$importe  = $neto;//($row_detallefac1["neto"] * (1 + ($row_detallefac1["porciva"] / 100.0)));
			$importe  = number_format($importe, 2, ',','.');
			$df_neto  = number_format($neto, 2, ',','.');//number_format(($row_detallefac1["neto"] * (1 + ($row_detallefac1["porciva"] / 100.0))), 2, ',','.');
			$df_importe    	= $df_importe.$importe."\n";
...
if ($ptcomp == 116) {
	if (isset($df_descrip4)) {
        ...
        $pdf->SetY($j+9);
		$pdf->SetX(27);
		$pdf->MultiCell(150,6,$df_descrip4,0,'L');
		$pdf->SetY($j);
        $df_descrip1 = "";
		$df_descrip2 = "";
        $df_descrip3 = "";
	...
	$pdf->SetY(-58); // $pdf->SetY(-25);
   	...
   	if ($pserie !=53 ) {	
		...
		// FActura A
   		...
   		$pdf->Cell(0,8,'  Neto 21 %                   Neto 10,5 %               Uso Plataforma                  IVA 21 %                         IVA 10,5 %             Gs. Adm.                Importe Total',0,0,'L');
   		$pdf->SetY(-30);
		$pdf->SetFont('Arial','B',10);
		...
		//$pdf->Cell(0,8,'                                         21 %                                                         10,5 %                                                 10%                                                    ',0,0,'L');
   	 	$pdf->Cell(0,8,'   Neto 21 %                Neto 10,5 %               Uso Plataforma                                                                                            Gs. Adm.             Importe Total',0,0,'L');
		$pdf->SetY(-50);
		$pdf->SetFont('Arial','B',10);
