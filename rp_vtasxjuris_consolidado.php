...
$tot_ac_prov_subastas     = 0.00;
$tot_ac_comis_subastas    = 0.00;
$tot_ac_serv_subastas     = 0.00;
$tot_ac_public_subastas   = 0.00;
$tot_ac_lotes21_subastas  = 0.00;
$tot_ac_lotes105_subastas = 0.00;

$tot_ac_prov_inmob     = 0.00;
$tot_ac_comis_inmob    = 0.00;
$tot_ac_serv_inmob     = 0.00;
$tot_ac_public_inmob   = 0.00;
$tot_ac_lotes21_inmob  = 0.00;
$tot_ac_lotes105_inmob = 0.00;
...
$descrip_prov=array();
for ($j=1;$j < 30; $j++) {
	$query_prov = sprintf("SELECT * FROM provincias WHERE codpais = 1 and  codnum = %s", $j);
...
$query_cabfac = sprintf("SELECT * FROM cabfac WHERE (tcomp BETWEEN 51 AND 64 OR tcomp IN (86,89,92,93,94,103,104,105,111,112,113)) AND fecreg BETWEEN %s AND %s AND usuario not in (25,26) ORDER BY fecreg, nrodoc, tcomp", $fecha_desde, $fecha_hasta);
$cabecerafac = mysqli_query($amercado, $query_cabfac) or die("ERROR LEYENDO CABFAC 68 ".$query_cabfac." ");
...
$pdf->SetY(15);
$pdf->Cell(130);
$pdf->Cell(20,10,' Ventas por jurisdicci�n Subastas',0,0,'L');
...
{	
	...
	//if ($tcomp !=  51 && $tcomp !=  52 && $tcomp !=  53 && $tcomp !=  54 && $tcomp !=  55 && $tcomp !=  56  &&  $tcomp != 57 && $tcomp != 58 && $tcomp != 59 && $tcomp != 60 && $tcomp !=  61 && $tcomp !=  62 && $tcomp !=  63 && $tcomp !=  64 && $tcomp !=  86 && $tcomp !=  89 && $tcomp !=  92  && $tcomp !=  93 && $tcomp !=  94 && $tcomp !=  103 && $tcomp !=  104 && $tcomp !=  105)
		//continue;
	...
	if ($i <= 22) {
		if ($codrem != "" && $codrem > 0) {
			...
			if (mysqli_num_rows($dir_exhib) > 0) {
				...
				if ($tcomp == 51 || $tcomp == 53 || $tcomp == 89 || $tcomp == 111) {
        ...
        //echo "CODPROV = ".$prov_remate." (prov_remate) | ";
		// Leo la descripci�n de la provincia
		$query_provincias = sprintf("SELECT * FROM provincias WHERE codpais = 1 AND codnum = $prov_remate");
        ...
        //echo "DESCPROV = ".$desc_prov." (desc_prov) || ";
		$tot_comision = 0.00;
		$tot_public   = 0.00;
		$tot_servicios= 0.00;
		$tot_neto21 = 0.00;
		...
		switch ($tcomp) {
            ...
            case 112:
                ...
                while ($row_detfac = mysqli_fetch_array($detfac)) {
                    if ($row_detfac["concafac"] == 24 || $row_detfac["concafac"] == 2 || $row_detfac["concafac"] == 4 || $row_detfac["concafac"] == 3 || $row_detfac["concafac"] == 28)
			...
			case 55:
				...
				$detfac1 = mysqli_query($amercado, $query_detfac1) or die("ERROR LEYENDO DETFAC ".$query_detfac1." ");
				...
				$detfac2 = mysqli_query($amercado, $query_detfac2) or die("ERROR LEYENDO DETFAC ".$query_detfac2." ");
				...
				$detfac3 = mysqli_query($amercado, $query_detfac3) or die("ERROR LEYENDO DETFAC ".$query_detfac3." ");
            ...
            case 113:
                ...
                while ($row_detfac = mysqli_fetch_array($detfac)) {
                    if ($row_detfac["concafac"] == 20 || $row_detfac["concafac"] == 28 || $row_detfac["concafac"] == 33 || $row_detfac["concafac"] == 41 || $row_detfac["concafac"] == 43)
                        $tot_public   += $row_detfac["neto"];
                    if ($row_detfac["concafac"] == 14)
				        $lotes21      += $row_detfac["neto"];
                    ...
                    if ($row_detfac["concafac"] == 19 || $row_detfac["concafac"] == 36)
                        $tot_comision += $row_detfac["neto"];
                    if ($row_detfac["concafac"] == 18 || $row_detfac["concafac"] == 21 || $row_detfac["concafac"] == 25 || $row_detfac["concafac"] == 32 || $row_detfac["concafac"] == 42)
				        $tot_servicios += $row_detfac["neto"];
                }
                $cliente      = $row_cabecerafac["cliente"];
                $tot_iva21    = $row_cabecerafac["totiva21"];
			...
			case 60:
                ...
                $tot_servicios= 0.00;
				 ...
				 while ($row_detfac = mysqli_fetch_array($detfac)) {
                     if ($row_detfac["concafac"] == 2 || $row_detfac["concafac"] == 4 || $row_detfac["concafac"] == 12) 
                         $tot_public   += $row_detfac["neto"];
...
//echo "ANTES DE IMPRIMIR CODPROV 1= ".$prov_remate." (prov_remate) | ";
//echo "ANTES DE IMPRIMIR DESCPROV 1= ".$desc_prov." (desc_prov) || ";
        ...
        //echo "CODPROV 2 = ".$prov_remate." (prov_remate) | ";
		// Leo la descripci�n de la provincia
		$query_provincias = sprintf("SELECT * FROM provincias WHERE codpais = 1 AND codnum = $prov_remate");
	...
	switch ($tcomp) {
        ...
        $nro_cliente   = $row_entidades['numero'];
        $cuit_cliente  = $row_entidades['cuit'];
//echo "ANTES DE IMPRIMIR CODPROV 2= ".$prov_remate." (prov_remate) | ";
//echo "ANTES DE IMPRIMIR DESCPROV 2= ".$desc_prov." (desc_prov) || ";
        // Imprimo los renglones
        $pdf->SetY($valor_y);
        $pdf->Cell(1);
        $pdf->Cell(19,6,$fecha,0,0,'L');
...
$tot_ac_prov_subastas     = $tot_ac_prov;
$tot_ac_comis_subastas    = $tot_ac_comis;
$tot_ac_serv_subastas     = $tot_ac_serv ;
$tot_ac_public_subastas   = $tot_ac_public ;
$tot_ac_lotes21_subastas  = $tot_ac_lotes21;
$tot_ac_lotes105_subastas = $tot_ac_lotes105;
...
// ACA COMIENZA LA PARTE DE INMOBILIARIA
$acum_prov=array();
$acum_public=array();
$acum_serv=array();
$acum_comis=array();
...
$query_cabfac = sprintf("SELECT * FROM cabfac WHERE (tcomp BETWEEN 51 AND 64 OR tcomp IN (86,89,92,93,94,103,104,105,111,112,113)) AND fecreg BETWEEN %s AND %s AND usuario in (25,26) ORDER BY fecreg, nrodoc, tcomp", $fecha_desde, $fecha_hasta);
$cabecerafac = mysqli_query($amercado, $query_cabfac) or die("ERROR LEYENDO CABFAC 1306 ");
...
$pdf->SetY(15);
$pdf->Cell(130);
$pdf->Cell(20,10,' Ventas por jurisdicci�n Inmobiliaria',0,0,'L');
$pdf->SetFont('Arial','B',9);
$pdf->SetY(25);
$pdf->Cell(3);
$pdf->Cell(20,16,'    Fecha',1,0,'L');
...
$tot_ac_prov_inmob     = $tot_ac_prov;
$tot_ac_comis_inmob    = $tot_ac_comis;
$tot_ac_serv_inmob     = $tot_ac_serv ;
$tot_ac_public_inmob   = $tot_ac_public ;
$tot_ac_lotes21_inmob  = $tot_ac_lotes21;
$tot_ac_lotes105_inmob = $tot_ac_lotes105;
...
// CALCULO LOS PORCENTAJES DE CADA DIVISION
$total_subastas = $tot_ac_comis_subastas + $tot_ac_serv_subastas + $tot_ac_public_subastas;
$total_inmob = $tot_ac_comis_inmob + $tot_ac_serv_inmob + $tot_ac_public_inmob;
$total_general = $total_subastas + $total_inmob;

$porc_subastas = $total_subastas / $total_general * 100.0;
$porc_inmob = $total_inmob / $total_general * 100.0;
...
$pdf->SetY($valor_y);
$pdf->Cell(10);
$pdf->Cell(30,6,"TOTAL SUBASTAS    ",0,0,'L');
$pdf->SetX(72);
$pdf->Cell(26,6,$total_subastas,0,0,'R');
$pdf->SetX(122);
$pdf->Cell(26,6,$porc_subastas." % ",0,0,'R');

$valor_y = $valor_y + 6;

$pdf->SetY($valor_y);
$pdf->Cell(10);
$pdf->Cell(30,6,"TOTAL INMOBILIARIA ",0,0,'L');
$pdf->SetX(72);
$pdf->Cell(26,6,$total_inmob,0,0,'R');
$pdf->SetX(122);
$pdf->Cell(26,6,$porc_inmob." % ",0,0,'R');

$valor_y = $valor_y + 6;

$pdf->SetY($valor_y);
$pdf->Cell(10);
$pdf->Cell(30,6,"TOTAL GENERAL ",0,0,'L');
$pdf->SetX(72);
$pdf->Cell(26,6,$total_general,0,0,'R');
$pdf->SetX(122);
$pdf->Cell(26,6,"100,00 % ",0,0,'R');
