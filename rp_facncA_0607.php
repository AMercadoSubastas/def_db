...
mysqli_select_db($amercado, $database_amercado);
	...
	if ($remate!="" && $remate!=0 && $ptcomp != 57 && $ptcomp != 59 && $ptcomp != 61 && $ptcomp != 63 ) {
		...
		while($row_detallefac = mysqli_fetch_array($detallefac)) {
			...
			$descrip1      = substr($row_detallefac['descrip'],0,55);
			
			$descrip2      = substr($row_detallefac['descrip'],55,55);
		...
		while($row_detallefac1 = mysqli_fetch_array($detallefac1)) {
			...
			$descrip1   	= substr($row_detallefac1['descrip'],0,62);
			$df_descrip1   	= $df_descrip1.$descrip1."\n";
			$descrip2   	= substr($row_detallefac1['descrip'],63,62);
			$df_descrip2   	= $df_descrip2.$descrip2."\n";
			$descrip3   	= substr($row_detallefac1['descrip'],125,62);
	...
	$pdf->SetY(-38); // $pdf->SetY(-25);
   	...
   	if ($pserie==29 || $pserie == 31) {	
		...
		// FActura A
    	if ($ptcomp == 59)
   		$pdf->Cell(0,8,' Neto 0 %                         Neto 10,5 %              Comisi�n 10%                   IVA 21 %                         IVA 10,5 %              RG 3337                       Total',0,0,'L');
		else
