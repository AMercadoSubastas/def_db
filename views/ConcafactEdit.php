...
?>
<main class="edit">
<form name="fconcafactedit" id="fconcafactedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
...
<?php if ($Page->codnum->Visible) { // codnum ?>
    <div id="r_codnum"<?= $Page->codnum->rowAttributes() ?>>
        <label id="elh_concafact_codnum" class="<?= $Page->LeftColumnClass ?>"><?= $Page->codnum->caption() ?><?= $Page->codnum->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
        <div class="<?= $Page->RightColumnClass ?>"><div<?= $Page->codnum->cellAttributes() ?>>
<span id="el_concafact_codnum">
...
<input type="hidden" data-table="concafact" data-field="x_codnum" data-hidden="1" name="x_codnum" id="x_codnum" value="<?= HtmlEncode($Page->codnum->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<script>
loadjs.ready(["fconcafactedit", "datetimepicker"], function () {
    ...
    ew.createDateTimePicker("fconcafactedit", "x_fechahora", ew.deepAssign({"useCurrent":false,"display":{"sideBySide":false}}, options));
});
</script>
<?php } ?>
</span>
...
<span id="el_concafact_activo">
    <select
        ...
        data-select2-id="fconcafactedit_x_activo"
        <?php } ?>
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fconcafactedit", function() {
    var options = { name: "x_activo", selectId: "fconcafactedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fconcafactedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fconcafactedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fconcafactedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<span id="el_concafact_tipoiva">
    <select
        ...
        data-select2-id="fconcafactedit_x_tipoiva"
        <?php } ?>
        data-table="concafact"
...
<script>
loadjs.ready("fconcafactedit", function() {
    var options = { name: "x_tipoiva", selectId: "fconcafactedit_x_tipoiva" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fconcafactedit.lists.tipoiva?.lookupOptions.length) {
        options.data = { id: "x_tipoiva", form: "fconcafactedit" };
    } else {
        options.ajax = { id: "x_tipoiva", form: "fconcafactedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_concafact_impuesto">
    <select
        ...
        data-select2-id="fconcafactedit_x_impuesto"
        <?php } ?>
        data-table="concafact"
...
<script>
loadjs.ready("fconcafactedit", function() {
    var options = { name: "x_impuesto", selectId: "fconcafactedit_x_impuesto" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fconcafactedit.lists.impuesto?.lookupOptions.length) {
        options.data = { id: "x_impuesto", form: "fconcafactedit" };
    } else {
        options.ajax = { id: "x_impuesto", form: "fconcafactedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<span id="el_concafact_tieneresol">
    <select
        ...
        data-select2-id="fconcafactedit_x_tieneresol"
        <?php } ?>
        data-table="concafact"
...
<script>
loadjs.ready("fconcafactedit", function() {
    var options = { name: "x_tieneresol", selectId: "fconcafactedit_x_tieneresol" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fconcafactedit.lists.tieneresol?.lookupOptions.length) {
        options.data = { id: "x_tieneresol", form: "fconcafactedit" };
    } else {
        options.ajax = { id: "x_tieneresol", form: "fconcafactedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fconcafactedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fconcafactedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
