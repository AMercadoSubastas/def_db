...
mysqli_select_db($amercado, $database_amercado);
	...
	if ($remate!="" && $remate != 0) {
		while($row_detallefac = mysqli_fetch_array($detallefac)) {
			...
			if ($lote_num=="" ) {
				$descrip1      = substr($row_detallefac['descrip'],0,73);
	...
	if ($ptcomp == 56 || $ptcomp == 62) {
		...
		$pdf->SetY(15);
		$pdf->Cell(130);
		$pdf->Image('images/logo_adrianC.jpg',10,8,85);
	}
	$pdf->SetFont('Arial','B',10);
	$pdf->SetY(25);
	$pdf->SetX(15);
	...
	$pdf->Image('images/tipo_b.gif',100,8);
	...
	$pdf->SetY(-38); // $pdf->SetY(-25);
   	...
   	if ($pserie==29 || $pserie == 31) {	
		...
		$total = $tot_neto21 + $tot_neto105 + $tot_comision+$tot_iva21+$tot_iva105;
		if ($tot_neto21 != 0)
			$tot_neto21   = $tot_neto21 + $tot_iva21;
		...
		//$pdf->Cell(0,8,'                                         21 %                                                         10,5 %                                                 10%                                                    ',0,0,'L');
   	 	$pdf->Cell(0,8,' Neto 21 %                         Neto 10,5 %              Comisi�n 10%                                                                                                                  Total',0,0,'L');
