...
/**
 * Table class for ajax_client
 */
class AjaxClient extends DbTable
{
    protected $SqlFrom = "";
    ...
    public function __construct()
    {
        ...
        $this->TableVar = "ajax_client";
        $this->TableName = 'ajax_client';
        $this->TableType = "TABLE";
        $this->ImportUseTransaction = $this->supportsTransaction() && Config("IMPORT_USE_TRANSACTION");
        $this->UseTransaction = $this->supportsTransaction() && Config("USE_TRANSACTION");

        // Update Table
        $this->UpdateTable = "ajax_client";
        $this->Dbid = 'DB';
        $this->ExportAll = true;
        ...
        $this->clientID->SearchOperators = ["=", "<>", "IN", "NOT IN", "<", "<=", ">", ">=", "BETWEEN", "NOT BETWEEN"];
        ...
        $this->zipCode = new DbField(
            ...
            'zipCode', // Name
            '`zipCode`', // Expression
            '`zipCode`', // Basic search expression
            200, // Type
            15, // Size
            -1, // Date/Time format
            false, // Is upload field
            '`zipCode`', // Virtual expression
            false, // Is virtual
            false, // Force selection
            false, // Is Virtual search
            'FORMATTED TEXT', // View Tag
    ...
    {
        return ($this->SqlFrom != "") ? $this->SqlFrom : "ajax_client";
    }
    ...
    {
        $keys = [];
        $val = $current ? $this->clientID->CurrentValue : $this->clientID->OldValue;
        if (EmptyValue($val)) {
            return "";
        } else {
            $keys[] = $val;
    ...
    public function setKey($key, $current = false, $keySeparator = null)
    {
        ...
        if (count($keys) == 1) {
            if ($current) {
                $this->clientID->CurrentValue = $keys[0];
            } else {
                $this->clientID->OldValue = $keys[0];
            }
        }
    }
    ...
    public function getRecordFilter($row = null, $current = false)
    {
        ...
        if (is_array($row)) {
            ...
            $val = !EmptyValue($this->clientID->OldValue) && !$current ? $this->clientID->OldValue : $this->clientID->CurrentValue;
        }
        if (!is_numeric($val)) {
    ...
    public function getReturnUrl()
    {
        ...
        return $_SESSION[$name] ?? GetUrl("AjaxClientList");
    }
    ...
    public function getModalCaption($pageName)
    {
        ...
        return match ($pageName) {
            "AjaxClientView" => $Language->phrase("View"),
            "AjaxClientEdit" => $Language->phrase("Edit"),
            "AjaxClientAdd" => $Language->phrase("Add"),
            default => ""
        };
    }
    ...
    public function getApiPageName($action)
    {
        return match (strtolower($action)) {
            Config("API_VIEW_ACTION") => "AjaxClientView",
            Config("API_ADD_ACTION") => "AjaxClientAdd",
            Config("API_EDIT_ACTION") => "AjaxClientEdit",
            Config("API_DELETE_ACTION") => "AjaxClientDelete",
            Config("API_LIST_ACTION") => "AjaxClientList",
            default => ""
        };
    }
    ...
    {
        if ($parm != "") {
            $url = $this->keyUrl("AjaxClientView", $parm);
        } else {
            $url = $this->keyUrl("AjaxClientView", Config("TABLE_SHOW_DETAIL") . "=");
        }
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AjaxClientEdit", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AjaxClientList", "action=edit");
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AjaxClientAdd", $parm);
        return $this->addMasterUrl($url);
    }
    ...
    {
        $url = $this->keyUrl("AjaxClientList", "action=copy");
        return $this->addMasterUrl($url);
    }
    ...
    public function getDeleteUrl($parm = "")
    {
        if ($this->UseAjaxActions && ConvertToBool(Param("infinitescroll")) && CurrentPageID() == "list") {
            ...
            return $this->keyUrl("AjaxClientDelete", $parm);
        }
    }
    ...
    {
        $json = "";
        $json .= "\"clientID\":" . VarToJson($this->clientID->CurrentValue, "number");
        $json = "{" . $json . "}";
        if ($htmlEncode) {
    ...
    public function keyUrl($url, $parm = "")
    {
        if ($this->clientID->CurrentValue !== null) {
            $url .= "/" . $this->encodeKeyValue($this->clientID->CurrentValue);
        } else {
    ...
    public function getRecordKeys()
    {
        ...
        if (Param("key_m") !== null) {
            ...
            $keyValues = $isApi
                ...
                : []; // Non-API
            if (($keyValue = Param("clientID") ?? Route("clientID")) !== null) {
                $arKeys[] = $keyValue;
    ...
    public function getFilterFromRecordKeys($setCurrent = true)
    {
        ...
        foreach ($arKeys as $key) {
            ...
            if ($setCurrent) {
                $this->clientID->CurrentValue = $key;
            } else {
                $this->clientID->OldValue = $key;
            }
    ...
    public function renderListRow()
    {
        ...
        $this->clientID->ViewValue = FormatNumber($this->clientID->ViewValue, $this->clientID->formatPattern());
