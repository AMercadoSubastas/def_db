...
$Page->showMessage();
?>
<form name="fpaisesadd" id="fpaisesadd" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
...
<?php if ($Page->activo->Visible) { // activo ?>
    <div id="r_activo"<?= $Page->activo->rowAttributes() ?>>
        <label id="elh_paises_activo" for="x_activo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->activo->caption() ?><?= $Page->activo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<span id="el_paises_activo">
    <select
        ...
        data-select2-id="fpaisesadd_x_activo"
        <?php } ?>
        data-table="paises"
        data-field="x_activo"
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fpaisesadd", function() {
    var options = { name: "x_activo", selectId: "fpaisesadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fpaisesadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fpaisesadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fpaisesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.paises.fields.activo.selectOptions);
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fpaisesadd"><?= $Language->phrase("AddBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fpaisesadd" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
