...
$row_ser = mysqli_fetch_array($ser); 
$fechatope_ser = $row_ser['fechatope'];
$nrotope_ser = $row_ser['nrohasta'];
$nroact_ser = $row_ser['nroact'];
if ($fechatope_ser < $fecha_hoy) {
    echo "EL CAI DEL FORMULARIO DE LIQUIDACION A0001 VENCIO EL ".$fechatope_ser."  ";
    exit;
}
if ($nrotope_ser - 20 <= ($nroact_ser)) {
    echo "EL TALONARIO DE LIQUIDACION A0001 TIENE MENOS DE 20 FORMULARIOS  ";
...
$nroact_ser = $row_ser['nroact'];
if ($fechatope_ser < $fecha_hoy) {
    echo "EL CAI DEL FORMULARIO DE LIQUIDACION B0001 VENCIO EL ".$fechatope_ser."  ";
    
}
if ($nrotope_ser-20 <= ($nroact_ser)) {
    echo "EL TALONARIO DE LIQUIDACION B0001 TIENE MENOS DE 20 FORMULARIOS  ";
...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "liquidacion")) {
    ...
    */
	...
	$actualizafac = sprintf("UPDATE cabfac SET tcompsal=%s , seriesal=%s  , ncompsal = %s  WHERE codrem=%s and estado = %s",
                      	GetSQLValueString($_POST['tcomp'], "int"),
                      	GetSQLValueString($_POST['serie'], "int"),
						GetSQLValueString($_POST['liquidacion'], "int"),
						GetSQLValueString($_POST['remate_num'], "int"),
	...
	$actualizadet = sprintf("UPDATE detfac  SET tcomsal=%s , seriesal=%s  , ncompsal = %s  WHERE (codrem=%s AND concafac IS NULL AND codlote IS NOT NULL)",
                       	GetSQLValueString($_POST['tcomp'], "int"),
                       	GetSQLValueString($_POST['serie'], "int"),
						GetSQLValueString($_POST['liquidacion'], "int"),
						GetSQLValueString($_POST['remate_num'], "int"));
...
if(isset($_POST['remate']) || isset($_GET['remate'])){  
	$res = mysqli_query("select * from remates where ncomp='$rematenum'") or die("ERROR ACTUALIZANDO EL REMATE");
	...
	$iva_21 = mysqli_query("select * from impuestos") or die("ERROR LEYENDO IMPUESTOS");
	$porcen_iva = (mysqli_result($iva_21,0,1)/100) ;
	...
	if ($tipo_de_iva== 1) {
		$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) ,SUM( `totneto21` *'$porcen_iva' ) ,  SUM( (`totneto21`  *'$porcen_iva') + `totneto21`)  as total_21 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND `tcomp`!='52'  AND `tcomp`!='54' AND `tcomp`!='57'  AND `tcomp`!='58'  AND `tcomp`!='59'    AND `tcomp`!='103'  AND `tcomp`!='92'   AND `tcomp`!='104' AND `tcomp`!='86' AND `en_liquid` = 1)" ; 
		
		$query_remate = mysqli_query($amercado, $res1) or die("ERROR LEYENDO CABFAC");
		$totalRows_remate = mysqli_num_rows($query_remate); 
	}
	else {
		$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) ,SUM( `totneto21` *'$porcen_iva' ) ,  SUM( (`totneto21`  *'$porcen_iva') + `totneto21`)  as total_21 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND `tcomp`!='52'  AND `tcomp`!='54' AND `tcomp`!='57'  AND `tcomp`!='58'    AND `tcomp`!='103' AND `tcomp`!='59'   AND `tcomp`!='92'   AND `tcomp`!='104' AND `tcomp`!='86' AND `en_liquid` = 1)" ; 
		
		$query_remate = mysqli_query($amercado, $res1) or die("ERROR LEYENDO CABFAC 2");
		$totalRows_remate = mysqli_num_rows($query_remate); 
	...
	if ($tipo_de_iva== 1) {
		$res2 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105_2 , SUM( `totneto21` ) , SUM( `totneto21` *'$porcen_iva' ) , SUM( `totneto21` *'$porcen_iva' + `totneto21`)  as total_21_2 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND (`tcomp`='57' OR `tcomp`='58' OR `tcomp`='93' OR `tcomp`='113') AND `en_liquid` = 1 AND `cliente` != '$clientes')" ;
		$query_remate2 = mysqli_query($amercado, $res2) or die("ERROR LEYENDO CABFAC 4");
		$totalRows_remate2 = mysqli_num_rows($query_remate2); 
	}
	else {
		$res2 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105_2 , SUM( `totneto21` ) , SUM( `totneto21` *'$porcen_iva' ) , SUM( `totneto21` *'$porcen_iva' + `totneto21`)  as total_21_2 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND (`tcomp`='57' OR`tcomp`='58' OR`tcomp`='93' OR `tcomp`='113')  AND `en_liquid` = 1 AND `cliente` != '$clientes')" ;
		$query_remate2 = mysqli_query($amercado, $res2) or die("ERROR LEYENDO CABFAC 5");
		$totalRows_remate2 = mysqli_num_rows($query_remate2); 
	...
	$totneto21 = mysqli_result($query_remate,0,3) - mysqli_result($query_remate2,0,3);// + mysqli_result($query_remate3,0,0);
	$iva21 = mysqli_result($query_remate,0,4) + mysqli_result($query_remate11,0,4) - mysqli_result($query_remate2,0,4);// + mysqli_result($query_remate3,0,1);
    $netonograv = 0.00;
...
mysqli_select_db($amercado, $database_amercado);
$query_cheques_total = "SELECT SUM( cartvalores.importe) FROM cartvalores WHERE  cartvalores.estado = 'S' AND cartvalores.codrem ='$rematenum' AND cartvalores.tcomp != 90" ;
...
if ($tipo_de_iva== 1) {
	$query_factura_total     = sprintf("SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 52 || cabfac.tcomp = 54 || cabfac.tcomp = 59 || cabfac.tcomp = 92 || cabfac.tcomp = 104 || cabfac.tcomp = 86 || cabfac.tcomp = 112 || cabfac.tcomp = 98) AND cabfac.estado != 'A' AND cabfac.codrem ='$rematenum' AND cabfac.en_liquid = 1 AND cabfac.cliente = %s", $clientes);
	$factura_total           = mysqli_query($amercado, $query_factura_total) or die("ERROR LEYENDO CABFAC 52 , 54, 59 y 60 ".$query_factura_total);
    ...
    if ($rematenum == 3825)
        $totfactura = $totfactura * -1;
	
    //Veo si tiene alguna Nota de Cr�dito
    $query_nc_total     = sprintf("SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 57 || cabfac.tcomp = 58 || cabfac.tcomp = 93 || cabfac.tcomp = 113) AND cabfac.estado != 'A' AND cabfac.codrem ='$rematenum' AND cabfac.en_liquid = 1 AND cabfac.cliente = %s", $clientes);
	$nc_total           = mysqli_query($amercado, $query_nc_total) or die("ERROR LEYENDO CABFAC 57 , 58 , 93 y 113");
	$row_nc_total 	     = mysqli_fetch_assoc($nc_total);
	$totalRows_nc_total = mysqli_num_rows($nc_total);
	$totnc1             = number_format ($row_nc_total['SUM( cabfac.totbruto)'] , 2 , "," , ".");
	...
	$query_factura_total     = sprintf("SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 52 || cabfac.tcomp = 54 || cabfac.tcomp = 59 || cabfac.tcomp = 92 || cabfac.tcomp = 104 || cabfac.tcomp = 86 || cabfac.tcomp = 112 || cabfac.tcomp = 98) AND cabfac.estado != 'A' AND cabfac.codrem ='$rematenum' AND cabfac.en_liquid = 1 AND cabfac.cliente = %s", $clientes);
	$factura_total           = mysqli_query($amercado, $query_factura_total) or die("ERROR LEYENDO CABFAC 52 , 54, 59 y 60");
	$row_factura_total 	     = mysqli_fetch_assoc($factura_total);
	$totalRows_factura_total = mysqli_num_rows($factura_total);
	$totfactura1             = number_format ($row_factura_total['SUM( cabfac.totbruto)'] , 2 , "," , ".");
    ...
    //Veo si tiene alguna Nota de Cr�dito
    $query_nc_total     = sprintf("SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 57 || cabfac.tcomp = 58 || cabfac.tcomp = 93 || cabfac.tcomp = 113) AND cabfac.estado != 'A' AND cabfac.codrem ='$rematenum' AND cabfac.en_liquid = 1 AND cabfac.cliente = %s", $clientes);
	$nc_total           = mysqli_query($amercado,$query_nc_total, $amercado) or die("ERROR LEYENDO CABFAC 57 , 58 , 93, 113");
	$row_nc_total 	     = mysqli_fetch_assoc($nc_total);
	$totalRows_nc_total = mysqli_num_rows($nc_total);
	$totnc1             = number_format ($row_nc_total['SUM( cabfac.totbruto)'] , 2 , "," , ".");
...
$total_general = $total_remate - $totalgastos ; //round(($total_remate-$totalgastos),2);
...
$serie       = mysqli_query($amercado, $query_serie) or die("ERROR LEYENDO SERIES 367");
      ...
      <td><table width="100%" border="1" cellspacing="1" cellpadding="1">
        <tr>
          <td bgcolor="#FFFFFF"><div align="center">
            ...
            <td height="33" colspan="4" align="center" bgcolor="#FFFFFF"><a href="a_liquid.php"><img src="images/salir_but.gif" width="55" height="33" border="0" /></a></td>
          </div></td>
          </tr>
      </table></td>
    </tr>
