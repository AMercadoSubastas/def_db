...
mysqli_query($amercado, $amercado,"SET NAMES latin1;" ) or die(mysqli_error($amercado));
	...
	$pserie = $_GET['fserie'];
	$pncomp = $_GET['fncomp'];
    if ($ptcomp == 51 || $ptcomp == 52) {
        $query_cabf = sprintf("SELECT * FROM cabfac WHERE  tcomp IN (51,52) AND serie = %s AND ncomp = %s", $pserie, $pncomp);
    ...
    if ($ptcomp == 111 || $ptcomp == 112) {
        $query_cabf = sprintf("SELECT * FROM cabfac WHERE  tcomp IN (111,112) AND serie = %s AND ncomp = %s", $pserie, $pncomp);
	...
	$cabeleyenda     = mysqli_query($amercado, $query_leyenda) or die("Err. Line 91, EL CBTE NO EXISTE O TIENE UN ERROR");
	...
	if ($remate!="" && $remate!=0 && $ptcomp != 57 && $ptcomp != 59 && $ptcomp != 61 && $ptcomp != 63  && $ptcomp != 113) {
		//echo " 12  - ";
		while($row_detallefac = mysqli_fetch_array($detallefac, MYSQLI_BOTH)) {
			...
			if ($lote_num=="" ) {
	          ...
	          if ($pncomp == 9743 || $pncomp == 9741 || $pncomp == 9742 || $pncomp == 9744 || $pncomp == 15627) {
                  ...
                  $descrip1      = substr(utf8_decode($row_detallefac['descrip']),0,90);//substr($row_lotes['descor'],0,92);
                  $descrip2      = substr(utf8_decode($row_detallefac['descrip']),90,90);//substr($row_lotes['descor'],92,70);
              }
			}
			
			
			if(($ptcomp == 51 || $ptcomp == 53) && ($pncomp != 15627)) {
				if ($descrip2 == "") {
		...
		while($row_detallefac1 = mysqli_fetch_array($detallefac1, MYSQLI_BOTH)) {
			...
			$descrip1   	= substr(utf8_decode($row_detallefac1['descrip']),0,80);
			$df_descrip1   	= $df_descrip1.$descrip1."\n";
			$descrip2   	= substr(utf8_decode($row_detallefac1['descrip']),80,80);
			$df_descrip2   	= $df_descrip2.$descrip2."\n";
	...
	$pdf->Cell(59);
    $pdf->SetFont('Arial','',10);
	if (isset($tip_ind_cliente) && $pserie != 31) {
    ...
    if ($ptcomp != 111 && $ptcomp != 112 && $ptcomp != 113) {
        ...
        $pdf->SetY(-57); // $pdf->SetY(-25);
        //Arial italic 8
        $pdf->SetFont('Arial','I',6);
        $pdf->SetX(70);
        $pdf->Cell(60,4,"Datos para transferencia bancaria.",0,0,'C');
        $pdf->SetY(-54);
        $pdf->SetX(60);
        $pdf->Cell(85,4,utf8_decode("Banco B.B.V.A. FRANCÉS CBU: 0170123020000000738338. Titular: ADRIAN MERCADO S.A."),0,0,'C');
        $pdf->SetY(-50);
        $pdf->SetX(10);
        ...
        $pdf->SetY(-72);
        $pdf->SetFont('Arial','I',8);
        if ($pserie==29 || $pserie == 31 || $pserie == 49) {	
            $total = $tot_neto21 + $tot_neto105 + $tot_comision + $tot_iva21 + $tot_iva105 + $tot_resol + $tot_neto0;
            $tot_neto21   = number_format($tot_neto21, 2, ',','.');
            $tot_neto105  = number_format($tot_neto105, 2, ',','.');
            $tot_comision = number_format($tot_comision, 2, ',','.');
