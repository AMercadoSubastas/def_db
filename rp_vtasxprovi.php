...
$descrip_prov=array();
for ($j=1;$j < 30; $j++) {
	$query_prov = sprintf("SELECT * FROM provincias ");
...
$query_cabfac = sprintf("SELECT * FROM cabfac WHERE (tcomp BETWEEN 51 AND 64 OR tcomp IN (89,92,93,94,103,104,105,111,112)) AND fecreg BETWEEN %s AND %s ORDER BY fecreg, nrodoc, tcomp", $fecha_desde, $fecha_hasta);
...
{	
    ...
    $usuario    = $row_cabecerafac["usuario"];
	...
	*/
	...
	if ($i <= 22) {
		if ($codrem != "" && $codrem > 0) {
            ...
            echo "PROV_REMATE = ".$prov_remate." PCIA = ".$pcia." _ ";
		if ($prov_remate != $pcia)
            continue;
		// Leo la descripci�n de la provincia
		//echo "PROV_REMATE = ".$prov_remate." CBTE = ".$row_cabecerafac["nrodoc"]." ==== ";
		...
		switch ($tcomp) {
            ...
            case 111:
				...
				$tot_iva21    = ($row_cabecerafac["totneto21"] + $row_cabecerafac["totcomis"] + $row_cabecerafac["totimp"]) * $porc_iva21;
			...
			case 58:
				...
				if (mysqli_num_rows($detfac1) > 0) {
                    ...
                    if (strpos($descri_det, "DEB")=== FALSE && strpos($descri_det, "D�B")=== FALSE) {
                        ...
                        //echo "y va el tercero... tcomp= ".$tcomp." Nro ".$ncomp."  ";
					...
					if ($row_cabefac = mysqli_fetch_assoc($cabefac)) {
						if ($row_cabefac["tcomp"] == 54) {
							...
							//Debo leer los renglones de detfac para ver de que se trata
							$query_detfacfc1 = "SELECT * FROM detfac WHERE tcomp = $tcomp_cabefac AND ncomp = $ncomp_cabefac AND (descrip like '%INMOB%' OR descrip like '%GASTOS%')";
			...
			case 94:
			case 60:
				if (($row_cabecerafac["totiva21"] != 0 || $row_cabecerafac["totiva105"] != 0) && $row_cabecerafac["ncomp"] != 177) {
					...
					$cliente      = $row_cabecerafac["cliente"];
					$lotes21      = 0.00;//$row_cabecerafac["totneto21"] ;
                    $lotes105     = 0.00;//$row_cabecerafac["totneto105"];
					$tot_public   = $row_cabecerafac["totneto21"];
                    $tot_comision = 0.00; //$row_cabecerafac["totcomis"];
                    $tot_iva21    = $row_cabecerafac["totiva21"];;//($row_cabecerafac["totneto21"] + $row_cabecerafac["totcomis"]) * $porc_iva21;
                    $tot_iva105   = 0.00;//$row_cabecerafac["totiva105"];
                    $tot_iva      = $tot_iva21 + $tot_iva105;
                    $tot_resol    = $desc_prov;
                    $total        = $row_cabecerafac["totbruto"];
                    $nroorig      = $row_cabecerafac["nrodoc"];	
          ...
          case 111:
			...
			case 58:
				...
				if (mysqli_num_rows($detfac1) > 0) {
					...
					if (strpos($descri_det, "DEB")=== FALSE && strpos($descri_det, "D�B")=== FALSE) {
                        ...
                        $cabefac = mysqli_query($amercado, $query_cabf) or die("ACA STA LA CAGADA 7".$nrodoc_rel);
                        //echo "NRODOC_REL =".$nrodoc_rel."  ";
                    }
					else {
						$query_cabf = "SELECT * FROM cabfac WHERE  tcomp in (60) AND ncomp = $nrodoc_rel";
					    $cabefac = mysqli_query($amercado, $query_cabf) or die("ACA STA LA CAGADA 8".$nrodoc_rel);
                        //echo "NRODOC_REL =".$nrodoc_rel."  ";
                    }
			...
			case 60:
				if (($row_cabecerafac["totiva21"] != 0 || $row_cabecerafac["totiva105"] != 0) && $row_cabecerafac["ncomp"] != 177) {
                    ...
                    $lotes105     = 0.00;//$row_cabecerafac["totneto105"];
					$tot_public   = $row_cabecerafac["totneto21"];
                    $tot_comision = $row_cabecerafac["totcomis"]; //$row_cabecerafac["totcomis"];
                    $tot_iva21    = 0.00;//($row_cabecerafac["totneto21"] + $row_cabecerafac["totcomis"]) * $porc_iva21;
                    $tot_iva105   = 0.00;//$row_cabecerafac["totiva105"];
                    $tot_iva      = $tot_iva21 + $tot_iva105;
                    $tot_resol    = $desc_prov;
                    $total        = $row_cabecerafac["totbruto"];
