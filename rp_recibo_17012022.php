...
for ($k=0;$k<2;$k++) {
    ...
    $selec_cheques = mysqli_query( $query_cheques) or die("ERROR LEYENDO 103");

    $tot_cheques = "SELECT importe  FROM cartvalores WHERE ncomprel ='$pncomp' AND tcomprel ='2' AND serierel='3' AND tcomp='8' AND serie='6'";
    $result_cheques = mysqli_query( $tot_cheques,$amercado) or die("ERROR LEYENDO 106");
    $cheques_tot = 0 ;
    ...
    $selec_cheques11 = mysqli_query( $query_cheques11) or die("ERROR LEYENDO 105");
    ...
    $result_cheques11 = mysqli_query( $tot_cheques11,$amercado) or die("ERROR LEYENDO 111");
    $cheques_tot11 = 0 ;

    while($row11 = mysqli_fetch_array($result_cheques11)){
    ...
    $selec_rem = mysqli_query( $query_rem,$amercado) or die("ERROR LEYENDO 107 ");

    $tot_rem = "SELECT importe, ncomp  FROM cartvalores WHERE ncomprel ='$pncomp' AND tcomprel ='2' AND serierel='3' AND tcomp='4' ";
    $result_rem = mysqli_query( $tot_rem ,$amercado) or die("ERROR LEYENDO 112");
    $rem_tot = 0 ;
    while($row41 = mysqli_fetch_array($result_rem)){
        $rem_tot0	= $row41['0'];
        $rem_texto = $row41['1'];
    ...
    // EFECTIVO
    $query_efectivo = "SELECT importe, codchq,tcomp  FROM cartvalores WHERE ncomprel ='$pncomp' AND tcomprel ='2' AND serierel='3' AND (tcomp='12' OR tcomp='91' OR tcomp='97')";
	...
	$pdf->SetY(36);
	$pdf->SetX(15);
	$pdf->Cell(150,10,'CABA  (C1107AAV) Tel/Fax: +54 11 3984-7400');
	$pdf->SetY(40);
	$pdf->SetX(15);
    ...
    */
	...
	$pdf->SetY(76);
    $pdf->SetFont('Arial','BI',10);
	$pdf->Cell(20,10,utf8_decode('Recibi(mos) la suma de: '),0,0,'L');	
	$pdf->SetY(80);
	$pdf->SetFont('Arial','',8);
	...
	$pdf->SetY(85);
	$pdf->SetFont('Arial','BI',10);
	if ($pncomp == 260000 || $pncomp == 260001)
    ...
    if ($pncomp == 260001) {
        ...
        $pdf->Cell(60,10,utf8_decode('Depósito en garantía por transferencia:---------------- $ '),0,0,'L');
        $pdf->SetX(80);
        $pdf->Cell(20,10,$efe_nd,0,0,'R');
        $pdf->SetX(15);
        $pdf->SetY($valorY + 5);
        ...
        if ($efe_tot!=0) {
            $efe_tot2  = number_format($efe_tot, 2, ',','.');
            if ($tcompefe == 91) {
                $pdf->Cell(45,10,utf8_decode('Dev.Crédito a Favor:'.$ndeb.'-  $ '),0,0,'L');
                $pdf->SetX(80);
                $pdf->Cell(20,10,$efe_tot2,0,0,'R');	
            }
            else if ($tcompefe == 99) {
                $pdf->Cell(45,10,utf8_decode('Devolución de credito a favor:').$ndeb.'------------------  $ ',0,0,'L');
                $pdf->SetX(80);
                $pdf->Cell(45,10,$efe_tot2,0,0,'R');
            }
            else if ($tcompefe == 97) {
                $pdf->Cell(20,10,'Cancela contra FCPRO:'.$ndeb.'---------------------------  $ ',0,0,'L');
                $pdf->SetX(80);
                $pdf->Cell(20,10, $efe_tot2,0,0,'R');
            }
            else {
            ...
            if ($tcompefe == 91) {
                $pdf->Cell(45,10,utf8_decode('Crédito a Favor:'.$ndeb.'--------------------------  $ '),0,0,'L');
                $pdf->SetX(80);
                $pdf->Cell(20,10,$efe_tot2,0,0,'R');	
            }
            else if ($tcompefe == 97) {
	...
	if (mysqli_num_rows($selec_cheques)!=0) {	
    	...
    	while ($cheques = mysqli_fetch_array($selec_cheques)){
			...
			$importe_cheque2= number_format($importe_cheque, 2, ',','.');
      		$str_cheques = $nombre_bancos.utf8_decode("- Nº ").$codchq."- $ ".$importe_cheque2." - Fecha: ".$fechachq." - ".$str_cheques ;
            $lista_chq[$j] = $nombre_bancos.utf8_decode("- Nº ").$codchq." - $ ".$importe_cheque2." - Fecha: ".$fechachq." - " ;
            //$str_cheques = "";
            $j++;
	 	} 
	    //echo $str_cheques;
        ...
        //echo (strlen($str_cheques));
  		if ($j == 1) {
    		...
    		$y = $y+4 ;
   			$pdf->SetY($y);
			if ($fact_tcomp[0] == 51 || $fact_tcomp[0] == 53)
	   			$pdf->Cell(20,10,"Total cheques: $ ".$total_cheques.utf8_decode(" Retira contra acreditación de cheques"),0,0,'L');
			else
 	...
 	if (mysqli_num_rows($selec_cheques11)!=0) {	  
    	...
    	while ($cheques11 = mysqli_fetch_array($selec_cheques11)){
    	 	...
    	 	if ( $cheques_texto11 == 1) {
	   		...
	   		$query_de_bancos = "SELECT *  FROM bancos WHERE codnum ='$codban'";
	        $selecciona_bancos = mysqli_query( $query_de_bancos,$amercado) or die("ERROR LEYENDO 1838");
	   		$row_bancos = mysqli_fetch_assoc($selecciona_bancos);
			...
			$importe_cheque112= number_format($importe_cheque11, 2, ',','.');
      		$str_cheques11 = $nombre_bancos.utf8_decode("- Nº").$codchq."- $".$importe_cheque112." ,".$str_cheques11 ;
	 	} 
	
		if ($fact_tcomp[0] == 51 || $fact_tcomp[0] == 53) 
		 	$str_cheques11 = $str_cheques11.utf8_decode(" Retira contra acreditación de cheques."); 
		...
		$ley_chk25 = substr($str_cheques11,3360,140);
  		...
  		if (strlen($str_cheques11) < 141) {
    		...
    		$y = $y+5 ;
   			$pdf->SetY($y);
			if ($fact_tcomp[0] == 51 || $fact_tcomp[0] == 53)
	   			$pdf->Cell(20,10,"Total cheques a 3ros.: $ ".$total_cheques11.utf8_decode(" Retira contra acreditación de cheques"),0,0,'L');
			else
    ...
    $depositos_texto = 1;
 	if (mysqli_num_rows($selec_depositos)!=0) {	
    	...
    	while ($depositos = mysqli_fetch_array($selec_depositos)){
            //echo "ENTRO A DEPOSITOS ".$j;
     		$codban =$depositos['0'];
     		$codsuc =$depositos['1'];
     		$codchq =utf8_decode($depositos['2']);
            $codfec = $depositos['4'];
            $fechadep = substr($codfec,8,2)."/".substr($codfec,5,2)."/".substr($codfec,0,4);
                //echo "CODBAN = ".$codban."  CODSUC = ".$codsuc."  CODCHQ = ".$codchq." - ";
        ...
        //if ((strlen($str_deposito) < 841) and (strlen($str_deposito) >700 )) {
        if ($q == 6) {
   			$pdf->SetY($y);
		   	$pdf->SetFont('Arial','B',7);
		   	$pdf->Cell(20,10,$ley_dep1,0,0,'L');	
 	...
 	if (mysqli_num_rows($selec_depositos_terceros)!=0) {	   
    	while ($depositos_terceros = mysqli_fetch_array($selec_depositos_terceros)){
			...
			$importe_deposito_terceros2 = number_format($importe_deposito_terceros, 2, ',','.');
      		$str_deposito_terceros = $nombre_bancos_terceros.utf8_decode("- Nº").$codchq_terceros."- $".$importe_deposito_terceros2.",".$str_deposito_terceros ;
	 	} 
    ...
    */
 	if (mysqli_num_rows($selec_efectivo)!=0) {	
  		if ( $efectivo_texto==1) {
			if ($pncomp == 26231) {
		   		$y = $y+4 ;
		   		$pdf->SetY($y);
		   		$pdf->Cell(20,10,utf8_decode('Total Crédito por N Cr.A0004-00001300: $ ').$efe_nd,0,0,'L');	
		   		$y = $y+4 ;
		   		$pdf->SetY($y);
		   		$pdf->Cell(20,10,'Total Efectivo: $ '.$efe_tot2,0,0,'L');	
				...
				if ($tcompefe == 91) {
					$y = $y+4 ;
					$pdf->SetY($y);
					$pdf->Cell(20,10,utf8_decode('Crédito '),0,0,'L');	
					$y = $y+4 ;
					$pdf->SetY($y);
					$pdf->Cell(30,10,utf8_decode('Total pago por Crédito: $ ').$efe_tot2,0,0,'L');	
					$efectivo_texto = $efectivo_texto+1;
				}
                else if ($tcompefe == 97) {
					$pdf->Cell(20,10,'Fc. Prov.: ',0,0,'L');	
					$y = $y+4 ;
					$pdf->SetY($y);
					$pdf->Cell(20,10,'Total Fc. Prov.: $ '.$efe_tot2,0,0,'L');	
				}
				else {
					$y = $y+4 ;
					$pdf->SetY($y);
    ...
    //===========================================REMATES======================================
 	if (mysqli_num_rows($selec_rem)!=0) {	
  		if ( $rem_texto!="") {
                while ($rem = mysqli_fetch_array($selec_rem)){
					$y = $y+4 ;
					$pdf->SetY($y);
                    $rem_ncomp = $rem["ncomp"];
	  	...
	  	if ((mysqli_num_rows($selec_ganancias)!=0)) {
	  		...
	  		while ($ganancia = mysqli_fetch_array($selec_ganancias)){	
	 			...
	 			if ( $ganancia_texto==1) {
      				...
      				$pdf->SetY($y);
	  				$pdf->SetFont('Arial','',10);
	  				$pdf->Cell(20,10,utf8_decode('Retención de Ganancias'),0,0,'L'); 
	    			$ganancia_texto =  2;
       			}
	  		...
	  		$pdf->Cell(20,10,utf8_decode('Total Retención Ganancias: $ ').$total_ganancia2,0,0,'L');
	   	...
	   	if ((mysqli_num_rows($selec_iva)!=0)) {
	  		...
	  		while ($iva = mysqli_fetch_array($selec_iva)){	
		 		...
		 		if ( $ing_iva_texto ==1) {
      				...
      				$pdf->SetY($y);
	  				$pdf->SetFont('Arial','',10);
	  				$pdf->Cell(20,10,utf8_decode('Retención de IVA'),0,0,'L'); 
       				$ing_iva_texto =  2;
       			}
      		...
      		$pdf->SetY($y);
	  		$pdf->SetFont('Arial','',10);
	 		$pdf->Cell(20,10,utf8_decode('Total Retención IVA: $ ').$total_iva,0,0,'L');
	   	}
	  	// SUSS  
	    	...
	    	$total_suss =0;
	  		...
	  		while ($sus = mysqli_fetch_array($selec_suss)){	
		 		...
		 		if ( $ing_suss_texto ==1) {
      				...
      				$pdf->SetY($y);
	  				$pdf->SetFont('Arial','',10);
	  				$pdf->Cell(20,10,utf8_decode('Retención de SUSS'),0,0,'L'); 
       				$ing_suss_texto =  2;
       			}
	 		...
	 		$pdf->Cell(20,10,utf8_decode('Total Retención SUSS $ ').$total_suss,0,0,'L');
	  	}
	}
...
for ($j=0;$j < $tope_fact;$j++) {
    ...
    if ($fact_tcomp[$j] == 51 || $fact_tcomp[$j] == 53 || $fact_tcomp[$j] == 89) {
        for ($k=0;$k<2;$k++) {
        ...
        $detallefac = mysqli_query( $query_detfac,$amercado) or die("ERROR LEYENDO 3592");
        $totalRows_detallefac = mysqli_num_rows($detallefac);

        //Leo el remate
        $query_remate = sprintf("SELECT * FROM remates WHERE ncomp = %s", $remate);
        $remates = mysqli_query( $query_remate,$amercado) or die("ERROR LEYENDO 3597");
        ...
        $remates_expo = mysqli_query( $query_remate_expo,$amercado) or die("ERROR LEYENDO 3610");
        ...
        $localidad_rem = mysqli_query( $query_localidades_rem,$amercado) or die("ERROR LEYENDO 3622");
        ...
        $provincia_rem = mysqli_query( $query_provincia_rem,$amercado) or die("ERROR LEYENDO 3628");
        ...
        // Leo el cliente
        $query_entidades = sprintf("SELECT * FROM entidades WHERE  codnum = %s", $cliente);
        $enti = mysqli_query( $query_entidades,$amercado) or die("ERROR LEYENDO 3635");
        $row_entidades = mysqli_fetch_assoc($enti);
        $nom_cliente   = $row_entidades["razsoc"];
        $calle_cliente = $row_entidades["calle"];
        $nro_cliente   = $row_entidades["numero"];
        ...
        // Leo la localidad
        $query_localidades = sprintf("SELECT * FROM localidades WHERE  codnum = %s", $loc_cliente);
        $localidad = mysqli_query( $query_localidades,$amercado) or die("ERROR LEYENDO 3648");
        $row_localidades = mysqli_fetch_assoc($localidad);
        $nomloc = $row_localidades["descripcion"];
        ...
        $remi = mysqli_query($amercado,  $sql_remi )  or die("ERROR LEYENDO 3660");
        ...
        if ($tot_remitos == 0) {
            ...
            $remito = mysqli_query($amercado, $query_remito) or die("ERROR LEYENDO 3668");
            ...
            $act_remito = mysqli_query($amercado, $query_act_remito) or die("ERROR LEYENDO 3673");
            ...
            $ins_remi = mysqli_query($amercado, $insert_remito) or die ("ERROR GRABANDO REMITO 3676");
        }
        ...
        $pdf->Cell(130);
        if ($inmob == 1)
            $pdf->Image('images/AM-Inmobiliaria.png',45,8);
        else
            $pdf->Image('images/AM-Subastas.png',45,10);
        $pdf->Image('images/equis.jpg',101,7.5);
        $pdf->SetFont('Arial','BI',10);
        $pdf->SetY(29);
        $pdf->SetX(15);
        ...
        if ($remate!="" ) {
            ...
            while($row_detallefac = mysqli_fetch_array($detallefac)) {
                ...
                $df_descrip1   = $descrip1; //$df_descrip1.$descrip1;
                $df_descrip2   = $descrip2; //$df_descrip2.$descrip2;
                $df_descrip3   = $descrip3; //$df_descrip2.$descrip2;
