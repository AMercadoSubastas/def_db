...
<span id="el_localidades_activo">
    <select
        ...
        data-select2-id="flocalidadesadd_x_activo"
        <?php } ?>
        data-table="localidades"
        data-field="x_activo"
...
<script>
loadjs.ready("flocalidadesadd", function() {
    var options = { name: "x_activo", selectId: "flocalidadesadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (flocalidadesadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "flocalidadesadd" };
    } else {
        options.ajax = { id: "x_activo", form: "flocalidadesadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
