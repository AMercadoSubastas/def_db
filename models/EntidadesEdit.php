 ...
 * Page class
 */
class EntidadesEdit extends Entidades
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "EntidadesEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "EntidadesEdit";
    ...
    protected function getEditRow($rsold)
    {
        ...
        // razsoc
        $this->razsoc->setDbValueDef($rsnew, $this->razsoc->CurrentValue, $this->razsoc->ReadOnly);
        ...
        // tellinea
        $this->tellinea->setDbValueDef($rsnew, $this->tellinea->CurrentValue, $this->tellinea->ReadOnly);

        // telcelu
        $this->telcelu->setDbValueDef($rsnew, $this->telcelu->CurrentValue, $this->telcelu->ReadOnly);

        // tipoent
        $this->tipoent->setDbValueDef($rsnew, $this->tipoent->CurrentValue, $this->tipoent->ReadOnly);
        ...
        // calif
        $this->calif->setDbValueDef($rsnew, $this->calif->CurrentValue, $this->calif->ReadOnly);
        ...
        // mailcont
        $this->mailcont->setDbValueDef($rsnew, $this->mailcont->CurrentValue, $this->mailcont->ReadOnly);
        ...
        // pagweb
        $this->pagweb->setDbValueDef($rsnew, $this->pagweb->CurrentValue, $this->pagweb->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        if (isset($row['razsoc'])) { // razsoc
            $this->razsoc->CurrentValue = $row['razsoc'];
        }
        ...
        if (isset($row['tellinea'])) { // tellinea
            $this->tellinea->CurrentValue = $row['tellinea'];
        }
        if (isset($row['telcelu'])) { // telcelu
            $this->telcelu->CurrentValue = $row['telcelu'];
        }
        if (isset($row['tipoent'])) { // tipoent
            $this->tipoent->CurrentValue = $row['tipoent'];
        }
        ...
        if (isset($row['calif'])) { // calif
            $this->calif->CurrentValue = $row['calif'];
        }
        ...
        if (isset($row['mailcont'])) { // mailcont
            $this->mailcont->CurrentValue = $row['mailcont'];
        }
        ...
        if (isset($row['pagweb'])) { // pagweb
            $this->pagweb->CurrentValue = $row['pagweb'];
        }
