...
if(isset($_GET['remate'])){  
		...
		if ($tipo_de_iva == 1) {
			$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) , SUM( `totneto21` *'$procen_iva' ) , SUM( `totneto21` *'$procen_iva' + `totneto21`)  as total_21 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND `tcomp`!='18' AND `tcomp`!='19' )" ;
			$query_remate = mysqli_query($amercado, $res1) or die(mysqli_error($amercado));
			$totalRows_remate = mysqli_num_rows($query_remate); 
		}
		else {
			$res1 ="SELECT SUM( `totneto105` ) , SUM( `totiva105` ) , SUM( `totneto105` + `totiva105` ) as total_105 , SUM( `totneto21` ) , SUM( `totneto21` *'$procen_iva' ) , SUM( `totneto21` *'$procen_iva' + `totneto21`)  as total_21 FROM `cabfac` WHERE (`codrem` = '$rematenum' AND `tcomp`!='27' AND `tcomp`!='28' )" ;
			$query_remate = mysqli_query($amercado, $res1) or die(mysqli_error($amercado));
			$totalRows_remate = mysqli_num_rows($query_remate); 
		}
...
if ($tipo_de_iva== 1) {
		$query_factura_total     = "SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 18 OR cabfac.tcomp = 19) AND cabfac.estado = 'S' AND cabfac.codrem ='$rematenum'" ;
		...
		$query_factura_total     = "SELECT SUM( cabfac.totbruto) FROM cabfac WHERE (cabfac.tcomp = 27 OR cabfac.tcomp = 28)  AND cabfac.estado = 'S' AND cabfac.codrem ='$rematenum'" ;
		$factura_total           = mysqli_query($amercado, $query_factura_total) or die(mysqli_error($amercado));
		$row_factura_total       = mysqli_fetch_assoc($factura_total);
		$totalRows_factura_total = mysqli_num_rows($factura_total);
		$totfactura1             = number_format ($row_factura_total['SUM( cabfac.totbruto)'] , 2 , "," , ".");
...
<head>
	...
	<SCRIPT type="text/javascript"> var DHTML_SUITE_THEME_FOLDER = 'themes/'; </SCRIPT> 
	...
	<form id="liquidacion" name="liquidacion" method="POST" action="<?php echo $editFormAction; ?>">
  		<div align="left">
  		  <table width="1000" border="0" align="center" cellpadding="1" cellspacing="1">
  		    <td bgcolor="#FFFFFF">
            	<table width="640" border="0" cellspacing="1" cellpadding="1"> </td>
                <? $tipo_de_iva = 1; ?>
  		     	 <?    if ($tipo_de_iva == 1) { ?>
  		      		<tr>
  		        	...
  		        	<td ><input name="tcomp_nombre" type="text" class="phpmaker" value="LIQUIDACION A" />
  		          		 <input name="tcomp" type="hidden" value="3" />          
  		          	</td>
  		        	<td height="20" bgcolor="#FFFFFF" class="ewTableHeader">&nbsp;Serie</td>
  		        	<td><input name="serie_nombre" type="text" class="phpmaker" value="SERIE DE LIQUIDACION A" /></td>
  		        		<input name="serie" type="hidden" value="2" />
	          		</tr>
			  	<? } else { ?>
  		      		<tr>
  		            <td height="20" bgcolor="#FFFFFF"><span class="ewTableHeader">&nbsp;Tipo de comprobante</span> </td>
  		        	...
  		        	<td height="20" bgcolor="#FFFFFF" class="ewTableHeader">&nbsp;Serie</td>
  		        	<td><input name="serie_nombre" type="text" class="phpmaker" value="SERIE DE LIQUIDACION B" /></td>
  		        		<input name="serie" type="hidden" value="13" />
	          		</tr> 
  		      
  		      	<?  }  ?>
  		      		<tr>
  		        	<td height="20" bgcolor="#FFFFFF" class="ewTableHeader">&nbsp;Num de liquidacion </td>
  		        	<td><input name="liquidacion" type="text"  value="<? echo "$liquidacion" ?>" readonly/>          </td>
  		        	...
  		        	<td><input name="fecha_liq" type="text" size="11" maxlength="11" /><a href="javascript:showCal('Calendar8')"><img src="images/ew_calendar.gif" width="15" height="15" border="0"></td>
  		      		...
  		      		<input name="codpais" type="hidden" size="11" maxlength="11" value="<? echo $codpais; ?>"/>
  		      		<input name="codprov" type="hidden" size="11" maxlength="11" value="<? echo $codprov; ?>"/>
  		      		<input name="codloc"  type="hidden" size="11" maxlength="11" value="<? echo $codloc; ?>"/>
  		      <tr>
  		        	<td height="20" bgcolor="#FFFFFF" class="ewTableHeader">&nbsp;Cliente</td>
  		        	<td><input name="cliente" type="text"  value="<? echo $nom_clientes; ?>" readonly/>          </td>
  		        	<td bgcolor="#FFFFFF" class="ewTableHeader" >&nbsp;Rubro</td>
  		        	<td bgcolor="#FFFFFF" class="ewTableHeader" >
  		          	<select name="rubro">
  		      	...
  		      	<input name="num_cliente" type="hidden"  value="<? echo $clientes; ?>" readonly/>
  		      	<input name="fecha_remate" type="hidden"  value="<? echo $fremate; ?>" readonly/>
  		    	<tr>
  		        	...
  		        	<td><input name="fecha_remate1" type="text" class="phpmaker" size="11" value ="<? echo  $fecha_remate; ?>"/></td>
  		   		</tr>
  		      	<tr>
  		        	<td height="20" bgcolor="#FFFFFF" class="ewTableHeader">&nbsp;Lugar de Remate </td>
  		        	<td colspan="3"><input name="lugar_remate" type="text" size="60" value="<? echo $direccion;  ?>"  /></td>
  		        </tr>
  		      </table></td>
  		      </tr>
  		    <tr>
  		        ...
  		        <tr>
  		          <td colspan="3" bgcolor="#FFFFFF"><table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#FFFFFF">
  		            ...
  		            <tr>
  		              ...
  		              <td width="69%"><input name="importe_total" type="text" class="phpmaker" value="<? echo $total_remate; ?>"/></td>
	                </tr>
  		            </table></td>
  		          </tr>
  		        <tr>
  		          <td width="31%" bgcolor="#FFFFFF"><span class="ewTableHeader">&nbsp;Neto Gravado&nbsp;<? echo $iva_15_porcen; ?>&nbsp;%</span> </td>
  		          <td width="27%"><input name="neto105" class="phpmaker" type="text" id="neto105" value="<? echo $totneto105; ?>"/></td>
  		          <td width="42%" >&nbsp;</td>
	            </tr>
  		        <tr>
  		          <td bgcolor="#FFFFFF" ><span class="ewTableHeader">&nbsp;<? echo $iva_15_desc; ?>&nbsp;% </span></td>
  		          <td  ><input  name="iva105" type="text" class="phpmaker" value="<? echo $iva105; ?>" /></td>
  		          <td>&nbsp;</td>
	            </tr>
  		        <tr>
  		          <td bgcolor="#FFFFFF"><span class="ewTableHeader">&nbsp;Total <? echo $iva_15_porcen; ?>&nbsp;%</span></td>
  		          <td><input name="total105" type="text" class="phpmaker" value="<? echo $total_105; ?>" /></td>
  		          <td><input name="total105_1" type="text" class="phpmaker" value="<? echo $total_105; ?>"/></td>
	            </tr>
  		        <tr>
  		          <td width="31%" bgcolor="#FFFFFF" class="ewTableHeader"><span class="Estilo1">&nbsp;Neto Gravado&nbsp;<? echo $iva_21_porcen; ?>&nbsp;%</td>
  		          <td width="27%"><input name="neto21" type="text" class="phpmaker" value="<? echo $totneto21; ?>"/></td>
  		          <td width="42%">&nbsp;</td>
	            </tr>
  		        <tr>
  		          <td bgcolor="#FFFFFF"><span class="ewTableHeader">&nbsp;<? echo $iva_21_desc; ?>&nbsp;% </span></td>
  		          <td><input name="iva21" type="text" class="phpmaker" value="<? echo $iva21; ?>"/></td>
  		          <td bgcolor="#FFFFFF" class="ewLayout">&nbsp;</td>
	            </tr>
  		        <tr>
  		          <td bgcolor="#FFFFFF"><span class="ewTableHeader">&nbsp;Total <? echo $iva_21_porcen; ?>&nbsp;%</span></td>
  		          <td><input name="total21" type="text" class="phpmaker" value="<? echo $total_21; ?>"/></td>
  		          <td><input name="total21_1" type="text" class="phpmaker" value="<? echo $total_21; ?>"/></td>
	            </tr> <? } else { 
  		    ...
  		    <tr>
  		      <td width="31%" bgcolor="#FFFFFF"><span class="ewTableHeader">&nbsp;Neto Gravado&nbsp;<? echo $iva_15_porcen ;?>&nbsp;%</span> </td>
  		      <td width="27%"><input name="neto105" type="text" id="neto105" form="liquidacion" value="<? echo $totneto105; ?>"/></td>
  		      <td width="42%" >&nbsp;</td>
	        </tr>
  		    <tr>
  		      <td bgcolor="#FFFFFF" ><span class="ewTableHeader">&nbsp;<? echo $iva_15_desc; ?>&nbsp;% </span></td>
  		      <td  ><input  name="iva105" type="text" value="<? echo $iva105; ?>" /></td>
  		      <td>&nbsp;</td>
	        </tr>
  		    <tr>
  		      <td bgcolor="#FFFFFF"><span class="ewTableHeader">&nbsp;Total <? echo $iva_15_porcen; ?>&nbsp;%</span></td>
  		    ...
  		    <tr>
  		      ...
  		      <td width="27%"><input name="acuenta" type="text" class="phpmaker" value="<? echo $totalcheques; ?>" /></td>
  		    ...
  		    <tr>
  		      ...
  		      <td><input name="gastos_autor" type="text" class="phpmaker"  value="<? echo $totfactura; ?>" /></td>
  		    ...
  		    <tr>
  		      ...
  		      <td><input name="total_resta" type="text" class="phpmaker" value="<? echo $totalgastos; ?>" /></td>
	        </tr>
  		    <tr>
  		      <td background="images/separador3.gif" colspan="3">&nbsp;</td>
	        </tr>
  		    <tr>
  		      <td  bgcolor="#FFFFFF"><span class="Estilo1"><span class="ewTableHeader">Saldo a favor del cliente </span></td><td><input name="total_general" type="text" class="phpmaker" value="<? echo $total_general ?>" /></td>
  		      <td><a href="otros_gastos_liq.php?tot_general=<? echo $total_general; ?>&&liquidacion=<? echo $liquidacion; ?>&&remate=<? echo $rematenum; ?>&&serie=<? echo $tserie; ?>&&tipocomp=<? echo $tcomprobante; ?>" target="_blank"><img src="images/medios_pago1.gif" width="103" height="19" border="0" ></a></td>
	        </tr>
	      </table></td>
  		  </tr>
	  </div>
      ...
      <td><table width="100%" border="0" cellspacing="1" cellpadding="1">
        <tr>
          <td><div align="center">
            <input name="Submit" type="submit" class="phpmaker" value="Grabar" />
          </div></td>
        </tr>
      </table></td>
    </tr>
