...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "proveedor")) {
	...
	if ($k==0) {
		...
		echo '<script language="javascript">alertaConfirmado("factura='.$facnum.'&tcomp= '.$tipocomp.'");</script>';
		
		}

	// ACA VALIDO LA FECHA DE ALTA
	$serieNum = $_POST['serie'];
	$fechaSQL = mysqli_query($amercado, "SELECT fechatope FROM series WHERE codnum = $serieNum");
	var_dump("SELECT fechatope FROM series WHERE codnum = $serieNum");
	$sqli = mysqli_fetch_assoc($fechaSQL);
	$fechaAlta = $sqli['fechatope']; // Fecha en formato 'yyyy-mm-dd' o el que corresponda
	$fechaReg = $_POST['fecha_f2'];
	if ($fechaReg <= $fechaAlta){
		$siga = 0;
		echo '<script language="javascript">alertaError("La fecha de carga del comprobante debe ser posterior a '.$fechaAlta.'");</script>';		
	}
	...
	// echo "tipo_cbte = ".$tipo_cbte." - ";
	// echo "cond_iva = ".$cond_iva." - ";
    if ($cond_iva != 1 && ($tipo_cbte == 32 || $tipo_cbte == 34 || $tipo_cbte == 36 || $tipo_cbte == 110)) {
        $siga = 0;
		echo "<script>alertaError('le pifiaste en el tipo de factura o en la cond de iva del proveedor $tipo_cbte y $tipo_cbte');</script>";
    }
...
if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text")!="NULL") {
	if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "proveedor")) {
        ...
        //echo "grabo la cabecera ";
	 	...
	 	$facnum = GetSQLValueString($_POST['ncomp'], "text");
		$tipocomp = GetSQLValueString($_POST['serie_texto'], "text");
  		echo "<script> alertaConfirmado('factura= $facnum&tcomp= $tipocomp');</script>"; 
...
function ValidoRepe(form) {
	alertaError("HOLA DENTRO DE LA FUNCION VIENDO QUE LE PIFIASTE AL CBTE");
	...
	for(i=0;i< $k; i++) {
		if ($cbte[i] == cbte_ing && $prov[i] == prov_ing && $tipo[i] == tcomp_ing) {
			alertaError("EL COMPROBANTE YA HA SIDO INGRESADO CON ANTERIORIDAD");
		}
	}
...
function validarFormulario(form)
{
	...
	// PRIMER LOTE
	//alertaError("TIPOIVA = :\n"+tipoiva);
	//alertaError("TIPOIVA1 = :\n"+tipoiva1);
	//alertaError("TIPOIVA2 = :\n"+tipoiva2);
	//alertaError("TIPOIVA3 = :\n"+tipoiva3);

	if (monto.length!=0.0) {
		
			//alertaError("TIENE MONTO, IVA = "+tipoiva);
		if (tipoiva == 0) {
		...
		if (tipoiva == 21.0) {
			...
			//alertaError("tot_mon ="+tot_mon);
			//alertaError("imp_tot_21 ="+imp_tot_21);
			//alertaError("tot_general ="+tot_general);
	...
	// alertaError("S1+"+series);
	
		
	//var serie = proveedor.serie.value;
	//alertaError(proveedor.serie.value);
	//	alertaError("Serie"+series);
     	// alertaError("s2"+series);
      	if ( series == '11' ) {
		// Oculto
		// alertaError()
		proveedor.totneto21.value = tot_mon.toFixed(2) ;	
		...
		//   alertaError(proveedor.tot_general_1.value);
...
//     var fecha_cbte = form.fecha_f2.value; // asumiendo formato 'dd/mm/yyyy'
//     var fechaTope = "31/10/2023"; // puedes ajustar esta fecha según necesites

//     // Convertir fechas a objetos Date para una comparación fácil
//     var partesFechaCbte = fecha_cbte.split('/');
//     var partesFechaTope = fechaTope.split('/');

//     var dateCbte = new Date(partesFechaCbte[2], partesFechaCbte[1] - 1, partesFechaCbte[0]);
// 	console.log(dateCbte);
//     var dateTope = new Date(partesFechaTope[2], partesFechaTope[1] - 1, partesFechaTope[0]);
// 	console.log(dateTope);
//     // Comparar las fechas
//     if (dateCbte <= dateTope) {
//         alertaError("La fecha de carga del comprobante debe ser posterior a " + fechaTope);
//         form.fecha_f2.value = "";
//         form.fecha_f2.focus();
//     }
// }
...
function ArmarNrodoc(form)
{
	...
	var error ="";
	var nrodoc = "";
  		//alertaError("tipo_fac = "+tipo_fac);
	 	//alertaError("pto_vta = "+pto_vta);
		//alertaError("nro_fac_pro = "+nro_fac_pro);
	...
	if (tipo_fac=="" || pto_vta=="" || nro_fac_pro=="") {
		...
		// Valido si es numero
		if (!/^([0-9])*$/.test(pto_vta)) {
	    	alertaError("El valor " + pto_vta + " no es un número");
			form.ncomp.value = "";
			form.pto_vta_pro.focus();
			return;
		}
		if (!/^([0-9])*$/.test(nro_fac_pro)) {
	    	alertaError("El valor " + nro_fac_pro + " no es un número");
			form.ncomp.value = "";
		...
		if (pto_vta > 99999 || pto_vta < 1) {
			alertaError("El punto de venta debe estar entre 1 y 99999");
			form.ncomp.value = "";
			form.pto_vta_pro.focus();
			return;
		}
		if (nro_fac_pro > 99999999 || nro_fac_pro < 1) {
			alertaError("El Nro de Cbte debe estar entre 1 y 99999999");
			form.ncomp.value = "";
			form.nro_fac_prov.focus();
			return;
		}
...
<form id="proveedor" name="proveedor" method="POST" class="form-horizontal ewForm ewAddForm" action="AFacprov">
...
<td colspan="3" valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="1" cellpadding="1">
        <tr>
          ...
          <td width="24%"><select name="tipos" onchange="agregarOpciones(this.form)" >
                                      ...
                                      <option value="32">FACT PROVEEDOR A</option>
                                      <option value="33">FACT PROVEEDOR C</option>
									  <option value="65">FACT PROVEEDOR M</option>
                                      <option value="34">NOTA DEB PROVEEDOR A</option>
                                      <option value="35">NOTA DEB PROVEEDOR C</option>
			  							<option value="88">NOTA DEB PROVEEDOR M</option>
									  <option value="36">NOTA CRED PROVEEDOR A</option>
                                      <option value="37">NOTA CRED PROVEEDOR C</option>
			  						<option value="87">NOTA CRED PROVEEDOR M</option>
                                      <option value="107">FACT PROVEEDOR EXT</option>
                                      <option value="110">LIQUID. GASTOS COMUNES</option>
          ...
          <input name="fecha_tope"  type="hidden"  value="31-10-2023" />
        ...
        <td height="20" class="ewTableHeader">Pto. de Vta. de Factura </td>
         ...
         <tr>
		  ...
		  <td><img src="calendar/img.gif" width="26" height="19" onclick="showCal('Calendar13')" border="0"/></td>
             
            <td width="30">Fecha Cbte </td>
          <td width="30"><input name="fecha_f3" id="fecha_f3" required="required"  type="text" size="11" /></td>
		  <td><img src="calendar/img.gif" width="26" height="19" onclick="showCal('Calendar18')" border="0"/></td>
...
do {  
?>
            <option value="<?php echo $row_conceptos_a_facturar['nroconc'];?>"><?php echo $row_conceptos_a_facturar['nroconc'];?>&nbsp;<?php echo $row_conceptos_a_facturar['descrip'];?></option>
            <?php
