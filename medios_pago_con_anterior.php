...
if (isset($_POST['tcomp']) && GetSQLValueString($_POST['tcomp'], "int")=="18") {
	if (GetSQLValueString($_POST['leyendafc'], "text")!="NULL") {
 		$insertSQLfactley = sprintf("INSERT INTO factley (tcomp, serie, ncomp, leyendafc, codrem) VALUES (%s, %s, %s, %s, %s)",
                    ...
                    1, // GetSQLValueString($_POST['serie'], "int"),
...
if (isset($_POST['tcomp']) && GetSQLValueString($_POST['tcomp'], "int")=="19") {
	if (GetSQLValueString($_POST['leyendafc'], "text")!="NULL") {
 		$insertSQLfactley = sprintf("INSERT INTO factley (tcomp, serie, ncomp, leyendafc, codrem) VALUES (%s, %s, %s, %s, %s)",
                    GetSQLValueString($_POST['tcomp'], "int"),
                    5, // GetSQLValueString($_POST['serie'], "int"),
					GetSQLValueString($_POST['numfac'], "int"),
					GetSQLValueString($_POST['leyendafc'], "text"),
                    GetSQLValueString($_POST['codrem'], "int"));
...
if (isset($_POST['tcomp']) && GetSQLValueString($_POST['tcomp'], "int")=="27") {
	if (GetSQLValueString($_POST['leyendafc'], "text")!="NULL") {
 		$insertSQLfactley = sprintf("INSERT INTO factley (tcomp, serie, ncomp, leyendafc, codrem) VALUES (%s, %s, %s, %s, %s)",
                 	GetSQLValueString($_POST['tcomp'], "int"),
                    11, // GetSQLValueString($_POST['serie'], "int"),
					GetSQLValueString($_POST['numfac'], "int"),
					GetSQLValueString($_POST['leyendafc'], "text"),
                    GetSQLValueString($_POST['codrem'], "int"));
...
if (isset($_POST['tcomp']) && GetSQLValueString($_POST['tcomp'], "int")=="28") {
	if (GetSQLValueString($_POST['leyendafc'], "text")!="NULL") {
 		$insertSQLfactley = sprintf("INSERT INTO factley (tcomp, serie, ncomp, leyendafc, codrem) VALUES (%s, %s, %s, %s, %s)",
                 	GetSQLValueString($_POST['tcomp'], "int"),
                    12, // GetSQLValueString($_POST['serie'], "int"),
					GetSQLValueString($_POST['numfac'], "int"),
					GetSQLValueString($_POST['leyendafc'], "text"),
                    GetSQLValueString($_POST['codrem'], "int"));
...
$num_fact = $num_fact1[0];
$total_dol = 0;
$tcomp = $tcomp[0] ; //ver, decia $tcompu[0]
$serie = $serie1[0] ; // ver, decia $serie
$remate = $remate_num[0];
$tot_general = $tot_general[0];
//$tip_comp = '18';
//$serie = '1';
//$num_fact = '6503';
//echo "ANTES DEL PRIMER SQL\n";
$sql_deposito = sprintf("SELECT   bancos.nombre ,codchq , importe , fechapago , codrem FROM cartvalores , bancos   WHERE tcomp = 9  AND serie = 7  AND tcomprel = %s AND serierel = %s AND ncomprel = %s AND bancos.codnum = cartvalores.codban",$tip_comp,$serie,$num_fact);
		
		$sql = sprintf("SELECT bancos.nombre , codchq , importe , fechapago , codrem FROM cartvalores , bancos WHERE tcomp =8 AND serie =6 AND tcomprel = %s AND serierel = %s AND ncomprel = %s AND bancos.codnum = cartvalores.codban", $tip_comp,$serie,$num_fact);
		
		$sql_dolares = sprintf("SELECT importe FROM cartvalores  WHERE tcomp = 13 AND serie = 9  AND tcomprel = %s  AND serierel = %s AND ncomprel = %s",$tip_comp,$serie,$num_fact);	

		$sql_pesos = sprintf("SELECT importe  FROM cartvalores  WHERE tcomp =12 AND serie =8 AND tcomprel = %s AND serierel = %s AND ncomprel = %s",$tip_comp,$serie,$num_fact);
		
		$sql_iva = sprintf("SELECT * FROM cartvalores  WHERE tcomp = 40 AND serie = 22 AND tcomprel = %s  AND serierel = %s AND ncomprel = %s ", $tip_comp, $serie, $num_fact);
	
		$sql_ing_brutos = sprintf("SELECT * FROM cartvalores  WHERE tcomp =41 AND serie =23 AND tcomprel =%s AND serierel =%s AND ncomprel =%s",$tip_comp,$serie, $num_fact) ;
		
		$sql_ganancias = sprintf("SELECT * FROM cartvalores  WHERE tcomp =42 AND serie =24 AND tcomprel = %s AND serierel = %s AND ncomprel = %s",$tip_comp,$serie,$num_fact);
		
		$sql_suss = sprintf("SELECT * FROM cartvalores WHERE tcomp =43 AND serie =25 AND tcomprel = %s AND serierel = %s AND ncomprel = %s",$tip_comp,$serie,$num_fact);
		$precio2 =0 ;
		//echo "DESPUES DEL ULTIMO SQL\n";
      ...
      <tr>
        <td><?php // ($efectivo + $precio2);
			 ...
			 if ($monto_ing_brutos!=0) {
	         	$leyenda = $leyenda." Ret. IIBB $".$monto_ing_brutos." Comp N".$comp_ing_brutos."\n";
