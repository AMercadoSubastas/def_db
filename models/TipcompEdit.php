 ...
 * Page class
 */
class TipcompEdit extends Tipcomp
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "TipcompEdit";
    ...
    // CSS class/style
    public $CurrentPageName = "TipcompEdit";
    ...
    protected function getEditRow($rsold)
    {
        ...
        // esfactura
        $this->esfactura->setDbValueDef($rsnew, strval($this->esfactura->CurrentValue) == "1" ? "1" : "0", $this->esfactura->ReadOnly);

        // esprovedor
        $this->esprovedor->setDbValueDef($rsnew, strval($this->esprovedor->CurrentValue) == "1" ? "1" : "0", $this->esprovedor->ReadOnly);

        // codafip
        $this->codafip->setDbValueDef($rsnew, $this->codafip->CurrentValue, $this->codafip->ReadOnly);
    ...
    protected function restoreEditFormFromRow($row)
    {
        ...
        if (isset($row['esfactura'])) { // esfactura
            $this->esfactura->CurrentValue = $row['esfactura'];
        }
        if (isset($row['esprovedor'])) { // esprovedor
            $this->esprovedor->CurrentValue = $row['esprovedor'];
        }
        if (isset($row['codafip'])) { // codafip
            $this->codafip->CurrentValue = $row['codafip'];
        }
