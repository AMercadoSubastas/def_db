...
<span id="el_tipoiva_activo">
    <select
        ...
        data-select2-id="ftipoivaedit_x_activo"
        <?php } ?>
        data-table="tipoiva"
        data-field="x_activo"
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("ftipoivaedit", function() {
    var options = { name: "x_activo", selectId: "ftipoivaedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (ftipoivaedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "ftipoivaedit" };
    } else {
        options.ajax = { id: "x_activo", form: "ftipoivaedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
