...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "factura")) {
    ...
    $ImpTotal     = GetSQLValueString($_POST['tot_general_1'], "double"); //121.00;
    $ImpTotConc   = 0.00; // GetSQLValueString($_POST['totiva21_1'], "double"); // 0.00;
    $ImpNeto      = GetSQLValueString($_POST['totneto21_1'], "double"); //100.00;
    $ImpOpEx      = 0.00;
    $ImpIVA       = GetSQLValueString($_POST['totiva21_1'], "double"); // 21.00;
    $ImpTrib      = 0.00;
    ...
    $IvaAlicuotaBaseImp = GetSQLValueString($_POST['totneto21_1'], "double");// 100.00;
    $IvaAlicuotaImporte = GetSQLValueString($_POST['totiva21_1'], "double");//21.00;   
	//========================================================================================================================
	//LA MANDO AL WS
	...
	if (isset($_POST['descripcion']) && GetSQLValueString($_POST['descripcion'], "text")!="NULL") {
		...
		VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",
			...
			if (!empty($_POST['imprime'])) { 
				...
				$insertGoTo = "rp_facnc.php?ftcomp=$tipcomp&&fserie=$numserie&&fncomp=$num_fac";
				if (isset($_SERVER['QUERY_STRING'])) {
					$insertGoTo .= (strpos($insertGoTo, '?')) ? "&" : "?";
					$insertGoTo .= $_SERVER['QUERY_STRING'];
				}
				header(sprintf("Location: %s", "rp_facnc.php?ftcomp=$tipcomp&&fserie=$numserie&&fncomp=$num_fac")); 
	
			} else { 
				$facnum = $CbteDesde; //GetSQLValueString($_POST['num_factura'], "int");
				$insertGoTo = "factura_ok.php?factura=$num_fac";
				if (isset($_SERVER['QUERY_STRING'])) {
					$insertGoTo .= (strpos($insertGoTo, '?')) ? "&" : "?";
					$insertGoTo .= $_SERVER['QUERY_STRING'];
				}
				header(sprintf("Location: %s", "factura_ok.php?factura=$num_fac")); 
			}
		}
	}
}
    ...
    <tr>
      <td colspan="2" valign="top" bgcolor="#82BADD"><table width="694" border="1" cellspacing="1" cellpadding="1">
        ...
        <tr>
          ...
          <td><input name="num_factura" type="num_factura" size="25" class="phpmakerlist"  value= <?php if(isset($num_factura)) echo $num_factura; ?> /></td>
          
          <td class="ewTableHeader">Fecha Factura </td>
          <td><input name="fecha_factura1" type="text" size="24" id="fecha_factura1" value= <?php echo $fecha_hoy; ?> />
