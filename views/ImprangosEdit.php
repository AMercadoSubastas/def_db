...
?>
<main class="edit">
<form name="fimprangosedit" id="fimprangosedit" class="<?= $Page->FormClassName ?>" action="<?= CurrentPageUrl(false) ?>" method="post" autocomplete="off">
<script>
...
<span id="el_imprangos_codimp">
    <select
        ...
        data-select2-id="fimprangosedit_x_codimp"
        <?php } ?>
...
<?php if (!$Page->codimp->IsNativeSelect) { ?>
<script>
loadjs.ready("fimprangosedit", function() {
    var options = { name: "x_codimp", selectId: "fimprangosedit_x_codimp" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fimprangosedit.lists.codimp?.lookupOptions.length) {
        options.data = { id: "x_codimp", form: "fimprangosedit" };
    } else {
        options.ajax = { id: "x_codimp", form: "fimprangosedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
...
<?php } ?>
<input type="hidden" data-table="imprangos" data-field="x_codimp" data-hidden="1" data-old name="o_codimp" id="o_codimp" value="<?= HtmlEncode($Page->codimp->OldValue ?? $Page->codimp->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<input type="hidden" data-table="imprangos" data-field="x_secuencia" data-hidden="1" data-old name="o_secuencia" id="o_secuencia" value="<?= HtmlEncode($Page->secuencia->OldValue ?? $Page->secuencia->CurrentValue) ?>">
</span>
</div></div>
    </div>
<?php } ?>
...
<?php if ($Page->activo->Visible) { // activo ?>
    <div id="r_activo"<?= $Page->activo->rowAttributes() ?>>
        <label id="elh_imprangos_activo" for="x_activo" class="<?= $Page->LeftColumnClass ?>"><?= $Page->activo->caption() ?><?= $Page->activo->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
...
<span id="el_imprangos_activo">
    <select
        ...
        data-select2-id="fimprangosedit_x_activo"
        <?php } ?>
        data-table="imprangos"
        data-field="x_activo"
...
<script>
loadjs.ready("fimprangosedit", function() {
    var options = { name: "x_activo", selectId: "fimprangosedit_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fimprangosedit.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fimprangosedit" };
    } else {
        options.ajax = { id: "x_activo", form: "fimprangosedit", limit: ew.LOOKUP_PAGE_SIZE };
    }
    options.minimumResultsForSearch = Infinity;
    options = Object.assign({}, ew.selectOptions, options, ew.vars.tables.imprangos.fields.activo.selectOptions);
    ew.createSelect(options);
});
</script>
<?php } ?>
...
<button class="btn btn-primary ew-btn" name="btn-action" id="btn-action" type="submit" form="fimprangosedit"><?= $Language->phrase("SaveBtn") ?></button>
<?php if (IsJsonResponse()) { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" data-bs-dismiss="modal"><?= $Language->phrase("CancelBtn") ?></button>
<?php } else { ?>
<button class="btn btn-default ew-btn" name="btn-cancel" id="btn-cancel" type="button" form="fimprangosedit" data-href="<?= HtmlEncode(GetUrl($Page->getReturnUrl())) ?>"><?= $Language->phrase("CancelBtn") ?></button>
<?php } ?>
    </div><!-- /buttons offset -->
