 ...
 * Page class
 */
class BancosAdd extends Bancos
{
    use MessagesTrait;
    ...
    // CSS class/style
    public $CurrentPageName = "BancosAdd";
        ...
        switch ($this->CurrentAction) {
            case "copy": // Copy an existing record
                if (!$rsold) { // Record not loaded
                    ...
                    $this->terminate("BancosList"); // No matching record, return to list
                    return;
                }
                break;
            case "insert": // Add new record
                ...
                if ($this->addRow($rsold)) { // Add successful
                    ...
                    if (GetPageName($returnUrl) == "BancosList") {
                        $returnUrl = $this->addMasterUrl($returnUrl); // List page, return to List page with correct master key if necessary
                    } elseif (GetPageName($returnUrl) == "BancosView") {
                    ...
                    if ($this->IsModal && $this->UseAjaxActions) {
                        ...
                        if (GetPageName($returnUrl) != "BancosList") {
                            ...
                            $returnUrl = "BancosList"; // Return list page content
                        }
                    }
    ...
    protected function loadFormValues()
    {
        ...
        // Check field name 'codbanco' first before field var 'x_codbanco'
        $val = $CurrentForm->hasValue("codbanco") ? $CurrentForm->getValue("codbanco") : $CurrentForm->getValue("x_codbanco");
        if (!$this->codbanco->IsDetailKey) {
            if (IsApi() && $val === null) {
                $this->codbanco->Visible = false; // Disable update for API request
            } else {
                $this->codbanco->setFormValue($val);
            }
        }
    ...
    {
        global $CurrentForm;
        $this->codbanco->CurrentValue = $this->codbanco->FormValue;
    ...
    public function renderRow()
    {
        ...
        if ($this->RowType == RowType::VIEW) {
            ...
            $this->codbanco->EditValue = HtmlEncode($this->codbanco->CurrentValue);
    ...
    protected function validateForm()
    {
        ...
        $validateForm = true;
            if ($this->codbanco->Visible && $this->codbanco->Required) {
                if (!$this->codbanco->IsDetailKey && EmptyValue($this->codbanco->FormValue)) {
                    $this->codbanco->addErrorMessage(str_replace("%s", $this->codbanco->caption(), $this->codbanco->RequiredErrorMessage));
                }
            }
    ...
    protected function getAddRow()
    {
        ...
        // codbanco
        $this->codbanco->setDbValueDef($rsnew, $this->codbanco->CurrentValue, false);
    ...
    {
        if (isset($row['codbanco'])) { // codbanco
            $this->codbanco->setFormValue($row['codbanco']);
        }
    ...
    protected function setupBreadcrumb()
    {
        ...
        $url = CurrentUrl();
        $Breadcrumb->add("list", $this->TableVar, $this->addMasterUrl("BancosList"), "", $this->TableVar, true);
