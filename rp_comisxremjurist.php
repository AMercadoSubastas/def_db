...
//echo "REMATE NRO 0 ".$remates[0]." - ";
//echo "REMATE NRO 1 ".$remates[1]." - ";
for ($k = 0; $k < $tope;$k++) {
    //echo "K = ".$k." - ".$remates[$k]."  " ;
    //echo "for REMATE NRO 0 ".$remates[0]." - ";
    //echo "for REMATE NRO 1 ".$remates[1]." - ";     
    $query_cabf = sprintf("SELECT * FROM cabfac WHERE codrem = $remates[$k] AND tcomp NOT IN (98,99) ORDER BY tcomp  , ncomp");
    $cabf = mysqli_query($amercado, $query_cabf) or die("ERROR LEYENDO CABFAC 57".$query_cabf." - ");	
    ...
    $query_cabfac = sprintf("SELECT * FROM cabfac WHERE (tcomp BETWEEN 51 AND 64 OR tcomp IN (89,92,93,94,104,105,111,112,113)) AND codrem = %s ORDER BY fecreg, nrodoc, tcomp", $remates[$k]);
    ...
    $pdf->SetY(15);
    $pdf->Cell(110);
    $pdf->Cell(60,10,' Ventas por Subasta Consolidada (Pcia. de Bs. As.) - Usuario cobranzas: '.$nombre_usu,0,0,'L');
    ...
    $pdf->SetX(115);
    $pdf->Cell(23,16,' Publicidad y ',0,0,'L');
    $pdf->SetY(25);
    $pdf->SetX(138);
    ...
    $pdf->Cell(24,16,'   IVA D�bito',1,0,'L');
                    ...
                    if (mysqli_num_rows($detfac1) > 0) {
                        ...
                        if ($row_cabefac = mysqli_fetch_assoc($cabefac)) {
                            if ($row_cabefac["tcomp"] == 54) {
                                ...
                                $query_detfacfc2 = utf8_encode("SELECT * FROM detfac WHERE tcomp = $tcomp_cabefac AND ncomp = $ncomp_cabefac AND (descrip like '%PUBLIC%' OR descrip like '%SE�A%' ");
                                if ($detfacfc2 = mysqli_query($amercado, $query_detfacfc2)) // or die("ERROR LEYENDO DETFAC 774 ".$query_detfacfc2." ");
                                if (mysqli_num_rows($detfacfc2) > 0) {
                                    $row_detfacfc2 = mysqli_fetch_assoc($detfacfc2);
                                    $cliente      = $row_cabecerafac["cliente"];
...
$pdf->SetY(22);
$pdf->SetX(70);
$pdf->Cell(26,16,'   Publicidad y',0,0,'L');
$pdf->SetY(25);
$pdf->SetX(96);
$pdf->Cell(26,16,'    Servicios ',1,0,'L');
$pdf->Cell(26,16,'    Comisiones ',1,0,'L');
