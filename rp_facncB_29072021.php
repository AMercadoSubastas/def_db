...
//Agregamos la libreria para generar c�digos QR
...
mysqli_select_db($amercado, $database_amercado);
	...
	$pncomp = $_GET['fncomp'];
	$totalFilas = 0;
    if ($ptcomp == 53 || $ptcomp == 54) {
        $query_cabf = sprintf("SELECT * FROM cabfac WHERE  tcomp IN (53,54) AND serie = %s AND ncomp = %s", $pserie, $pncomp);
        $cabefac = mysqli_query($amercado, $query_cabf) or die("ERROR LEYENDO CABECERA DE FACTURA");
