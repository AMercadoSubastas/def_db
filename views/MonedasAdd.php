...
<span id="el_monedas_activo">
    <select
        ...
        data-select2-id="fmonedasadd_x_activo"
        <?php } ?>
        data-table="monedas"
        data-field="x_activo"
...
<?php if (!$Page->activo->IsNativeSelect) { ?>
<script>
loadjs.ready("fmonedasadd", function() {
    var options = { name: "x_activo", selectId: "fmonedasadd_x_activo" },
        el = document.querySelector("select[data-select2-id='" + options.selectId + "']");
    if (!el)
        return;
    options.closeOnSelect = !options.multiple;
    options.dropdownParent = el.closest("#ew-modal-dialog, #ew-add-opt-dialog");
    if (fmonedasadd.lists.activo?.lookupOptions.length) {
        options.data = { id: "x_activo", form: "fmonedasadd" };
    } else {
        options.ajax = { id: "x_activo", form: "fmonedasadd", limit: ew.LOOKUP_PAGE_SIZE };
    }
