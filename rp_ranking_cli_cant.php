...
$pdf->Cell(200);
$pagina = $pdf->PageNo();
$pdf->Cell(30,10,'Pagina : '.$pagina,0,0,'L');
$pdf->SetY(10);
$pdf->Cell(230);
$pdf->Cell(40,10,'Fecha   : '.$fechahoy,0,0,'L');
$pdf->SetY(15);
$pdf->Cell(70);
$pdf->Cell(20,10,' Ranking de Operaciones por Cliente',0,0,'L');
$pdf->SetFont('Arial','B',9);
$pdf->SetY(25);
$pdf->Cell(10);
$pdf->Cell(60,8,'     Razon Social  ',1,0,'L');
$pdf->Cell(25,8,'       CUIT  ',1,0,'L');
$pdf->Cell(45,8,'     Cant. Operac.      ',1,0,'L');
...
{	
    ...
    if ($row_cabecerafac["tcomp"] != 115 and $row_cabecerafac["tcomp"] != 116 and $row_cabecerafac["tcomp"] != 117 and $row_cabecerafac["tcomp"] != 103 and $row_cabecerafac["en_liquid"] == 1 )
        continue;
	...
	if ($tcomp ==  119 ||  $tcomp ==  120 || $tcomp == 135 || $tcomp == 121 || $tcomp == 137) {
		$tc = "NC-";
		$signo = -1;
		$total   = 0;
	}
	...
	$total   = 1;//($row_cabecerafac["totneto105"] + $row_cabecerafac["totneto21"] + $row_cabecerafac["totcomis"]) * $signo;
...
{	
			...
			if ($acum_cli[$row_entidades["codnum"]] == 0)
				continue;
...
for ($j=0;$j < $cant_cli;$j++) {
	if ($linea < 48) {
			...
			$pdf->SetY($valor_y);
  			$pdf->Cell(1);
  			$pdf->SetX(10);
  			$pdf->Cell(80,6,$nom_cliente[$j],0,0,'L');
