 ...
 * Page class
 */
class RematesView extends Remates
{
    use MessagesTrait;
    ...
    // Page object name
    public $PageObjName = "RematesView";
    ...
    // CSS class/style
    public $CurrentPageName = "RematesView";
    ...
    public function run()
    {
        ...
        if (($keyValue = Get("codnum") ?? Route("codnum")) !== null) {
            ...
            $returnUrl = "RematesList"; // Return to list
        }
    ...
    protected function setupOtherOptions()
    {
        ...
        */
        ...
        // "detail_lotes"
        $item = &$option->add("detail_lotes");
        $body = $Language->phrase("ViewPageDetailLink") . $Language->tablePhrase("lotes", "TblCaption");
        ...
        $body = "<a class=\"btn btn-default ew-row-link ew-detail\" data-action=\"list\" href=\"" . HtmlEncode(GetUrl("LotesList?" . Config("TABLE_SHOW_MASTER") . "=remates&" . GetForeignKeyUrl("fk_ncomp", $this->ncomp->CurrentValue) . "")) . "\">" . $body . "</a>";
        $links = "";
        ...
        if ($detailPageObj->DetailView && $Security->canView() && $Security->allowView(CurrentProjectID() . 'remates')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-view\" data-action=\"view\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailViewLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getViewUrl(Config("TABLE_SHOW_DETAIL") . "=lotes"))) . "\">" . $Language->phrase("MasterDetailViewLink", null) . "</a></li>";
            if ($detailViewTblVar != "") {
                $detailViewTblVar .= ",";
            }
        ...
        if ($detailPageObj->DetailEdit && $Security->canEdit() && $Security->allowEdit(CurrentProjectID() . 'remates')) {
            $links .= "<li><a class=\"dropdown-item ew-row-link ew-detail-edit\" data-action=\"edit\" data-caption=\"" . HtmlTitle($Language->phrase("MasterDetailEditLink")) . "\" href=\"" . HtmlEncode(GetUrl($this->getEditUrl(Config("TABLE_SHOW_DETAIL") . "=lotes"))) . "\">" . $Language->phrase("MasterDetailEditLink", null) . "</a></li>";
            if ($detailEditTblVar != "") {
                $detailEditTblVar .= ",";
            }
    ...
    */
    public function exportData($doc, $keys)
    {
        ...
        // Export detail records (lotes)
        if (Config("EXPORT_DETAIL_RECORDS") && in_array("lotes", explode(",", $this->getCurrentDetailTable()))) {
            $lotes = new LotesList();
            $rsdetail = $lotes->loadRs($lotes->getDetailFilterFromSession(), $lotes->getSessionOrderBy()); // Load detail records
            if ($rsdetail) {
                ...
                if (!$this->isExport("csv") || Config("EXPORT_DETAIL_RECORDS_FOR_CSV")) {
                    ...
                    $oldtbl = $doc->getTable();
                    $doc->setTable($lotes);
                    $lotes->exportDocument($doc, $rsdetail, 1, $detailcnt);
                    $doc->setTable($oldtbl);
                }
