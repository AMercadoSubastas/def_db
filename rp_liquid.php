...
$saldoafav   = number_format($saldoafav, 2, ',','.');
    ...
    if ($ptcomp == 31 && $iva == 7) {
        ...
        $pdf->SetY(228);
        $pdf->SetX(40);
        $pdf->Cell(30,10,utf8_decode("Presenta irregularidades en categorización como responsable inscripto en el impuesto al valor agregado"),0,0,'L');
    }
	...
	echo "REGISTROS = ".$totalRows_cartval." - ";
	$efectivo ="";	$cheques = "";	$depositos ="";	$retenciones = ""; $facturas_asoc = "";
	$str_chk ="";	$str_dep ="";	$str_efe ="";	$str_ret =""; $str_facturas_asoc = 0; $str_fact = "";
	...
	while($row_cartval = mysqli_fetch_array($cartval)) { 
		echo " IMPORTE = ".$row_cartval["importe"]." _ ";
    ...
    // Facturas asociadas
	if  ($tipo_comp=='115'  || $tipo_comp=='116'   || $tipo_comp=='117'  || $tipo_comp=='125'  || $tipo_comp=='126'   || $tipo_comp=='127') {
		// Entro la primera vez
		
	    $str_facturas_asoc = $importe + $str_facturas_asoc;
        $str_fact .= $codchq." - ";
		echo "FACTURAS ".$str_fact."  IMPORTE = ".$str_facturas_asoc." - ";
        //$importe = number_format($importe, 2, ',','.') ;
		$vez_fact= 1;
	}
...
$query_lotes = "SELECT * FROM detfac WHERE codrem = '$remate' AND (tcomp='115' OR tcomp='116' OR tcomp='117' OR tcomp='119'   OR tcomp='120') AND tcomsal = $ptcomp AND ncompsal= '$liquidacion'  ORDER BY ncomp ASC";
...
while($row_lotes = mysqli_fetch_array($lotes)) 
{
	...
	$neto = $row_lotes["neto"];
	if ($row_lotes["tcomp"] == 119 || $row_lotes["tcomp"] == 120 || $row_lotes["tcomp"] == 121 || $row_lotes["tcomp"] == 135 || $row_lotes["tcomp"] == 137)
		$neto = $neto * -1;
	...
	if ($tcomp == 115 || $tcomp == 116 || $tcomp == 117 )
		$tc = "Fc-";
	if ($tcomp == 119 || $tcomp == 120 || $tcomp == 121 || $tcomp == 137 )
		$tc = "Nc-";
...
$query_lotes = "SELECT * FROM detfac WHERE codrem = '$remate' AND (tcomp='115' OR tcomp='116' OR tcomp='117' OR tcomp='119' OR tcomp='120' OR tcomp='121' OR tcomp='122' OR tcomp='123' OR tcomp='124' OR tcomp='136')  AND  tcomsal = $ptcomp AND ncompsal='$liquidacion' ORDER BY ncomp ASC";
